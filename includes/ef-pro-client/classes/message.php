 ...
 *
 * Message in this context is local record of messages saved to EF Pro
 *
...
class message extends json_arrayable
{
	 ...
	 * @since 0.0.1
	 *
	 * @param int $efp_id ID on remote app
	 ...
	 */
	public function __construct($efp_id, $hash, $local_id, $entry_id = null, $type = 'main')
	{
	...
	public static function from_array(array $data)
	{
		...
		$obj = new self($data[ 'efp_id' ], $data[ 'hash' ], $data[ 'local_id' ], $data[ 'type' ]);
			...
			case 'save' :
				if ( empty($this->local_id) ) {
					return container::get_instance()->get_messages_db()->create(
						$this->get_efp_id(), $this->get_hash(), $this->get_entry_id()
					);
				}
				break;
			case 'delete' :
	...
	/**
	 * Get Engage Forms Pro ID (ID on remote app)
	 *
	 * @since 0.0.1
	 *
	 * @return int
	 */
	public function get_efp_id()
	{
		return $this->efp_id;
	}
	...
	{
		return $this->get_client()->send_saved($this->get_efp_id());
	}
	...
	public function get_pdf_link()
	{
		return add_query_arg('public', $this->get_client()->get_keys()->get_public(),
			engage_forms_pro_app_url() . '/pdf/' . $this->get_hash());
	}
	...
	{
		return $this->get_client()->get_html($this->get_efp_id());
	}
