...
namespace engagewp\engageforms\pro\log;
...
/**
 * Class handler - Monolog handler for EF Pro remote logging
 * @package engagewp\engageforms\pro\log
 */
class handler extends AbstractHandler
{
	...
	/**
	 * Name of EF transient for tracking recents
	 *
	 * @since 0.11.0
	 *
	 * @var string
	...
	protected function high_enough_level($level)
	{
		 ...
		 *
		 */
		$highest = apply_filters('engage_forms_pro_minimum_level',
	...
	protected function prepare(array $record)
	{
		...
		if ( method_exists('Engage_Forms_DB_Tables',
	...
	protected function is_recent_repeat($message)
	{
		if ( empty($this->tracking_key) ) {
			$this->tracking_key = engage_forms_pro_log_tracker_key(EF_PRO_VER);
		}
		...
		$this->recents[] = $hash;
		\Engage_Forms_Transient::set_transient($this->tracking_key, $this->recents);
		return false;
	...
	protected function get_recent()
	{
		if ( empty($this->recents) ) {
			$this->recents = \Engage_Forms_Transient::get_transient($this->tracking_key);
