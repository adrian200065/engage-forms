...
namespace engagewp\engageforms\pro\api\local;
...
/**
 * Class files
 * @package engagewp\engageforms\pro\api\local
 */
class files implements \Engage_Forms_API_Route {
	...
	public function key_check(\WP_REST_Request $request)
	{
		...
		return engage_forms_pro_compare_to_saved_keys($public, $token);
	}
	...
	public function get_file(\WP_REST_Request $request)
	{
		...
		if ( $file && file_exists($file) ) {
			...
			if( ! $this->verify_file($file,$hash ) ){
				return new \WP_Error( 412, 'Hash is not valid', 'engage-forms' );
			}
		...
		container::get_instance()->get_logger()->send('File not found by EF Pro for attachment', [
			'file' => $request[ 'file' ],
		]);

		return new \WP_Error('404', __('File not found', 'engage-forms'));
	}
