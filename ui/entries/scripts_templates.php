...
<script type="text/javascript">
	var init_ef_baldrick;
	var perPage = jQuery('#ef-entries-list-items').val();
	function ef_set_limits( el ){
		jQuery( el ).data('perpage', perPage );
	}

	function ef_refresh_view(obj){
		...
		if(obj.rawData.undo === obj.params.trigger.data('panel')){
			obj.params.trigger.closest('tr').addClass('ef-deleted-row');
		}else{
			obj.params.trigger.closest('tr').removeClass('ef-deleted-row');
		}
	...
	function setup_pagination(obj){
		...
		var total			= obj.rawData.total,
			...
			exporter		= jQuery('.engage-entry-exporter'),
			...
			pagenav			= jQuery('.engage-table-nav'),
			...
			bulk_actions	= jQuery('#ef_bulk_action'),
		...
		exporter.find('.engage-forms-entry-exporter').attr('href', 'admin.php?page=engage-forms&export=' + form);
		exporter.show();
		pagenav.show();
	...
	jQuery(function($){
		...
		init_ef_baldrick = function(){
			$('.efajax-trigger').baldrick({
				...
				callback : function( obj ){
					...
					if( obj.params.trigger.is( 'form' ) ){
						...
						form = obj.params.target.find( 'form.engage_forms_form' );
					}
					if( form.length ){
						var validate = form.parsley({
							errorsWrapper : '<span class="help-block engage_ajax_error_block"></span>',
		...
		function do_page_navigate(el){
			...
			var	form_trigger	= $('.form_entry_row.highlight').find('.form-entry-trigger'),
				...
				perpage         = parseInt( $( '#ef-entries-list-items' ).val() );
		...
		$( document ).on('ef.add ef.remove ef.enable ef.disable', function(){
			$(window).trigger('resize');
		});
	});
	...
	jQuery(document).on('change', '#ef-entries-list-items', function(){
	...
	jQuery(function($){
		...
		$('body').on('change', '.ef-bulkcheck', function(){

			var checked = $(this),
				parent = checked.closest('.ef-table-viewer'),
				checks = parent.find('.ef-entrycheck'),
				action = $('#ef_bulk_action');

			checks.prop('checked', checked.prop('checked'));

		});

		$('body').on('change', '.ef-entrycheck', function(){

			var checkall	= $('.ef-bulkcheck'),
				allchecks	= $('.ef-entrycheck'),
				checked 	= $('.ef-entrycheck:checked');
		...
		$('body').on('click', '.ef-bulk-action', function(){

			var action 		= $('#ef_bulk_action'),
				bulkCheck 	= $('.ef-bulkcheck'),
				nonce 		= $('#ef_toolbar_actions'),
			...
			var checks 	= $('#form-entries-viewer .ef-entrycheck:checked'),
...
<script type="text/html" id="forms-list-alt-tmpl">
	...
	<div class="list form-panel postbox" data-form="{{form}}">
		<table class="table table-condensed ef-table-viewer">
			<thead>
			<tr>
				<th style="width:16px;"><input type="checkbox" class="ef-bulkcheck"></th>
				<th><?php esc_html_e('ID', 'engage-forms' ); ?></th>
				<th><?php esc_html_e('Submitted', 'engage-forms' ); ?></th>
				{{#each fields}}
				<th>{{this}}</th>
				{{/each}}
			...
			<tr id="entry_row_{{_entry_id}}">
				<td style="width:16px;"><input type="checkbox" class="ef-entrycheck" value="{{_entry_id}}"></td>
				<td>{{_entry_id}}</td>
				<td>{{_date}}</td>
				{{#each data}}
				...
				<td style="text-align: right; width: 100px;white-space: nowrap;">
					<?php
						do_action('engage_forms_entry_actions');
					?>
				</td>
			</tr>
			{{/each}}
			{{else}}
			<tr><td colspan="100"><?php esc_html_e('No entries found', 'engage-forms' ); ?></td></tr>
			{{/if}}
			</tbody>
		</table>
	</div>
	{{else}}
	<p class="description"><?php esc_html_e('No entries yet.', 'engage-forms' ); ?></p>
	{{/if}}
</script>
...
<script type="text/html" id="view-entry-tmpl">
	...
	<div id="main-entry-panel" class="tab-detail-panel" data-tab="<?php esc_html_e('Entry', 'engage-forms' ); ?>">
		<h4>
			<?php  esc_html_e('Submitted', 'engage-forms' ); ?> <small class="description">{{date}}</small>
		</h4>
		<hr>
		{{#each data}}
	...
	<div id="meta-{{@key}}" data-tab="{{name}}" class="tab-detail-panel">
		...
		{{/each}}
		{{/unless}}
		<?php do_action('engage_forms_entry_meta_templates'); ?>
	</div>
	{{/each}}
	{{/if}}
