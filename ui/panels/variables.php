...
<div class="engage-config-group">
	...
	<div class="engage-config-field" style="margin: 0px; padding: 4px 0px; width: 401px;">
		<div><?php echo __('Value', 'engage-forms'); ?></div>
	</div>
	<?php echo __('Behaviour', 'engage-forms'); ?>
</div>
<div id="variable_entry_list_none" <?php if(!empty($element['variables'])){ echo 'style="display:none;"'; } ?>><p class="description"><?php echo __('No variables defined', 'engage-forms'); ?></p></div>
	...
	if(!empty($element['variables']['keys'])){
		foreach($element['variables']['keys'] as $var_index=>$var_value){
			...
			<div class="engage-config-group">
				...
				<div class="engage-config-field" style="width:400px;">
				...
				<select name="config[variables][types][]" class="field-config" style="vertical-align: baseline;">
					<option value="static" <?php if( $element['variables']['types'][$var_index] == 'static'){ echo 'selected="selected"'; } ?>><?php echo __('Static', 'engage-forms'); ?></option>
					<option value="passback" <?php if( $element['variables']['types'][$var_index] == 'passback'){ echo 'selected="selected"'; } ?>><?php echo __('Passback', 'engage-forms'); ?></option>
					<option value="entryitem" <?php if( $element['variables']['types'][$var_index] == 'entryitem'){ echo 'selected="selected"'; } ?>><?php echo __('Entry List', 'engage-forms'); ?></option>
				</select>
...
<?php do_action( 'engage_forms_variables_config', $element ); ?>
<script type="text/html" id="variable-fields-tmpl">
	<div class="engage-config-group">
		...
		<select name="config[variables][types][]" class="field-config" style="vertical-align: baseline;">
			<option value="static"><?php echo __('Static', 'engage-forms'); ?></option>
			<option value="passback"><?php echo __('Passback', 'engage-forms'); ?></option>
			<option value="entryitem"><?php echo __('Entry List', 'engage-forms'); ?></option>
		</select>
		&nbsp;<button type="button" class="button remove-this-variable">&times;</button>
	</div>
</script>
<script>
	jQuery(document).on('click', '.remove-this-variable', function(){		
		jQuery(this).closest('.engage-config-group').remove();
	...
	jQuery(document).on('click', '.engage-add-variable', function(e){
		e.preventDefault();
	...
	function ef_set_system_variable() {
		//set-system-variable
		...
		system_values.variable = {
			...
			type	:	"<?php echo __('Variables', 'engage-forms'); ?>",
			wrap	:	['{','}']
		};
		// add values
		for(var v = 0; v < variables.length; v++){
			if(variables[v].value.length){
				...
				jQuery(variables[v]).closest('.engage-config-group').find('.var-value').data('parent', variables[v].value);
			}
		}
	}	
