...
//dump($element,0);
// Get Processors
$form_processors = $processors = Engage_Forms_Processor_Load::get_instance()->get_processors();
...
foreach($form_processors as $processor=>$config){
	...
	if(!empty($config['default'])){
		$form_processors_defaults[] = "processor_defaults." . sanitize_key( $processor ) . "_efg = " . json_encode($config['default']) .";";
	}
...
function processor_line_template($id = '{{id}}', $type = null){
	...
	$type_name = __('New Form Processor', 'engage-forms');
	if(!empty($type)){
	...
	?>
	<li class="<?php echo esc_attr('engage-processor-nav ' . $id); ?> <?php if(!empty($type)){ echo 'processor_type_'.$type; }; ?>" data-pid="<?php echo esc_attr($id); ?>">
		<a href="#<?php echo $id; ?>">
		<?php echo $type_name; ?>
...
function processor_wrapper_template($id = '{{id}}', $type = '{{type}}', $config_str = '{"default":"default value"}', $run_times = false, $conditions_str = '{"type" : ""}'){
	...
	?>
	<div class="engage-editor-processor-config-wrapper engage-editor-config-wrapper processor-<?php echo $type; ?>" id="<?php echo $id; ?>" data-type="<?php echo $type; ?>" style="display:none;">

		<div class="toggle_option_tab" data-pid="<?php echo esc_attr($id); ?>">
			<a href="<?php echo  esc_attr('#' .$id . '_settings_pane' ); ?>" class="button button-primary"><?php echo esc_html__( 'Settings', 'engage-forms' ); ?></a>
			<a href="<?php echo  esc_attr('#' .$id . '_conditions_pane' ); ?>" class="button set-conditions" data-pid="<?php echo  esc_attr($id); ?>">
                <?php echo esc_html__( 'Conditions', 'engage-forms' ); ?>
            </a>
		</div>
		<h3 data-title="<?php esc_html_e( 'New Form Processor', 'engage-forms'); ?>" class="engage-editor-processor-title"><?php echo $type_name; ?></h3>
		<div id="<?php echo esc_attr( $id . "_settings_pane"); ?>" class="wrapper-instance-pane">
			<div class="toggle_processor_event">
				<label
                    title="<?php echo esc_attr( __('Enable / Disable Processor', 'engage-forms') ); ?>"
                ...
                >
                    ...
                    >
                        <?php esc_html_e( 'Disable Processor', 'engage-forms' ); ?>
                    </span>
                    <span
                        class="not_active"
                        style="<?php if(  $enabled ){ ?> display:none;<?php } ?>"
                    >
                        <?php esc_html_e( 'Enable Processor', 'engage-forms' ); ?>
                    </span>
				</label>
            </div>
			<div class="engage-config-processor-notice" style="<?php if( $enabled ){ ?> display:none;<?php } ?>clear: both; padding: 20px 0px 0px;">
				<p style="padding:12px; text-align:center;background:#e7e7e7;" class="description">
                    <?php esc_html_e('Processor is currently disabled', 'engage-forms'); ?>
                </p>
			</div>
			<div class="engage-config-group" style="display:none;">
				<label for="<?php echo esc_attr($id . "_type" ); ?>">
                    <?php esc_html_e( 'Processor Type', 'engage-forms'); ?>
                </label>
				<div class="engage-config-field">
					<select class="block-input engage-select-processor-type" id="<?php echo esc_attr($id . "_type" ); ?>" name="config[processors][<?php echo esc_attr($id); ?>][type]" data-type="<?php echo esc_attr($type); ?>">
						<?php
			...
			<div class="engage-config-processor-setup" <?php if( ! $enabled ){ ?> style="display:none;"<?php } ?>>
			</div>
			<input type="hidden" class="processor_config_string block-input" value="<?php echo htmlentities( $config_str ); ?>" />
			<br>
			<br>
			<button class="button block-button delete-processor" data-confirm="<?php esc_html_e( 'Are you sure you want to remove this processor?', 'engage-forms'); ?>" type="button">
                <?php esc_html_e( 'Remove Processor', 'engage-forms'); ?>
            </button>
		</div>
...
<div class="engage-editor-processors-panel-wrap">
	<div class="engage-editor-processors-panel">
		...
		data-modal-title="<?php esc_html_e( 'Form Processors', 'engage-forms'); ?>"
		data-modal-height="700"
		data-modal-width="600"
		data-template="#form-processors-tmpl"
		data-callback="hide_single_processors"
		><?php esc_html_e( 'Add Processor', 'engage-forms'); ?></button>
	...
	<div class="engage-editor-processor-config">
	<?php
	...
	if(!empty($element['processors'])){
		foreach($element['processors'] as $processor_id=>$config){
			if(!empty($config['type'])){
				...
				// as was the ef_version in form config. so its safe to say that id this value is set, its the same version or higher
				if( empty( $element['ef_version'] ) ){
					$run_times = false;
				}else{
					$run_times = array();
				}				
...
<script type="text/html" id="form-processors-tmpl">
	<?php
		...
		foreach($form_processors as $processor_id=>$processor){
			$icon = EFCORE_URL . "assets/images/processor.png";
			...
			echo '<div class="form-modal-add-line'. ( !empty($processor['single']) ? ' is_single_processor' : null ) . '" data-type="' . $processor_id . '">';
				echo '<button type="button" class="button info-button add-new-processor" data-type="' . $processor_id . '">' . __('Use Processor', 'engage-forms') . '</button>';
				...
				if(!empty($processor['author'])){
					...
					echo '&nbsp' . esc_html__( 'by', 'engage-forms' ) . '&nbsp';
...
do_action('engage_forms_processor_templates', $form_processors);
...
foreach($form_processors as $processor=>$config){
	...
	if(isset($config['template'])){
		...
		echo '<p>' . esc_html__('This processor has no configurable options.', 'engage-forms') . '</p>';
	}
	echo "\r\n</script>\r\n";
