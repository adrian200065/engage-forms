...
if( ! isset( $_GET[  Engage_Forms_Admin::EDIT_KEY  ] ) || ! is_string(  Engage_Forms_Admin::EDIT_KEY  ) ){
	wp_die( esc_html__( 'Invalid form ID', 'engage-forms'  ) );
}

if( Engage_Forms_Admin::is_revision_edit() ){
	$element = $form = Engage_Forms_Forms::get_revision( $_GET[ Engage_Forms_Admin::REVISION_KEY ]  );
} else{
	$element = $form = Engage_Forms_Forms::get_form( $_GET[ Engage_Forms_Admin::EDIT_KEY ] );

}

if( empty( $element ) || ! is_array( $element ) ){
	wp_die( esc_html__( 'Invalid Form.', 'engage-forms'  ) );
}
/**
 ...
 */
do_action( 'engage_forms_prerender_edit', $element );
 ...
 */
$magic_tags = apply_filters( 'engage_forms_get_magic_tags', array(), $element['ID'] );

if(empty($element['success'])){
	$element['success'] = esc_html__( 'Form has been successfully submitted. Thank you.', 'engage-forms' );
}
...
// place nonce field
wp_nonce_field( 'ef_edit_element', 'ef_edit_nonce' );
...
do_action('engage_forms_edit_start', $element);

// Get Fieldtpyes
$field_types = Engage_Forms_Fields::get_all();

// Get Elements
$panel_extensions = Engage_Forms_Admin_Panel::get_panels();
...
<div class=\"engage-config-group engage-config-group-full\">
	<div class=\"engage-config-group\">
		<div class=\"engage-config-field\">
			<label><input id=\"{{_id}}_auto\" type=\"checkbox\" class=\"auto-populate-options field-config\" name=\"{{_name}}[auto]\" value=\"1\" {{#if auto}}checked=\"checked\"{{/if}}> ".esc_html__( 'Auto Populate', 'engage-forms' )."</label>
		</div>
	</div>
</div>
{{#if auto}}{{#script}}jQuery('#{{_id}}_auto').trigger('change');{{/script}}{{/if}}
<div class=\"engage-config-group-auto-options\" style=\"display:none;\">
	<div class=\"engage-config-group\">
		<label>". esc_html__( 'Source', 'engage-forms' ) . "</label>
		<div class=\"engage-config-field\">
			<select class=\"block-input field-config auto-populate-type\" name=\"{{_name}}[auto_type]\">
				<option value=\"\">" . esc_html__( 'Select a source', 'engage-forms' ) . "</option>
				<option value=\"post_type\"{{#is auto_type value=\"post_type\"}} selected=\"selected\"{{/is}}>" . esc_html__( 'Post Type', 'engage-forms' ) . "</option>
				<option value=\"taxonomy\"{{#is auto_type value=\"taxonomy\"}} selected=\"selected\"{{/is}}>" . esc_html__( 'Taxonomy', 'engage-forms' ) . "</option>";
				ob_start();
				 ...
				 * @since unknown
				 */
				do_action( 'engage_forms_autopopulate_types' );
				$field_options_template .= ob_get_clean() . "
			</select>
		</div>
	</div>
	
	<div class=\"engage-config-group engage-config-group-auto-taxonomy auto-populate-type-panel\" style=\"display:none;\">
		<label>". esc_html__( 'Taxonomy', 'engage-forms' )."</label>
		<div class=\"engage-config-field\">
	...
	<div class=\"engage-config-group engage-config-group-auto-post_type auto-populate-type-panel\" style=\"display:none;\">
		<label>".esc_html__( 'Post Type', 'engage-forms' ) ."</label>
		<div class=\"engage-config-field\">
	...
	<div class=\"engage-config-group engage-config-group-auto-taxonomy engage-config-group-auto-post_type auto-populate-type-panel\" style=\"display:none;\">
		<label>". esc_html__( 'Value', 'engage-forms' )."</label>
		<div class=\"engage-config-field\">
	...
	<div class=\"engage-config-group engage-config-group-auto-taxonomy auto-populate-type-panel\" style=\"display:none;\">
		<label>". esc_html__( 'Orderby', 'engage-forms' )."</label>
		<div class=\"engage-config-field\">
	 ...
	 * @since unknown
	 */
	do_action( 'engage_forms_autopopulate_type_config' );
	 ...
	 */
	$option_presets = apply_filters( 'engage_forms_field_option_presets', array(), $element );
...
<div class=\"engage-config-group-toggle-options\" {{#if auto}}style=\"display:none;\"{{/if}}>
	<div class=\"engage-config-group engage-config-group-full\">
		<button type=\"button\" class=\"button add-toggle-option add-option\" style=\"width: 180px;\">" . esc_html__( 'Add Option', 'engage-forms' ) . "</button>
		<button type=\"button\" data-bulk=\"#{{_id}}_bulkwrap\" class=\"button add-toggle-option\" style=\"width: 190px;\">" . esc_html__( 'Bulk Insert / Preset', 'engage-forms' ) . "</button>
		...
		<option value=\"\">" . esc_html__( 'Select a preset', 'engage-forms' ) . "</option>
		" . $preset_options . "
		</select>		
		<textarea style=\"resize:vertical; height:200px;\" class=\"block-input\" id=\"{{_id}}_batch\"></textarea>
		<p class=\"description\">" . esc_html__( 'Single option per line. These replace the current list.', 'engage-forms' ) . "</p>
		<button type=\"button\" data-options=\"#{{_id}}_batch\" class=\"button block-button add-toggle-option\" style=\"margin: 10px 0;\">" . esc_html__( 'Insert Options', 'engage-forms' ) . "</button>
		</div>
	</div>
	<div class=\"engage-config-group engage-config-group-full\">
	<label style=\"padding: 10px;\"><input type=\"radio\" class=\"toggle_set_default field-config\" name=\"{{_name}}[default]\" value=\"\" {{#unless default}}checked=\"checked\"{{/unless}}> " . esc_html__( 'No Default', 'engage-forms' ) . "</label>
	<label class=\"pull-right\" style=\"padding: 10px;\"><input type=\"checkbox\" class=\"toggle_show_values field-config\" name=\"{{_name}}[show_values]\" value=\"1\" {{#if show_values}}checked=\"checked\"{{/if}}> " . esc_html__( 'Show Values', 'engage-forms' ) . "</label>
	</div>
	
	<div class=\"engage-config-group engage-config-group-full toggle-options field-options engage-config-field\" data-field=\"{{_id}}\">
		{{#each option}}
			<div class=\"toggle_option_row 315\">
					...
					<a href=\"https://engageforms.com/doc/select-options/?utm_source=wp-admin&utm_medium=form-editor&utm_content=select-options\" target=\"_blank\" class=\"dashicons dashicons-editor-help\" style=\"float:right;\" data-toggle=\"tooltip\" data-placement=\"bottom\"  title=\"" . esc_attr( __( 'Learn more about using select field options', 'engage-forms' ) ) . "\"></a>
		
					<div class=\"engage-config-group\">
						<label class=\"option-setting-label option-setting-label-for-value\" for=\"opt-calc-val-{{@key}}\">
							". esc_html__( 'Calculation Value', 'engage-forms' ) . "
						</label>
						<input {{#unless ../show_values}} style=\"display:none;\"{{/unless}} type=\"text\" class=\"toggle_calc_value_field field-config option-setting \" name=\"{{../_name}}[option][{{@key}}][calc_value]\" value=\"{{#if ../show_values}}{{calc_value}}{{else}}{{label}}{{/if}}\" placeholder=\"" . esc_attr( __( 'Calculation Value', 'engage-forms' ) ) . "\" id=\"opt-calc-val-{{@key}}\" data-opt=\"{{@key}}\" />
					</div>
					
					<div class=\"engage-config-group\">
						<label class=\"option-setting-label option-setting-label-for-value\" for=\"opt-val-{{@key}}\">
							". esc_html__( 'Value', 'engage-forms' ) . " 
						</label>
						<input  {{#unless ../show_values}} style=\"display:none;\"{{/unless}} type=\"text\" class=\"toggle_value_field option-setting field-config  required \" name=\"{{../_name}}[option][{{@key}}][value]\" value=\"{{#if ../show_values}}{{value}}{{else}}{{label}}{{/if}}\"" . esc_attr( __( 'Value', 'engage-forms' ) ) . "\" id=\"opt-val-{{@key}}\" data-opt=\"{{@key}}\" />
						
					</div>
					
					<div class=\"engage-config-group\">
						<label class=\"option-setting-label option-setting-label-for-label\" for=\"opt-label-{{@key}}\">
							". esc_html__( 'Label', 'engage-forms' ) . "
						</label>
						<input {{#unless ../show_values}} style=\"width:245px;\"{{/unless}} type=\"text\" data-option=\"{{@key}}\" class=\"toggle_label_field option-setting field-config required\" name=\"{{../_name}}[option][{{@key}}][label]\" value=\"{{label}}\" placeholder=\"" . esc_attr( __( 'Label', 'engage-forms' ) ) . "\" for=\"opt-label-{{@key}}\" data-opt=\"{{@key}}\" />
					</div>	
	...
	<div style=\"display:none;\" class=\"notice error\">
		<p>" . esc_html__( 'Option values must be unique.', 'engage-forms' ) . "</p>
	</div>
</div>
";
...
$default_template = "
<div class=\"engage-config-group\">
	<label>" . esc_html__( 'Default', 'engage-forms' ) . "</label>
	<div class=\"engage-config-field\">
...
// type list
$field_type_list = array(
	esc_html__( 'Basic', 'engage-forms' )       => array(),
	esc_html__( 'Select', 'engage-forms' )         => array(),
	esc_html__( 'eCommerce', 'engage-forms' )         => array(),
	esc_html__( 'File', 'engage-forms' )      => array(),
	esc_html__( 'Content', 'engage-forms' )      => array(),
	esc_html__( 'Special', 'engage-forms' ) => array(),
...
foreach($field_types as $field_slug=>$config){
	...
	foreach((array) $categories as $category){
		if( !isset( $field_type_list[trim($category)] ) ){
			$category = esc_html__( 'Special', 'engage-forms' );
		}
		$field_type_list[trim($category)][$field_slug] = $config;
	}

	ob_start();
	do_action('engage_forms_field_settings_template', $config, $field_slug );
	...
	if(!empty($config['setup']['default'])){
		$field_type_defaults[] = "fieldtype_defaults." . sanitize_key( $field_slug ) . "_efg = " . json_encode($config['setup']['default']) .";";
	}
	...
	if(empty($config['setup']['preview']) || !file_exists( $config['setup']['preview'] )){
		...
		if(!empty($config['setup']['preview']) && function_exists($config['setup']['preview'])){
			...
			$wrapper_before = "<div class=\"preview-engage-config-group\">";
			$field_before = "<div class=\"preview-engage-config-field\">";
			$field_after = '</div>';
			$wrapper_after = '</div>';
			...
			if( file_exists( $config[ 'file' ] ) ){
				...
				$file = EFCORE_PATH . 'fields/generic-input';
			}
			ob_start();
			include $file;
...
function engage_forms_field_wrapper_template($field_id = '{{field_id}}', $label = '{{label}}', $slug = '{{slug}}', $caption = '{{caption}}', $hide_label = '{{hide_label}}', $required = '{{required}}', $entry_list = '{{entry_list}}', $type = null, $config_str = '{{json config}}', $conditions_str = '{"type" : ""}'){
...
$form = [];
	...
	?>
	<div class="engage-editor-field-config-wrapper engage-editor-config-wrapper ajax-trigger" 
	
        data-request="setup_field_type"
        data-event="field.drop"
        data-load-class="none"
        data-modal="field_setup"
        data-modal-title="<?php echo esc_html__( 'Fields', 'engage-forms' ); ?>"
    ...
    >
		...
		<h3 class="engage-editor-field-title">
		...
		<div id="<?php echo esc_attr($field_id); ?>_settings_pane" class="wrapper-instance-pane">
            ...
            <div class="engage-config-group">
				<label for="<?php echo esc_attr($field_id); ?>_type"><?php echo esc_html__( 'Field Type', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
					<select class="block-input engage-select-field-type" data-field="<?php echo esc_attr($field_id); ?>" id="<?php echo esc_attr($field_id); ?>_type" name="config[fields][<?php echo esc_attr($field_id); ?>][type]" data-type="<?php echo $type; ?>">
						<?php
			...
			<div class="engage-config-group">
				<label for="<?php echo esc_attr($field_id); ?>_fid"><?php echo esc_html__( 'ID', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group">
				<label for="<?php echo esc_attr($field_id); ?>_lable"><?php echo esc_html__( 'Name', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group hide-label-field">
				<label for="<?php echo esc_attr($field_id); ?>_hide_label"><?php echo esc_html__( 'Hide Label', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group">
				<label for="<?php echo esc_attr($field_id); ?>_slug"><?php echo esc_html__( 'Slug', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group required-field">
				<label for="<?php echo esc_attr($field_id); ?>_required"><?php echo esc_html__( 'Required', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group caption-field">
				<label for="<?php echo esc_attr($field_id); ?>_caption"><?php echo esc_html__( 'Description', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
			...
			<div class="engage-config-group entrylist-field">
				<label for="<?php echo esc_attr($field_id); ?>_entry_list"><?php echo esc_html__( 'Show in Entry List', 'engage-forms' ); ?></label>
				<div class="engage-config-field">
             ...
             */
            do_action( 'engage_forms_field_wrapper_before_field_setup', $config, $type, $field_id );
			?>

			<div class="engage-config-field-setup">
			</div>
             ...
             * @param string $field_id Template representation of Field ID. Probably {{ID}}
			 */
			do_action( 'engage_forms_field_wrapper_after_field_setup', $config, $type, $field_id );
			?>
			 ...
			 * @param string $field_id Template representation of Field ID. Probably {{ID}}
			 */
			do_action( 'engage_forms_field_wrapper_before_delete', $config, $type, $field_id );
			?>

			<button class="button delete-field block-button" data-confirm="<?php echo esc_html__( 'Are you sure you want to remove this field?. \'Cancel\' to stop. \'OK\' to delete', 'engage-forms' ); ?>" type="button"><?php echo esc_html__( 'Delete Field', 'engage-forms' ); ?></button>
			 ...
			 * @param string $field_id Template representation of Field ID. Probably {{ID}}
			 */
			do_action( 'engage_forms_field_wrapper_after_delete', $config, $type, $field_id );
			?>
        </div>
...
function field_line_template($id = '{{id}}', $label = '{{label}}', $group = '{{group}}'){
	...
	?>
	<li data-field="<?php echo $id; ?>" class="engage-field-line">
		<a href="#<?php echo $id; ?>">
			<i class="icn-right pull-right"></i>
			<i class="icn-field"></i>
			<?php echo htmlentities( $label ); ?>
		</a>
		<input type="hidden" class="engage-config-field-group" value="<?php echo $group; ?>" name="config[fields][<?php echo $id; ?>][group]" autocomplete="off">
	</li>
	<?php
...
<div class="engage-editor-header">
	...
	<div class="updated_notice_box">
		<?php esc_html_e( 'Updated Successfully', 'engage-forms'  ); ?>
	</div>


	<?php if( ! Engage_Forms_Admin::is_revision_edit() ){ ?>

		<div id="engage-header-save-button"></div>

		<a class="button engage-header-preview-button" target="_blank" href="<?php echo esc_url( Engage_Forms_Admin::preview_link( $element[ 'ID' ] ) ); ?>">
			<?php esc_html_e( 'Preview Form', 'engage-forms' ); ?>
		</a>
	<?php
	}else{ ?>

		<a
			href="<?php echo esc_url( Engage_Forms_Admin::form_edit_link($element[ 'ID' ] )); ?>"
		    class="button engage-header-return-button"
			id="engage-forms-return-from-revision"
		>
			<?php esc_html_e( 'Exit Revision', 'engage-forms' ); ?>
			
		</a>

		<a
			href="#"
		    class="button button-primary engage-header-restore-button"
			id="engage-forms-restore-revision"
			data-form="<?php echo esc_attr( $element[ 'ID' ] ); ?>"
			data-revision="<?php echo esc_attr( $element[ 'db_id' ] ); ?>"
		    data-edit-link="<?php echo esc_url( Engage_Forms_Admin::form_edit_link($element[ 'ID' ] )); ?>"
		>
			<?php esc_html_e( 'Restore Form Revision', 'engage-forms' ); ?>
			<span id="save_indicator" class="spinner" style="position: absolute; right: -33px;"></span>

		</a>

		<a class="button engage-header-preview-button" target="_blank" href="<?php echo esc_url( Engage_Forms_Admin::preview_link( $element[ 'ID' ], $element[ 'db_id' ] ) ); ?>">
			<?php esc_html_e( 'Preview Form Revision', 'engage-forms' ); ?>
		</a>
	...
	?>
	<a class="button engage-header-email-preview-button" target="_blank" href="<?php echo esc_url( add_query_arg( array(
			'ef-email-preview' => wp_create_nonce( $element[ 'ID' ] ),
			'ef-email-preview-form' => $element[ 'ID' ]
	),  get_home_url() ) ); ?>" <?php echo $has_email_preview; ?>>
		<?php esc_html_e( 'Preview Last Email', 'engage-forms' ); ?>
	</a>
</div>

<?php include EFCORE_PATH  . 'ui/panels/form-settings.php'; ?>
...
foreach($panel_extensions as $panel){
	...
	foreach($panel['tabs'] as $panel_slug=>$tab_setup){
		...
		echo "<div id=\"" . $panel_slug . "-config-panel\" class=\"engage-editor-body engage-config-editor-panel " . ( !empty($tab_setup['side_panel']) ? "engage-config-has-side" : "" ) . "\"".$active.">\r\n";
			if( !empty($tab_setup['side_panel']) ){
				echo "<div id=\"" . $panel_slug . "-config-panel-main\" class=\"engage-config-editor-main-panel\">\r\n";
			}
			echo '<h3>'.$tab_setup['label'];
				if( isset( $tab_setup[ 'tip' ] ) && is_array( $tab_setup[ 'tip' ] ) ) {
					printf( '<a href="%s" target="_blank" class="dashicons dashicons-editor-help engage-forms-tab-help-bubble" data-toggle="tooltip" data-placement="top"  title="%s"><span class="screen-reader-text">%s</span></a>',
						esc_url( $tab_setup[ 'tip' ][ 'link' ] ),
						esc_attr( $tab_setup[ 'tip' ][ 'text'] ),
						esc_html__( 'Click to view help doc on EngageForms.com' )
					);
				}
				if( !empty( $tab_setup['repeat'] ) ){
					// add a repeater button
					echo " <a href=\"#" . $panel_slug . "_tag\" class=\"add-new-h2 engage-add-group\" data-group=\"" . $panel_slug . "\">" . esc_html__( 'Add New', 'engage-forms' ) . "</a>\r\n";
				}
				// ADD ACTIONS
			...
			if(!empty($tab_setup['fields'])){
				...
				for($group_index = 0; $group_index < $depth; $group_index++){
					...
					if( !empty( $tab_setup['repeat'] ) ){
						echo "<div class=\"engage-config-editor-panel-group\">\r\n";
					}
					foreach($tab_setup['fields'] as $field_slug=>&$field){
						$wrapper_before = "<div class=\"engage-config-group\">";
						$field_before = "<div class=\"engage-config-field\">";
						$field_after = '</div>';
						$wrapper_after = '</div>';
					...
					if( !empty( $tab_setup['repeat'] ) ){
						echo "<a href=\"#remove_" . $panel_slug . "\" class=\"engage-config-group-remove\">" . esc_html__( 'Remove', 'engage-forms' ) . "</a>\r\n";
						echo "</div>\r\n";
					}
				}
				...
				if( !empty( $tab_setup['repeat'] ) ){
					...
					$field_template .= "	<div class=\"engage-config-editor-panel-group\">\r\n";
					...
					foreach($tab_setup['fields'] as $field_slug=>&$field){
						...
						$field_template .= "	<div class=\"engage-config-group\">\r\n";
							$field_template .= "		<label for=\"" . $field_id . "\">" . $field['label'] . "</label>\r\n";
							$field_template .= "		<div class=\"engage-config-field\">\r\n";
								ob_start();
								include $field_types[$field['type']]['file'];
					...
					$field_template .= "	<a href=\"#remove-group\" class=\"engage-config-group-remove\">" . esc_html__( 'Remove', 'engage-forms' ) . "</a>\r\n";
					$field_template .= "	</div>\r\n";
			...
			if(!empty($tab_setup['side_panel'])){
				echo "</div>\r\n";
				echo "<div id=\"" . $panel_slug . "-config-panel-side\" class=\"engage-config-editor-side-panel\">\r\n";
...
// PROCESSORS
do_action('engage_forms_edit_end', $element);
?>
...
<script type="text/html" id="form-fields-selector-tmpl">
	...
	<?php
		...
		$sorted_field_types = array(
			__( 'Basic', 'engage-forms' ) => '',
			__( 'Select', 'engage-forms' ) => '',
			__( 'File', 'engage-forms' ) => '',
			__( 'Content', 'engage-forms' ) => '',
			__( 'eCommerce', 'engage-forms' )  => '',
			__( 'Special', 'engage-forms' ) => '',
			
		);

		if( defined( 'EFCORE_SHOW_DISCONTINUED_FIELDS' ) && EFCORE_SHOW_DISCONTINUED_FIELDS  ){
			$sorted_field_types[ __( 'Discontinued', 'engage-forms' ) ] = '';
		}
		...
		foreach($field_types as $field_slug=>$config){
			...
			$svg = false;
			$icon = EFCORE_URL . "assets/images/field.png";
			...
			foreach($cats as $cat){
				$cat = trim($cat);
				if(  __( 'Discontinued', 'engage-forms' ) == $cat ){
					continue;
				}
				$template = '<div class="form-modal-add-line">';
					$template .= '<button type="button" class="button info-button set-current-field" data-field="{{id}}" data-type="' . $field_slug . '">' . esc_html__( 'Set Field', 'engage-forms' ) . '</button>';
					...
					$template .= '<p class="description">' . (!empty($config['description']) ? esc_html__( $config[ 'description' ] ) : esc_html__( 'No description given', 'engage-forms' ) ) . '</p>';
				$template .= '</div>';
				if(!isset($sorted_field_types[$cat])){
					$cat = __( 'Special', 'engage-forms' );
				}
...
<script type="text/html" id="engage_field_config_wrapper_templ">
<?php
    engage_forms_field_wrapper_template('{{id}}' );
?>
</script>
<script type="text/html" id="field-option-row-tmpl">
	...
	<div class="toggle_option_row 962">
		...
		<a href="https://engageforms.com/doc/select-options/?utm_source=wp-admin&utm_medium=form-editor&utm_content=discount" target="_blank" class="dashicons dashicons-editor-help" style="float:right;" data-toggle="tooltip" data-placement="bottom"  title="<?php  esc_attr_e( 'Learn more about using select field options', 'engage-forms'  ); ?>"></a>

		<div class="engage-config-group">
			<label class="option-setting-label option-setting-label-for-value" for="opt-calc-val-{{@key}}">
				<?php esc_html_e( 'Calculation Value', 'engage-forms' ); ?>
			</label>
			<input
				type="text"
                ...
                data-field="{{../_name}}"
				value="{{calc_value}}"
				placeholder="<?php esc_attr_e( 'Calculation Value', 'engage-forms'  ); ?>"
				id="opt-calc-val-{{@key}}"
		...
		<div class="engage-config-group">
			<label class="option-setting-label option-setting-label-for-value" for="opt-val-{{@key}}">
				<?php esc_html_e( 'Value', 'engage-forms' ); ?>
			</label>
			<input
				type="text"
				...
				value="{{value}}"
				placeholder="<?php esc_attr_e( 'Value', 'engage-forms' ); ?>"
				id="opt-val-{{@key}}"
				data-opt="{{@key}}"
				{{#unless ../show_values}} style="display:none;"{{/unless}}
			/>
		...
		<div class="engage-config-group">
			<label class="option-setting-label option-setting-label-for-label" for="opt-label-{{@key}}">
				<?php esc_html_e( 'Label', 'engage-forms' ); ?>
			</label>
			<input
				type="text"
				data-option="{{@key}}"
				...
				placeholder="<?php esc_attr_e( 'Label', 'engage-forms' ); ?>"
				for="opt-label-{{@key}}"
				data-opt="{{@key}}"
...
<script type="text/html" id="noconfig_field_templ" class="ef-editor-template">
<div class="engage-config-group">
	<label><?php _e( 'Default', 'engage-forms' ); ?></label>
	<div class="engage-config-field">
		<input type="text" class="block-input field-config" name="{{_name}}[default]" value="{{default}}">
	</div>
</div>
...
foreach($field_type_templates as $key=>$template){
	echo "<script type=\"text/html\" class=\"ef-editor-template\" id=\"" . $key . "\">\r\n";
		echo $template;
	echo "\r\n</script>\r\n";
}
?>
...
<?php
/**
 * Runs at the bottom of the Engage Forms form editor page
 *
 * @since 1.6.0
 */
do_action('engage_forms_editor_footer');
