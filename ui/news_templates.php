...
{{else}}
	<div class="alert error"><p><?php echo __('Unable to connect or no extensions available.', 'engage-forms'); ?></p></div>
{{/if}}
{{/if}}
</script>
...
{{#if channels}}
	...
	{{else}}
			{{#if url}}
				data-request="{{url}}"
			{{else}}
				data-request="<?php echo EFCORE_EXTEND_URL . '{{location}}/?version=' . EFCORE_VER; ?>"
			{{/if}}
			{{#if template}}
