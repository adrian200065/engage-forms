...
/**
 * Class Engage_Forms_GDPR
 *
 * Connects WordPress GDPR compliance tools to Engage Forms APIs.
 */
class Engage_Forms_GDPR
{
    ...
    protected static function callback($form_id, $type = 'exporter')
    {
         ...
         */
        return apply_filters( 'engage_forms_gdpr_callback', [__CLASS__, static::callback_name($form_id, $type )], $type, $form_id );
    }
    ...
    public static function enabled_forms($reset = false)
    {
        ...
        if ( ! empty($forms)) {
            foreach ($forms as $form_id) {
                $form = Engage_Forms_Forms::get_form($form_id);
                if (is_array($form) && Engage_Forms_Forms::is_privacy_export_enabled($form)) {
    ...
    {
        return 'engage-forms-' . sanitize_title_with_dashes($form['name']);
    }
    ...
    {
        if (!Engage_Forms_Forms::is_privacy_export_enabled($form)) {
            return [];
        }
        $results = self::get_results($email_address, $form, $page);
        $export_items = [];
        if (!static::done($results)) {
            $pii_fields = Engage_Forms_Forms::personally_identifying_fields($form, true);
            ...
            foreach ($results->get_fields() as $field_value) {
                ...
                if (!empty($entry['user'])) {
                    if (!empty($entry['user']['name'])) {
                        $data[] = [
                            'name' => __('WordPress User Name', 'engage-forms'),
                            'value' => $entry['user']['name']
                        ];
                    }
                    if (!empty($entry['user']['email'])) {
                        $data[] = [
                            'name' => __('WordPress User Email', 'engage-forms'),
    ...
    public static function perform_erase($email_address, array $form)
    {
        ...
        if (!static::done($results)) {
            ...
            Engage_Forms_Entry_Bulk::delete_entries($ids);
            $items_removed = true;
        }
    ...
    {
        $pii_query = new Engage_Forms_Query_Pii(
            $form,
            $email_address,
            new Engage_Forms_Query_Paginated($form, \engagewp\EngageFormsQueries\EngageFormsQueries())
        );
     ...
     * @since 1.7.0
     *
     * @param Engage_Forms_Entry_Field $result Result set
     * @return string
     */
