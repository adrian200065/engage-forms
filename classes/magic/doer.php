...
class Engage_Forms_Magic_Doer {
	...
	public static function do_field_magic( $value, $entry_id, $form ) {
		$matches = Engage_Forms_Magic_Util::explode_field_magic( $value );
		...
		if ( ! empty( $matches[ 1 ] ) ) {
			 ...
			 */
			$_value = apply_filters( 'engage_forms_pre_do_field_magic', null, $value, $matches, $entry_id, $form );
			if( ! is_null( $_value ) ){
				return $_value;
			}
			...
			foreach ( $matches[ 1 ] as $key => $tag ) {
				...
				$entry = Engage_Forms::get_slug_data( $tag, $form, $entry_id );

				$field = false;
				if ( $entry !== null ) {
					$field = Engage_Forms_Field_Util::get_field_by_slug( $tag, $form );
				}

				if( 'html' === Engage_Forms_Field_Util::get_type( $tag, $form ) ){
					$field = Engage_Forms_Field_Util::get_field($tag,$form );
					if( ! empty( $field[ 'config' ][ 'show_in_summary' ] ) ){
						$value = Engage_Forms::do_magic_tags( Engage_Forms_Field_Util::get_default( $field, $form ) );
					}
				}
			 ...
			 * @param array $form Current form ID
			 */
			return apply_filters( 'engage_forms_do_field_magic_value', $value, $matches, $entry_id, $form );
	...
	public static function do_bracket_magic( $value, $form, $entry_id, $magic_caller, $referrer ){
		...
		$form   = self::filter_form( $form, $entry_id );
		$magics = Engage_Forms_Magic_Util::explode_bracket_magic( $value );
		if ( ! empty( $magics[ 1 ] ) ) {
			 ...
			 * @param array $form Current form ID
			 */
			$_value = apply_filters( 'engage_forms_pre_do_bracket_magic', null, $value, $magics, $entry_id, $form );
			if( ! is_null( $_value ) ){
				return $_value;
			}
			...
			foreach ( $magics[ 1 ] as $magic_key => $magic_tag ) {
				...
				if ( count( $magic ) == 2 ) {
					switch ( strtolower( $magic[ 0 ] ) ) {
						case 'get':
							if ( isset( $_GET[ $magic[ 1 ] ] ) ) {
								$magic_tag = Engage_Forms_Sanitize::sanitize( $_GET[ $magic[ 1 ] ] );
							} else {
								// check on referer.
						...
						case 'post':
							if ( isset( $_POST[ $magic[ 1 ] ] ) ) {
								$magic_tag = Engage_Forms_Sanitize::sanitize( $_POST[ $magic[ 1 ] ] );
							} else {
								$magic_tag = null;
							}
							break;
						case 'request':
							if ( isset( $_REQUEST[ $magic[ 1 ] ] ) ) {
								$magic_tag = Engage_Forms_Sanitize::sanitize( $_REQUEST[ $magic[ 1 ] ] );
							} else {
								$magic_tag = null;
							}
							break;
								...
								foreach ( $form[ 'variables' ][ 'keys' ] as $var_index => $var_key ) {
									if ( $var_key == $magic[ 1 ] ) {
										if ( ! in_array( $magic_tag, $magic_caller ) ) {
											...
											$magic_tag      = Engage_Forms::do_magic_tags( $form[ 'variables' ][ 'values' ][ $var_index ], $entry_id, $magic_caller );
										} else {
					...
					switch ( $magic_tag ) {
						case 'entry_id':
							$magic_tag = Engage_Forms::get_field_data( '_entry_id', $form );
							if ( $magic_tag === null ) {
								// check if theres an entry
								if ( ! empty( $_GET[ 'ef_ee' ] ) ) {
									$entry = Engage_Forms::get_entry_detail( $_GET[ 'ef_ee' ], $form );
									if ( ! empty( $entry ) ) {
										$magic_tag = $entry[ 'id' ];
									}
								}
							...
							break;
						case 'entry_token':
							$magic_tag = Engage_Forms::get_field_data( '_entry_token', $form );
							break;
						case 'ip':
						...
						case 'summary':
							if ( ! empty( $form[ 'fields' ] ) ) {
								...
								$magic_paser = new Engage_Forms_Magic_Summary( $form, null );
						...
						case 'lostpassword_url' :
							...
							break;
						case 'current_url' :
							$magic_tag = urldecode( engage_forms_get_current_url() );
							break;
                        case 'privacy_page':
                            $magic_tag = esc_url_raw(engage_forms_privacy_policy_page_url());
							break;
						case 'referer_url' :
						case 'referrer_url' :
				 ...
				 *
				 * Can be used to make custom magic tags, but read https://engageforms.com/?post_type=doc&p=40609 first
				 *
				 * @since unknown
				 *
				 ...
				 *
				 */
				$filter_value = apply_filters( 'engage_forms_do_magic_tag', $magic_tag, $magics[ 0 ][ $magic_key ] );
			 ...
			 * @param array $form Current form ID
			 */
			return apply_filters( 'engage_forms_do_field_bracket_value', $value, $magics, $entry_id, $form );
		}
	...
	public static function magic_tag_meta_prepare( $entry_id ){
		...
		if( ! isset( self::$entry_details[ $entry_id ] ) ) {
			$entry_details = Engage_Forms::get_entry_detail( $entry_id );
			$this_form     = Engage_Forms_Forms::get_form( $entry_details[ 'form_id' ] );
	...
	public static function magic_image( array $field, $url, array $form = null ){
		if( Engage_Forms_Files::is_private( $field ) || ! filter_var( $url, FILTER_VALIDATE_URL ) ){
			return false;
		}
		 ...
		 * @param array $form Form config
		 */
		$use_link = apply_filters( 'engage_forms_magic_file_use_link', true, $field, $form );
		if( $use_link ){
			return sprintf( '<a href="%s">%s</a>', esc_url( $url ), esc_url( $url ) );
		}

		return sprintf( '<img src="%s" class="%s" />', esc_url( $url ), esc_attr( 'ef-image-magic-tag ef-image-magic-tag-' . $form[ 'ID' ]  ) );
	...
	protected static function filter_form( $form = null, $entry_id = null ){
		 ...
		 * @param array $form Form config
		 */
		$form = apply_filters( 'engage_forms_magic_form', $form, $entry_id );
