 ...
 */
abstract class Engage_Forms_Magic_Parser {
	 ...
	 * @since 1.5.0.7
	 *
	 * @var Engage_Forms_Field_Credit
	 */
	protected $credit_card_hasher;

	/**
	 * Engage_Forms_Magic_Parser constructor.
	 *
	 * @since 1.4.6
	 *
	...
	public function __construct( array $form, array $data = null ) {
		...
		/**
		 * Filter data for use by magic parsers extending the Engage_Forms_Magic_Parser class sunch as summary
		 *
		 * @since 1.5.2
		 *
		 ...
		 * @param array $form Form config
		 */
		$this->data = apply_filters( 'engage_forms_magic_parser_data', $data, $this->form );
	...
	protected function get_field_value( $field_id ){
		if( ! is_array( $this->data ) ){
			$this->data = Engage_Forms::get_submission_data( $this->form );
		}
	 ...
	 * @since 1.5.0.7
	 *
	 * @return Engage_Forms_Field_Credit
	 */
	protected function get_credit_card_hasher(){
		if( null === $this->credit_card_hasher ){
			$this->credit_card_hasher = new Engage_Forms_Field_Credit;
		}
