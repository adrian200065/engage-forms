 ...
 */
class Engage_Forms_Magic_Summary extends Engage_Forms_Magic_Parser {
	...
	protected function parse(){
		...
		if( empty( $this->ordered_fields ) ){
			$this->ordered_fields = $ordered_fields = Engage_Forms_Forms::get_fields( $this->form );
		}else{
		 ...
		 */
		$this->ordered_fields = $ordered_fields = apply_filters( 'engage_forms_summary_magic_fields', $ordered_fields, $this->form );
		...
		if ( ! empty( $ordered_fields ) ) {
			...
			foreach ( $ordered_fields as $field_id => $field ) {
				$tag_i++;
				$type = Engage_Forms_Field_Util::get_type( $field, $this->form );
				$not_support = Engage_Forms_Fields::not_support( $type, 'entry_list' );
				if( $not_support ){
				...
				if( Engage_Forms_Field_Util::is_file_field( $field_id, $this->form ) && Engage_Forms_Files::is_private( $field )  ){
					continue;
				}
				...
				switch( $type ){
					case 'html' :
						$field_value = Engage_Forms::do_magic_tags( Engage_Forms_Field_Util::get_default( $field_id, $this->form ) );
						break;
					case 'file'  :
						$field_value = Engage_Forms_Magic_Doer::magic_image( $field,  $this->get_field_value( $field_id ), $this->form );
						break;
					case 'calculation' :
						$field_value = Engage_Forms_Magic_Doer::calculation_magic( $field, $this->get_field_value( $field_id ) );
						break;
					...
					default :
						if (  null == $this->data ) {
							$field_values = (array) Engage_Forms::get_field_data( $field_id, $this->form );
						}else{
				...
				if ( $field_value !== null && ! is_array( $field_value ) && strlen( (string) $field_value ) > 0 ) {
					...
					/**
					 * Change value displayed for field inside Engage Forms summary magic tag
					 *
					 * @since 1.5.0.10
					 *
					 ...
					 * @param array $form Form config
					 */
					$field_value = apply_filters( 'engage_forms_magic_summary_field_value', $field_value, $field, $this->form );
		...
		if ( ! empty( $this_form[ 'variables' ] ) ) {
			foreach ( $this_form[ 'variables' ][ 'keys' ] as $var_key => $var_label ) {
				if ( $this_form[ 'variables' ][ 'types' ][ $var_key ] == 'entryitem' ) {
					$label = uefirst( str_replace( '_', ' ', $var_label ) );
					if ( $this->html ) {
	...
	protected function set_pattern(){
		 ...
		 */
		$this->pattern = apply_filters( 'engage_forms_summary_magic_pattern', '<strong>%s</strong><div style="margin-bottom:20px;">%s</div>', $this->form );
	...
	protected function should_use_label( array $field ){
		 ...
		 * @param array $form Form config
		 */
		return wp_validate_boolean( apply_filters( 'engage_forms_magic_summary_should_use_label', false, $field, $this->form ) );
	}
