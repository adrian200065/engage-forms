...
/**
 * Class Engage_Forms_Email_Resend
 */
class Engage_Forms_Email_Resend {
	...
	/**
	 * Engage_Forms_Email_Resend constructor.
	 *
	 * @since 1.5.2
	 *
	...
	public function resend(){
		 ...
		 *
		 */
		do_action( 'engage_forms_pre_email_resend', $this->form, $this->entry_id, $this->get_data() );
        $this->remove_magic_hooks();
        $this->form['mailer'][ 'recipients' ] = Engage_Forms::do_magic_tags($this->form['mailer'][ 'recipients' ], $this->entry_id, $this->data );
        $this->add_magic_hooks();
		Engage_Forms_Save_Final::do_mailer( $this->form, $this->entry_id, $this->get_data() );
        $this->remove_magic_hooks();
    }
	...
	protected function get_data(){
		if ( empty( $this->data ) ) {
			$this->data = Engage_Forms::get_submission_data( $this->form, $this->entry_id );
			foreach ( $this->data as $id => $datum ){
				$this->data[ $id ] = Engage_Forms_Magic_Doer::maybe_implode_opts( $datum );
			}
		}
	 ...
	 *
	 * @uses "engage_forms_magic_form" filter
	 *
	 * @since 1.5.2
	 *
	 * @param array $form
	 ...
	 *
	 * @uses "engage_forms_magic_parser_data" filter
	 *
	 * @since 1.5.2
	 *
	 * @param array $data
	...
	protected function apply_conditional_recipients(){
		...
		Engage_Forms_Processor_Load::get_instance()->get_processors();
			...
			foreach ( $this->form[ 'processors' ] as $processor_id => $processor ) {
				if ( 'conditional_recipient' === $processor[ 'type' ] ) {
					if ( isset( $processor[ 'conditions' ] ) && ! empty( $processor[ 'conditions' ][ 'type' ] ) ) {
						if ( ! Engage_Forms::check_condition( $processor[ 'conditions' ], $this->form, $this->entry_id ) ) {
							continue;
						}
					}
		...
		if ( ! empty( $conditional_recipient_processors ) ) {
			foreach ( $conditional_recipient_processors as $processor ) {
				Engage_Forms_Processor_Conditional_Recipient::post_processor( $processor[ 'config' ] );
			}
    ...
    {
        add_filter('engage_forms_magic_form', array($this, 'provide_form'), 10, 2);
        add_action('engage_forms_magic_parser_data', array($this, 'provide_data'), 10, 2);
    }
    ...
    {
        remove_filter('engage_forms_magic_form', array($this, 'provide_form'), 10);
        remove_filter('engage_forms_magic_parser_data', array($this, 'provide_data'), 10);
    }
