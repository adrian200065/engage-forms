...
/**
 * Handles settings for Engage Forms emails
 *
 ...
 */
class Engage_Forms_Email_Settings {
	 ...
	 * @var string
	 */
	protected static $option_key = '_engage_forms_email_api_settings';
	 ...
	 * @var string
	 */
	protected static $nonce_action = 'ef-emails';
	 ...
	 * Save email settings
	 * 
	 * @uses "wp_ajax_ef_email_save" action
	 * 
	 * @since 1.3.5
	 */
	...
	public static function maybe_add_hooks() {
		...
		if( 'wp' !== self::get_method() ){
			foreach ( self::allowed_apis() as $api ) {
				if ( self::valid( $api ) ) {
					add_filter( 'engage_forms_mailer', array( 'Engage_Forms_Email_Callbacks', $api ), 26, 3 );
					break;
				}
	...
	public static function nonce_field(){
		return wp_nonce_field( self::$nonce_action, 'efemail', false, false );
	}
	...
	protected static function allowed_apis() {
		 ...
		 */
		return apply_filters( 'engage_forms_allowed_email_apis', array(
			'wp',
			'sendgrid',
			'engage'
		) );
	 ...
	 *
	 * @uses "engage_forms_admin_footer" action
	 *
	 * @since 1.4.0
	 */
	public static function ui() {
		if (  ! version_compare( PHP_VERSION, '5.4.0', '>=' ) ) {
            echo'<div id="ef-email-settings-ui" aria-hidden="true" style="visibility: hidden;">' . esc_html__( 'Switching email services requires PHP 5.4 or later. PHP 5.6 is strongly recommended.', 'engage-forms' ) .'</div>';
		}else{
			include EFCORE_PATH . '/ui/emails/settings.php';
	 ...
	 * @since 1.4.0
	 *
	 * @uses "pre_update_option__engage_forms_email_api_settings"
	 *
