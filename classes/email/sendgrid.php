<?php
/**
 * API Client for sending Engage Forms emails via SendGrid
 *
...
class Engage_Forms_Email_SendGrid extends Engage_Forms_Email_Client{
	...
	public function set_api( array $keys ){
		...
		$options = apply_filters( 'engage_forms_sendgrid_options', $options  );
	...
	public function send(){
		...
		if( ! empty( $this->attachments ) ){
			/* @var Engage_Forms_Email_Attachment $ef_attachment_obj */
			foreach ( $this->attachments as $ef_attachment_obj ) {
				$attachment = new Attachment();
				$attachment->setContent( $ef_attachment_obj->get_encoded() );
				$attachment->setType( $ef_attachment_obj->type );
				$attachment->setFilename( $ef_attachment_obj->filename );
				$attachment->setDisposition( 'attachment' );
				$attachment->setContentId( md5( $ef_attachment_obj->filename ) );
		 ...
		 */
		$mail = apply_filters( 'engage_forms_sendgrid_before', $mail, $this->message );

		//@TODO Engage Exception
		try {
