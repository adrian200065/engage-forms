...
class Engage_Forms_Tracking {
	 ...
	 * @var string
	 */
	protected static $api_url = 'https://asimov.engage.space/ef-tracking/v3';
	 ...
	 * @var string
	 */
	protected static $row_tracking_key = '_engage_forms_tracking_last_row';
	...
	protected function __construct(){
		$enabled = false;
		if ( $enabled ) {
			add_action( 'init', array( 'Engage_Forms_DB_Track', 'get_instance' ), 1 );
		}

		add_action( 'engage_forms_tracking_send_rows', array( __CLASS__, 'send_rows' ) );
	}

	/**
	 * Get class instance
	 *
	 * @return \Engage_Forms_Tracking
	 */
	...
	public static function tracking_optin_status(){
		return get_option( '_engage_forms_tracking_allowed', 0 );
	}
	...
	public static function send_rows( ){
		...
		include_once EFCORE_PATH . 'classes/db/track.php';
		$highest = Engage_Forms_DB_Track::get_instance()->highest_id();
		...
		$body = array(
			'rows' => $rows,
			'plugins' => Engage_Forms_Support::get_plugins()
		);
		...
		if(  $partial ){
			wp_schedule_single_event( time() + 59,  'engage_forms_tracking_send_rows' );
		}
	...
	protected static function site_data(){
		...
		$data = array( 'wp' => urlencode( $wp_version ), 'php' => urlencode( PHP_VERSION ), 'db' => get_option( 'EF_DB', 0 ), 'url' => urlencode( site_url() ), 'tracking_optin' => $tracking_optin  );
	...
	protected static function prepare_rows_to_send( $last_row, $highest ) {
		...
		$rows = Engage_Forms_DB_Track::get_instance()->get_record( $ids );
		if ( ! empty( $rows ) ) {
			$fields    = Engage_Forms_DB_Track::get_instance()->get_fields();
