...
class Engage_Forms_Support {
	...
	protected function __construct(){
		$this->plugin_slug = Engage_Forms::PLUGIN_SLUG;
	...
	public static function add_menu_page(){
		add_submenu_page(
			'engage-forms',
			__( 'Support', 'engage-forms' ),
			'<span class="engage-forms-menu-dashicon"><span class="dashicons dashicons-sos"></span></span>' . __( 'Support', 'engage-forms' ) . '</span>',
	...
	public function scripts(  ){
		if( Engage_Forms_Admin::is_page( 'engage-form-support' ) ){
			Engage_Forms_Admin_Assets::enqueue_style( 'admin' );
			Engage_Forms_Admin_Assets::enqueue_script( 'support-page' );
		}
	}
	...
	public static function page(){
		include EFCORE_PATH . 'ui/support/page.php';
	}
				...
				if ( is_array( $version ) ) {
					...
					foreach ( $version as $what_v => $v ) {
						if ( ! is_array( $v ) ) {
							...
							if( $v[ 'active' ] ){
								$active = __( 'Active', 'engage-forms' );
							}else{
								$active = __( 'Not Active', 'engage-forms' );
							}
	...
	public static function short_debug_info( $html = true ){
		...
		$data = array(
			...
			'Engage Forms Version' => EFCORE_VER,
			'WP_DEBUG'              => WP_DEBUG
		);
		if( $html ){
			$html = '';
