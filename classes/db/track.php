 ...
 */
class Engage_Forms_DB_Track extends Engage_Forms_DB_Base {
	 ...
	 * @var string
	 */
	protected $table_name = 'ef_tracking';
	...
	protected function __construct(){
		add_action( 'engage_forms_submit_start',  array( $this, 'submit_start' ), 50, 2 );
		add_action( 'engage_forms_submit_complete',  array( $this, 'submit_complete' ), 50, 3 );
		add_action( 'engage_forms_submit_complete',  array( $this, 'email_tracking' ), 51, 3 );
		add_action( 'engage_forms_mailer_complete', array( $this, 'email_sent'), 50, 3 );
		add_action( 'engage_forms_mailer_failed', array( $this, 'email_fail' ), 50, 4 );
		add_action( 'engage_forms_mailer_invalid', array( $this, 'email_invalid' ), 50 );
	 ...
	 * @since 1.3.5
	 *
	 * @uses "engage_forms_submit_start"
	 *
	...
	public function submit_complete( $form, $referrer, $process_id ){
		if( isset( $form[ 'ID' ] ) ){
			$recorded = $this->create( array(
				...
				'process_id' => $process_id,
				'time' => current_time( 'mysql' ),
				'referrer' => ef_http_build_url( '', $referrer )
			));
	 ...
	 *
	 * @uses "engage_forms_mailer_complete"
	 *
	 * @since 1.3.5
	 *
	 ...
	 *
	 * @uses "engage_forms_mailer_failed"
	 *
	 * @since 1.3.5
	 *
	 * @param array $mail Mailer data
