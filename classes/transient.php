<?php
use engagewp\engageforms\ef2\Transients\TransientApiContract;
/**
 * Class Engage_Forms_Transient
 *
 ...
 * In 1.5.6 This API switched to using get/update_option() with WordPress "CRON job" to delete if needed, which it should not be most of the time, transient should be deleted on engage_forms_submit_complete aaction. Reasons explained in https://github.com/EngageWP/Engage-Forms/issues/1866
 * In 1.8.0 Created engagewp\engageforms\ef2\Transients\TransientApiContract as public API definition.
 */
class Engage_Forms_Transient  {

	/**
	 * Tracks the transients to be deleted at engage_forms_submit_complete action
	 *
	 * @since 1.5.6
	 *
	 * @var array
	 ...
	 */
	const CRON_ACTION = 'engage_forms_transient_delete';
	...
	public static function set_transient( $id, $data, $expires = null ){
		...
		engage_forms_get_v2_container()
			...
			->schedule( new \engagewp\engageforms\ef2\Jobs\DeleteTransientJob($id), $expires );
	 ...
	 * @since 1.5.5
	 *
	 * @uses "engage_forms_submit_complete"
	 */
	...
	public static function get_all(){
		global $wpdb;
		$like = '%' . $wpdb->esc_like( 'eftransdata' ) . '%';
