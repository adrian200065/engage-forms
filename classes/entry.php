...
class Engage_Forms_Entry {
	...
	/**
	 * Engage_Forms_Entry constructor.
	 *
	 * @since 1.4.0
	 *
	 ...
	 * @param Engage_Forms_Entry_Entry $entry Optional. Entry object.
	 */
	public function __construct( array $form, $entry_id = false, Engage_Forms_Entry_Entry $entry = null ) {
		$this->form = $form;
		if( null !== $entry ){
			if( null != $entry->id ){
     ...
     * @deprecated 1.5.8
	 *
	 * @param \Engage_Forms_Entry_Entry $entry
	 */
	public function get_entry_object( Engage_Forms_Entry_Entry $entry ){
	    _deprecated_function( __FUNCTION__, '1.5.8', 'Engage_Forms_Entry::set_entry_object' );
     ...
     * @return $this
     */
    public function set_entry_object( Engage_Forms_Entry_Entry $entry ){
        $this->entry = $entry;
        return $this;
    }
	 ...
	 * @since 1.4.0
	 *
	 * @return \Engage_Forms_Entry_Entry
	 */
	public function get_entry(){
        ...
        if( ! $this->entry ){
	        $this->entry = new Engage_Forms_Entry_Entry;
        }
		return $this->entry;
	}
	 ...
	 *
	 * @return Engage_Forms_Entry_Field|null
	 */
	...
	protected function find_entry(){
		$wpdb = $this->get_wpdb();
        if ( $wpdb ) {
            $table = $wpdb->prefix . 'ef_form_entries';
	...
	protected function find_fields(){
		$wpdb = $this->get_wpdb();
        if ( $wpdb ) {
            $table = $wpdb->prefix . 'ef_form_entry_values';
            $sql = $wpdb->prepare("SELECT * FROM $table WHERE `entry_id` = %d", $this->entry_id);
            $results = $wpdb->get_results($sql);
            if (!empty($results)) {
                foreach ($results as $result) {
                    $_field = new Engage_Forms_Entry_Field($result);
                    $this->fields[] = $_field;
                }
            }
        }
	...
	protected function find_metas(){
		$wpdb = $this->get_wpdb();
        if ( $wpdb ) {
            $table = $wpdb->prefix . 'ef_form_entry_meta';
            $sql = $wpdb->prepare("SELECT * FROM $table WHERE `entry_id` = %d", $this->entry_id);
            $results = $wpdb->get_results($sql);
            if (!empty($results)) {
                foreach ($results as $result) {
                    $_meta = new Engage_Forms_Entry_Meta();
		...
		if( is_numeric( $this->entry_id   ) ){
			if ( ! empty( $this->fields ) ) {
				foreach ( $this->fields as $i =>  $field ) {
					if ( $field instanceof  Engage_Forms_Entry_Field ) {
			...
			if ( ! empty( $this->meta ) ) {
				foreach ( $this->meta as $i => $meta ) {
					if ( $meta instanceof Engage_Forms_Entry_Meta ) {
	...
	public function update_status( $status ){
		...
		$wpdb = $this->get_wpdb();
        if ( $wpdb ) {
            $wpdb->update($wpdb->prefix . 'ef_form_entries', array('status' => $status), array('id' => $this->entry_id));
        }
	}
	 ...
	 */
	public function add_field( Engage_Forms_Entry_Field $field ){
	 ...
	 * @since 1.4.0
	 *
	 * @param \Engage_Forms_Entry_Meta $item
	 */
	public function add_meta( Engage_Forms_Entry_Meta $item ){
		$this->meta[] = $item;
	}
	...
	protected function save_entry() {
        ...
        if ( $wpdb ) {
            if (!$this->entry_id) {
                $wpdb->insert($wpdb->prefix . 'ef_form_entries', $this->entry->to_array());
                $this->entry_id = $this->entry->id = $wpdb->insert_id;
            } else {
                $wpdb->update($wpdb->prefix . 'ef_form_entries', $this->entry->to_array(), array(
                    'id' => $this->entry_id
                ));
            }
        }
	 ...
	 * @param \Engage_Forms_Entry_Field $field
	 *
	 * @return  Engage_Forms_Entry_Field
	 */
	protected function save_field( Engage_Forms_Entry_Field $field ){
		$field->entry_id = $this->entry_id;
		$wpdb = $this->get_wpdb();
		if( $wpdb ) {
            $data = $field->to_array();
            if (!isset($data['id'])) {
                ...
                Engage_Forms_Entry_Update::update_field($field);
            }
	 ...
	 * @since 1.4.0
	 *
	 * @param \Engage_Forms_Entry_Meta $meta
	 *
	 * @return Engage_Forms_Entry_Meta
	 */
	protected function save_meta( Engage_Forms_Entry_Meta $meta ){
		$meta->entry_id = $this->entry_id;
		$wpdb = $this->get_wpdb();
        if( $wpdb ) {
            $data = $meta->to_array();
            unset( $data[ 'id' ] );
            $wpdb->insert( $wpdb->prefix . 'ef_form_entry_meta',  $data );
