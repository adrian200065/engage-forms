...
class Engage_Forms_Save_Final {
	...
	public static function save_in_db( $form, $entryid = null ) {
		...
		if(!empty($form['db_support'])){
			...
			if(empty($entryid)){
				...
				$entryid = Engage_Forms::get_field_data( '_entry_id', $form );

				foreach($form['fields'] as $field_id=>$field){
					// add new and update
					Engage_Forms::save_field_data($field, $entryid, $form);
									...
									if(is_array($check_value)){
										foreach($check_value as &$sub_check_value){
											if(!is_array($sub_check_value)){
												$sub_check_value = Engage_Forms::do_magic_tags( $sub_check_value );
											}
										}
									}else{
										$check_value = Engage_Forms::do_magic_tags( $check_value );
									}
								}
							}else{
								$meta_value = Engage_Forms::do_magic_tags( $meta_value );
							}
				...
				// update status
				$wpdb->update($wpdb->prefix . 'ef_form_entries', array('status' => 'active'), array( 'id' => $entryid ) );
				...
				foreach($form['fields'] as $field_id=>$field){
					// add new and update
					Engage_Forms::update_field_data($field, $entryid, $form);
	...
	public static function do_mailer( $form, $entryid = null, $data = null, array $settings = null ) {
		...
		if ( ! $data ) {
			$data = Engage_Forms::get_submission_data($form, $entryid );
		}

		//fix for https://github.com/EngageWP/Engage-Forms/issues/888
		//Josh - Please find time to redo all CSV rendering please. Your buddy, Josh
		if( ! empty( $data ) ){
			foreach ( $data as $id => $datum ){
				$data[ $id ] = Engage_Forms_Magic_Doer::maybe_implode_opts( $datum );
			}
		...
		// do mailer!
		if ( empty( $settings ) ) {
			$sendername = __( 'Engage Forms Notification', 'engage-forms' );
			if ( ! empty( $form['mailer']['sender_name'] ) ) {
				$sendername = Engage_Forms::do_magic_tags( $form['mailer']['sender_name'] );
				if ( false !== strpos( $sendername, '%' ) ) {
					$isname = Engage_Forms::get_slug_data( trim( $sendername, '%' ), $form );
					if ( ! empty( $isname ) ) {
						$sendername = $isname;
					}
				}
			...
			if ( empty( $form['mailer']['sender_email'] ) ) {
				$sendermail = Engage_Forms_Email_Fallback::get_fallback( $form );
			} else {
				$sendermail = Engage_Forms::do_magic_tags( $form['mailer']['sender_email'] );
				if ( false !== strpos( $sendermail, '%' ) ) {
					$ismail = Engage_Forms::get_slug_data( trim( $sendermail, '%' ), $form );
					if ( is_email( $ismail ) ) {
						$sendermail = $ismail;
					}
				}
			...
			$mail              = array(
				'recipients'  => array(),
				'subject'     => Engage_Forms::do_magic_tags( $form['mailer']['email_subject'] ),
				'message'     => stripslashes( $form['mailer']['email_message'] ) . "\r\n",
				'headers'     => array(
					Engage_Forms::do_magic_tags( 'From: ' . $sendername . ' <' . $sendermail . '>' )
				),
				'attachments' => array()
			);
			...
			if ( isset( $form['mailer']['bcc_to'] ) && ! empty( $form['mailer']['bcc_to'] ) ) {
				...
				$bcc_array = array_map('trim', preg_split( '/[;,]/', Engage_Forms::do_magic_tags( $form['mailer']['bcc_to'] ) ) );
			...
			if ( isset( $form['mailer']['reply_to'] ) ) {
                                ...
                                $mail['headers'][] = Engage_Forms::do_magic_tags( 'Reply-To: ' . $form['mailer']['reply_to'] );
			}
			if ( ! $mail['replyto'] ) {
			...
			$mail['message'] = Engage_Forms::do_magic_tags( $mail['message'], $entryid, $data );
			...
			if ( ! empty( $hastags[1] ) ) {
				foreach ( $hastags[1] as $tag_key => $tag ) {
					...
					if( ! preg_match('/\s/', $tag) ){
						$tagval = Engage_Forms::get_slug_data( $tag, $form );
						if ( is_array( $tagval ) ) {
			...
			if ( ! empty( $hasifs[1] ) ) {
				...
				foreach ( $hasifs[0] as $if_key => $if_tag ) {
					...
					foreach ( $vars as $varkey => $varval ) {
						if ( is_string( $varkey ) ) {
							$var = Engage_Forms::get_slug_data( $varkey, $form );
							...
							$var = Engage_Forms::get_slug_data( $varval, $form );
							if ( ! empty( $var ) ) {
								// show code
								$mail['message'] = str_replace( $hasifs[0][ $if_key ], $content[0], $mail['message'] );
							} else {
			...
			if ( ! empty( $form['mailer']['recipients'] ) ) {
				$recipients_array = array_map('trim', preg_split( '/[;,]/', Engage_Forms::do_magic_tags( $form['mailer']['recipients'] ) ) );
				...
				$mail['recipients'][] = Engage_Forms_Email_Fallback::get_fallback( $form );
			}
			...
			foreach ( $data as $field_id => $row ) {
				...
				if( Engage_Forms_Field_Util::is_file_field( $field_id, $form ) && Engage_Forms_Files::is_private( Engage_Forms_Field_Util::get_field( $field_id, $form ) ) ){
					continue;
				}
				...
				$tag =  '%' . $key . '%';
				$parsed = Engage_Forms_Magic_Doer::do_field_magic( $tag, $entryid, $form );
			...
			// final magic
			$mail['message'] = Engage_Forms::do_magic_tags( $mail['message'], $entryid, $form );
			$mail['subject'] = Engage_Forms::do_magic_tags( $mail['subject'], $entryid, $form );

			// CSV
			$mail['csv'] = $csvfile = false;
			if ( ! empty( $form['mailer']['csv_data'] ) ) {
				/**
				 * Modify label or values for CSV attatched to Engage Forms email
				 *
				 * @since 1.5.3
				 *
				 ...
				 * @param array $form Form config
				 */
				$csv_data = apply_filters( 'engage_forms_email_csv_data', $csv_data, $form );
		...
		if( empty( $mail ) ){
			 ...
			 * @param array|null $data Optional. Data to use for sending. If not provided, will be retrieved by form ID.
			 */
			do_action( 'engage_forms_mailer_invalid', $form, $entryid, $data );
			return;
		 ...
		 * @since unknown in original location (Engage_Forms::save_final_form)
		 *
		 ...
		 */
		$mail = apply_filters( 'engage_forms_mailer', $mail, $data, $form, $entryid );
		 ...
		 * @param array $form The form config
		 */
		do_action( 'engage_forms_do_mailer', $mail, $data, $form);
		...
		if( ! empty( $mail ) ){
			...
			if( !empty( $form['debug_mailer'] ) ){
				add_action( 'phpmailer_init', array( 'Engage_Forms', 'debug_mail_send' ), 1000 );
			}
	...
	public static function after_send_email( $form, $data, $sent, $csvfile, $mail, $method ) {
		if ( $sent ) {
			 ...
			 * @param string $method Method for sending email
			 */
			do_action( 'engage_forms_mailer_complete', $mail, $data, $form, $method );
		} else {
			/**
			 ...
			 * @param string $method Method for sending email
			 */
			do_action( 'engage_forms_mailer_failed', $mail, $data, $form, $method );
		...
		if( isset( $form[ 'fields' ] ) ){
			...
			foreach( $fields as $field_id => $value ){
				if( array_key_exists($field_id, $form[ 'fields' ] ) ){
					$field = new Engage_Forms_Entry_Field();
					$field->value = $value;
