...
class Engage_Forms_Forms
{
    ...
    public static function get_forms($with_details = false, $internal_only = false, $orderby = false)
    {
        ...
        if (false === $internal_only) {
             ...
             *
             */
            $forms = apply_filters('engage_forms_get_forms', static::$index);
    ...
    {
        return Engage_Forms_DB_Form::get_instance()->make_revision_primary($revision_id);
    }
    ...
    protected static function get_stored_forms()
    {
        if (empty(self::$stored_forms)) {
            $forms = Engage_Forms_DB_Form::get_instance()->get_all();
            if (!empty($forms)) {
    ...
    public static function import_form($data, $trusted = true)
    {
        ...
        $importer = new Engage_Forms_Import_Form($data, $trusted);
    ...
    /**
     * Apply engage_forms_get_form filters
     *
    ...
    protected static function filter_form($id_name, $config, $is_revision = false)
    {
         ...
         *
         */
        $config = apply_filters('engage_forms_get_form', $config, $id_name, $is_revision);
         ...
         *
         */
        $config = apply_filters('engage_forms_get_form-' . $id_name, $config, $id_name, $is_revision);
    ...
    protected static function max_revisions(array $form)
    {
         ...
         *
         */
        $max_revisions = apply_filters('engage_forms_max_form_revisions', 15, $form);
    ...
    protected static function add_details($forms)
    {
        ...
        //See: https://github.com/EngageWP/Engage-Forms/pull/3354
        $valid_forms = [];
    ...
    public static function save_form($data, $type = 'primary')
    {
            ...
            foreach ($data['fields'] as &$field) {
                ...
                if (!empty($field['config']['option']) && is_array($field['config']['option'])) {
                    ...
                    if (!empty($field["config"]["manual_formula"]) && is_string($field["config"]["manual_formula"])) {
                        $field["config"]["manual_formula"] = Engage_Forms_Sanitize::finish_trim(Engage_Forms_Sanitize::sanitize_header($field["config"]["manual_formula"]));
                    }
                }
            }
        }
         ...
         *
         */
        $data = apply_filters('engage_forms_presave_form', $data);
         ...
         *
         */
        do_action('engage_forms_save_form', $data, $data['ID']);
    ...
    {
        require_once(EFCORE_PATH . 'classes/admin.php');

        // get form templates (PROBABLY NEED TO MOVE METHOD INTO THIS CLASS)
        $form_templates = Engage_Forms_Admin::internal_form_templates();
         ...
         *
         */
        $newform = apply_filters('engage_forms_create_form', $newform);
        ...
        // Fixes https://github.com/EngageWP/Engage-Forms/issues/3455
        self::clear_cache();
         ...
         *
         */
        do_action('engage_forms_create_form', $newform);
        return $newform;
    }
    ...
    public static function delete_form($id)
    {
        ...
        unset($forms[$id]);
        $deleted = Engage_Forms_DB_Form::get_instance()->delete_by_form_id($id);
        if ($deleted) {
    ...
    protected static function update_registry($new)
    {
         ...
         *
         */
        do_action('engage_forms_save_form_register', array(), $forms);
    ...
    public static function get_fields(array $form, $in_order = true)
    {
        ...
        if ($in_order) {
             ...
             *
             */
            $order = apply_filters('engage_forms_get_field_order', $order, $form);
    ...
    public static function personally_identifying_fields(array $form, $ids_only = false)
    {
        ...
        if (!empty($fields)) {
            foreach ($fields as $field_id => $field) {
                if (Engage_Forms_Field_Util::is_personally_identifying($field, $form)) {
    ...
    public static function email_identifying_fields(array $form, $ids_only = false)
    {
        ...
        if (!empty($fields)) {
            foreach ($fields as $field_id => $field) {
                if (Engage_Forms_Field_Util::is_email_identifying_field($field, $form)) {
     ...
     *
     * Note, does not save. Use Engage_Forms_Forms::save_form( Engage_Forms_Forms::update_privacy_export_enabled( $form, true ) ) );
     *
     * @param array $form Form config
    ...
    {
        $forms = Engage_Forms_DB_Form::get_instance()->get_by_form_id($form_id, false);
        $revisions = array();
        if (!empty($forms)) {
    ...
    {
        $form = Engage_Forms_DB_Form::get_instance()->get_record($id);
        $form = self::db_to_return($form);
    ...
    /**
     * If necessary migrate to database table added in Engage Forms 1.5.3
     *
     * @param array $form Form config
     *
     * @return array|bool
    ...
    {
        $in_db = Engage_Forms_DB_Form::get_instance()->get_by_form_id($form['ID']);
        if (empty($in_db)) {
    ...
    protected static function save_to_db(array $form, $type = 'primary')
    {
        ...
        if (isset($form['db_id'])) {
            ...
            $db_id = Engage_Forms_DB_Form::get_instance()->update($data);
            self::maybe_delete_old_revisions($form);
        } else {
            $db_id = Engage_Forms_DB_Form::get_instance()->create($data);
    ...
    protected static function get_from_db($form_id, $primary_only = true)
    {
        if (false === $primary_only) {
            return Engage_Forms_DB_Form::get_instance()
        ...
        if( ! self::form_cache_has($form_id) ){
            self::form_cache_add(
                $form_id,
                Engage_Forms_DB_Form::get_instance()
                    ->get_by_form_id(
                        $form_id,
                        $primary_only
                    )
    ...
    protected static function maybe_delete_old_revisions(array $form)
    {
        ...
        if (!empty($revisions)) {
            ...
            if ($max_revisions < count($revisions)) {
                ...
                Engage_Forms_DB_Form::get_instance()->delete($deletes);
