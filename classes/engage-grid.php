 ...
 * 2014 - David Cramer
 */
if( !class_exists( 'Engage_Form_Grid' )){
	
	class Engage_Form_Grid {
		...
		public function renderLayout($grid = false) {
			...
			foreach($grid as $row=>$cols){
				...
				/**
				 * Alter row ID attribute in Engage Grid
				 *
				 * @since 1.4.9
				 *
				 ...
				 */
				$rowID = apply_filters( 'engage_forms_grid_row_id', $rowID, $row, $this->config );
				...
				/**
				 * Alter row class attribute in Engage Grid
				 *
				 ...
				 * @param array $config Grid config. Contains form ID in form_id key.
				 */
				$rowClass = apply_filters( 'engage_forms_grid_row_class', $rowClass, $row, $this->config );
