 ...
 */
class Engage_Forms_Render_Assets
{
	...
	/**
	 * Tracks if EF_API_DATA has been outputted or not
	 *
	 * @since 1.5.7
	 *
	 * @var array
	...
	{
		if (0 === strpos($script, 'ef-')) {
			$slug = $script;
		} elseif (wp_script_is('ef-' . $script, 'registered')) {
			$slug = 'ef-' . $script;
		} elseif (wp_script_is('ef-' . $script . '-scripts', 'registered')) {
			$slug = 'ef-' . $script . '-scripts';
		} elseif (wp_style_is('ef-' . $script, 'registered')) {
			$slug = 'ef-' . $script;
		} elseif (wp_style_is('ef-' . $script . '-styles', 'registered')) {
			$slug = 'ef-' . $script . '-styles';
		} else {
			$slug = 'ef-' . sanitize_key(basename($script));
		}
	...
	{
		if (0 === strpos($style, 'ef-')) {
			$slug = $style;
		} elseif (wp_style_is('ef-' . $style . '-styles', 'registered')) {
			$slug = 'ef-' . $style . '-styles';
		} elseif (wp_style_is('ef-' . $style, 'registered')) {
			$slug = 'ef-' . $style;
		} else {
			$slug = 'ef-' . sanitize_key(basename($style));
		}
	...
	public static function get_style_includes()
	{
		 ...
		 * @since unknown
		 *
		 * @param array $style_includes To include or not. Default is value of option "_engage_forms_styleincludes"
		 */
		$style_includes = apply_filters('engage_forms_get_style_includes', $style_includes);
	...
	public static function get_core_styles()
	{
		$style_urls = [
			...
			'grid' => self::make_url('engage-grid', false),
			'form' => self::make_url('engage-form', false),
			'alert' => self::make_url('engage-alert', false),
			'field' => self::make_url('fields', false),
			'front' => self::make_url('engage-forms-front', false),
			'font' => self::make_url('efont', false),
			'table' => self::make_url('engage-table', false),
	...
	public static function get_core_scripts()
	{
		$script_urls = [
			...
			'handlebars' => EFCORE_URL . 'assets/js/handlebars.js',
			...
			'form-front' => self::make_url('engage-forms-front'),
	...
	public static function register()
	{
		...
		if (self::should_minify(true)) {
			...
			wp_register_script($slug, self::make_url('vue'), ['jquery'], EFCORE_VER, true);
		}

		$script_style_urls = [];

		// check to see language and include the language add on
		Engage_Forms_Render_Assets::maybe_validator_i18n(true);


		/**
		 * Filter script URLS for Engage Forms on the frontend, before they are enqueued.
		 *
		 * @since 1.3.1
		 *
		 * @param array $script_urls array containing all urls to register
		 */
		$script_style_urls[ 'script' ] = apply_filters('engage_forms_script_urls', $script_urls);

		/**
		 * Filter style URLS for Engage Forms on the frontend, before they are enqueued.
		 *
		 * @since 1.3.1
		 *
		 * @param array $script_urls array containing all urls to register
		 */
		$script_style_urls[ 'style' ] = apply_filters('engage_forms_style_urls', $style_urls);
		...
		foreach ($script_style_urls[ 'style' ] as $style_key => $style_url) {
			...
			wp_register_style($slug, $style_url, [], EFCORE_VER);
		...
		foreach ($script_style_urls[ 'script' ] as $script_key => $script_url) {
			...
			if ('validator' == $script_key) {
				wp_register_script(self::make_slug($script_key), $script_url, ['jquery'], EFCORE_VER, false);
				continue;
		...
		foreach (self::get_field_styles() as $style) {
			...
			self::$registered[ 'styles' ][] = $slug;
			wp_register_style($slug, $style, [], EFCORE_VER);
		 ...
		 */
		do_action('engage_forms_assets_registered', $script_style_urls);


	}

	/**
	 * Enqueue a style for Engage Forms front-end
	 *
	 * @since 1.5.0
	 *
	...
	public static function enqueue_style($style)
	{
		...
		if (!wp_style_is($style, 'enqueued')) {
			if (false !== strpos($style, '//')) {
				...
				wp_enqueue_style($slug, $style, [], EFCORE_VER);
	...
	/**
	 * Enqueue a script for Engage Forms front-end
	 *
	 * @since 1.5.0
	 *
	...
	public static function enqueue_script($script, $depts = ['jquery'])
	{
		...
		if ('editor-grid' === $script) {
			return Engage_Forms_Admin_Assets::enqueue_script($script);
		}


		if (in_array($script, ['validator', self::make_slug('validator')])) {
			$scripts = self::get_core_scripts();
			wp_enqueue_script(self::make_slug('validator'), $scripts[ 'validator' ], [], EFCORE_VER, false);
		...
		if (!wp_script_is($slug, 'enqueued') && wp_script_is($slug, 'registered')) {
			...
			if (!empty($depts) && is_array($depts) && !filter_var($script, FILTER_VALIDATE_URL)) {
				...
				wp_enqueue_script($slug, $script, $depts, EFCORE_VER, false);
			}
		...
		if (Engage_Forms::settings()->get_cdn()->combine()) {
			Engage_Forms_CDN_Init::get_cdn()->add_to_combiner($slug);
		}
	...
	/**
	 * Create a URL for a script/style used by Engage Forms
	 *
	 * @since 1.5.0
	 *
	...
	public static function make_url($name, $script = true)
	{
		...
		//@SEE https://github.com/EngageWP/Engage-Forms/issues/2487#issuecomment-388445315
		if ('edit' === $name) {
			$min = false;
		}
	...
	public static function should_minify($script = true)
	{
		 ...
		 */
		return apply_filters('engage_forms_render_assets_minify', true, $script);
	}
	 ...
	 * @since 1.8.4
	 */
	public static function ef_dependencies($tag){
	...
	public static function enqueue_form_assets()
	{
		...
		self::enqueue_script('render', self::ef_dependencies('render') );
		...
		if (!is_array(self::$api_data_localized) || !isset(self::$api_data_localized[ $field_script_to_localize ])) {
			wp_localize_script($field_script_to_localize, 'EF_API_DATA', [
				'rest' => [
					'root' => esc_url_raw(Engage_Forms_API_Util::url()),
					'rootV3' => esc_url_raw(Engage_Forms_API_Util::url('', false, 'v3')),
					'fileUpload' => esc_url_raw(Engage_Forms_API_Util::url('file', false, 'v3')),
					'tokens' => [
						'nonce' => esc_url_raw(Engage_Forms_API_Util::url('tokens/form')),
					],
					'nonce' => wp_create_nonce('wp_rest'),
				],
				'strings' => [
					'ef2FileField' => [
						'removeFile' => esc_attr__('Remove file', 'engage-forms'),
						'defaultButtonText' => esc_attr__('Drop files or click to select files to Upload',
							'engage-forms'),
						'fileUploadError1' => esc_attr__('Error: ', 'engage-forms'),
						'fileUploadError2' => esc_attr__(' could not be processed', 'engage-forms'),
						'invalidFiles' => esc_attr__('These Files have been rejected : ', 'engage-forms'),
						'checkMessage' => esc_attr__('Please check files type and size', 'engage-forms'),
						'invalidFileResponse' => esc_attr__('Unknown File Process Error', 'engage-forms'),
						'fieldIsRequired' => esc_attr__('Field is required', 'engage-forms'),
						'filesUnit' => esc_attr__('bytes', 'engage-forms'),
						'maxSizeAlert' => esc_attr__('This file is too large. Maximum size is ', 'engage-forms'),
						'wrongTypeAlert' => esc_attr__('This file type is not allowed. Allowed types are ',
							'engage-forms'),
					],
				],
				'nonce' => [
					'field' => Engage_Forms_Render_Nonce::nonce_field_name(),
				],
			]);
	...
	{
		if (!did_action('engage_forms_assets_registered')) {
			self::register();
		}
	}
	...
	protected static function register_script($script_key, $script_url, $depts = ['jquery'])
	{
		...
		if ('field' == $script_key) {
			...
			add_filter('engage_forms_render_assets_minify', '__return_false', 51);
			$script_url = self::make_url(str_replace('-', '/', $script_key));
			self::$registered[ 'scripts' ][] = $slug;
			wp_register_script($slug, $script_url, $depts, EFCORE_VER, true);
			remove_filter('engage_forms_render_assets_minify', '__return_false', 51);
			return;
		} elseif (in_array($script_key, [
			'state-events',
			'state-state',
	...
	{
		$fields = Engage_Forms_Fields::get_all();
		$scripts = [];
	...
	public static function maybe_validator_i18n($register = true)
	{
		 ...
		 */
		$print_strings = apply_filters('engage_forms_print_translation_strings_in_footer', false, $locale);
		...
		if ($register) {
			...
			if ($print_strings) {
				wp_localize_script(self::make_slug('validator'), 'EF_VALIDATOR_STRINGS', [
					'defaultMessage' => __( "This value seems to be invalid.", 'engage-forms' ),
					'type' => [
						'email' => __("This value should be a valid email.", 'engage-forms' ),
						'url' => __("This value should be a valid url.", 'engage-forms' ),
						'number' => __("This value should be a valid number.", 'engage-forms' ),
						'integer' => __("This value should be a valid integer.", 'engage-forms' ),
						'digits' => __("This value should be digits.", 'engage-forms' ),
						'alphanum' => __("This value should be alphanumeric.", 'engage-forms' ),
					],
					'notblank' => __("This value should not be blank.", 'engage-forms' ),
					'required' => __("This value is required", 'engage-forms' ),
					'pattern' => __("This value seems to be invalid.", 'engage-forms' ),
					'min' =>__( "This value should be greater than or equal to %s.", 'engage-forms' ),
					'max' => __("This value should be lower than or equal to %s.", 'engage-forms' ),
					'range' =>__( "This value should be between %s and %s.", 'engage-forms' ),
					'minlength' => __("This value is too short. It should have %s characters or more.", 'engage-forms' ),
					'maxlength' => __("This value is too long. It should have %s characters or fewer.", 'engage-forms' ),
					'length' => __("This value length is invalid. It should be between %s and %s characters long.", 'engage-forms' ),
					'mincheck' => __("You must select at least %s choices.", 'engage-forms' ),
					'maxcheck' => __("You must select %s choices or fewer.", 'engage-forms' ),
					'check' => __("You must select between %s and %s choices.", 'engage-forms' ),
					'equalto' => __("This value should be the same.", 'engage-forms' ),
				]);
				$script = "<script>window.Parsley.addMessages( '$code', EF_VALIDATOR_STRINGS ); window.Parsley.setLocale( '$locale' );</script>";
			...
			Engage_Forms_Render_Util::add_inline_data($script, ['ID' => rand()]);
	...
	public static function get_validator_locale_url($locale)
	{
		...
		$validator_url = EFCORE_URL . 'assets/js/i18n/' . $locale . '.js';
	...
	{
		if (file_exists(EFCORE_PATH . 'assets/js/i18n/' . $locale . '.js')) {
			// no need to check other possibilities- break if/else early
		} elseif (file_exists(EFCORE_PATH . 'assets/js/i18n/' . strtolower($locale) . '.js')) {
			$locale = strtolower($locale);
		} elseif (file_exists(EFCORE_PATH . 'assets/js/i18n/' . strtolower(str_replace('_', '-', $locale)) . '.js')) {
			$locale = strtolower(str_replace('_', '-', $locale));
		} elseif (file_exists(EFCORE_PATH . 'assets/js/i18n/' . strtolower(substr($locale, 0, 2)) . '.js')) {
			$locale = strtolower(substr($locale, 0, 2));
		} elseif (file_exists(EFCORE_PATH . 'assets/js/i18n/' . strtolower(substr($locale, 3)) . '.js')) {
    ...
    protected static function get_webpack_manifest()
    {
        if( ! is_array( self::$webpack_asset_manifest ) ){
            $path =  EFCORE_PATH .'/dist/asset-manifest.json';
            if (!file_exists($path)) {
                return [];
            }
	...
	public static function maybe_redirect_to_dist(){
		...
		if( $uri && 0 === strpos( $uri,'/dist/engage-hot-load' ) ){
	
			$path =  EFCORE_PATH  . $_SERVER['REQUEST_URI'];
			if( file_exists($path)){
				ef_redirect(untrailingslashit(EFCORE_URL) . $uri);exit;
			}
		}
