 ...
 */
class Engage_Forms_Render_AutoPopulation {
	...
	public function add_hooks(){
		add_filter( 'engage_forms_render_get_field', array( $this, 'easy_pods' ), 11, 2 );
		add_filter( 'engage_forms_autopopulate_post_type_args', array( $this, 'easy_queries' ), 11, 2 );
	}
	...
	public function remove_hooks(){
		remove_filter( 'engage_forms_render_get_field', array( $this, 'easy_pods' ), 11 );
		remove_filter( 'engage_forms_autopopulate_post_type_args', array( $this, 'easy_queries' ), 11 );
	}

	/**
	 * Auto-populate Easy Pods-based select fields
	 *
	 * @uses "engage_forms_render_get_field"
	 *
	 * @since 1.4.3
	 *
		...
		if ( ! empty( $field[ 'config' ][ 'auto' ] ) ) {
			if ( 'easy-pod' == $field[ 'config' ][ 'auto_type' ] && isset( $field[ 'config' ][ 'easy-pod' ] ) && function_exists( 'cep_get_easy_pod' ) && function_exists( 'pods' ) ) {
				...
				if( ! empty( $easy_pod ) ){
					...
					$pod_params = Engage_Easy_Pods::get_instance()->apply_query( array(), $pod, array('query' => $easy_pod ) );
					 ...
					 */
					$label_field = apply_filters( 'engage_forms_easy_pods_autopopulate_label_field', $label_field, $form, $pod, $easy_pod );
					if( 0 < $pod->total() ){
						while( $pod->fetch() ){
				...
				$field = Engage_Forms::format_select_options( $field );
	 ...
	 *
	 * @since "engage_forms_autopopulate_post_type_args"
	 *
	...
	public function easy_queries( $args, $field ){
		if ( ! empty( $field[ 'config' ][ 'auto' ] ) ) {
			if ( 'easy-query' == $field[ 'config' ][ 'auto_type' ] && isset( $field[ 'config' ][ 'easy-query' ] ) && defined( 'CAEQ_PATH' ) ) {
				$easy_query = \engagewp\caeq\options::get_single( $field[ 'config' ][ 'easy-query' ] );
				if ( ! empty( $easy_query ) ) {
					$args = \engagewp\caeq\core::get_instance()->build_query_args( $easy_query );
