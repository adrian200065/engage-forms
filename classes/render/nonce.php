 ...
 */
class Engage_Forms_Render_Nonce {
	 ...
	 * @var string
	 */
	protected static $action = 'engage_forms_front_';
	...
	public static function verify_nonce( $nonce, $form_id ){
		...
		if( ! $valid ){
			 ...
			 */
			do_action( 'engage_forms_verification_token_failed', $form_id );
		}
		return $valid;
	}
		...
		if ( ! $from_esi ) {
			if ( method_exists( 'LiteSpeed_Cache_API', 'esi_enabled' ) && LiteSpeed_Cache_API::esi_enabled() ) {
				if ( method_exists( 'LiteSpeed_Cache_API', 'v' ) && LiteSpeed_Cache_API::v( '1.2.4' ) ) {
					...
					return LiteSpeed_Cache_API::esi_url( 'engage_forms', 'Engage Forms', $params ) ;
				}
			}
		}
