...
class Engage_Forms
{
	 ...
	 * @var     string
	 */
	const VERSION = EFCORE_VER;

	/**
	 * @since 1.4.2
	 */
	const PLUGIN_SLUG = 'engage-forms';

	/**
	 * @var      string
	 */
	protected $plugin_slug = 'engage-forms';
	/**
	 * @var      string
	 */
	 ...
	 * @since 1.5.1
	 *
	 * @var Engage_Forms_DB_Tables
	 */
	protected static $tables;

	/**
	 * Used to track if system has initialized and prevent recursion in init_ef_internal() method.
	 *
	 * @since 1.3.5
	 *
	 * @var bool
	 ...
	 * Settings collection
	 *
	 * Access via Engage_Forms::settings()
	 *
	 * @since 1.5.3
	 *
	 * @var Engage_Forms_Settings
	 */
	...
	function __construct()
	{
		...
		add_filter('engage_forms_get_form_processors', array($this, 'get_form_processors'));
		add_filter('engage_forms_submit_redirect_complete', array($this, 'do_redirect'), 10, 4);
		add_action('engage_forms_edit_end', array($this, 'calculations_templates'));
		add_filter('engage_forms_render_get_field', array($this, 'auto_populate_options_field'), 10, 2);
		add_filter('engage_forms_render_get_field', array($this, 'apply_conditional_groups'), 10, 2);
		add_filter('engage_forms_view_field_paragraph', 'wpautop');

		//mailer magic
		add_filter('engage_forms_get_magic_tags', array($this, 'set_magic_tags'), 1);

		// action
		add_action('engage_forms_submit_complete', array($this, 'save_final_form'), 50);

		// find if profile is loaded
		add_action('wp_loaded', array($this, 'ef_init_system'), 25);
		add_action('wp', array($this, 'ef_init_preview'));

		// render shortcode
		add_shortcode('engage-form', array($this, 'shortcode_handler'));
		// modal shortcode
		add_shortcode('engage-form_modal', array($this, 'shortcode_handler'));
		add_action('wp_footer', array('Engage_Forms_Render_Modals', 'render_footer_modals'));

		//filter shortcode atts for defaults
		add_filter('shortcode_atts_engage-form', array('Engage_Forms_Shortcode_Atts', 'allow_default_set'), 5, 4);
		add_filter('shortcode_atts_engage-form_modal', array('Engage_Forms_Shortcode_Atts', 'allow_default_set'), 5,
			4);
		add_filter('shortcode_atts_engage-form', array('Engage_Forms_Shortcode_Atts', 'maybe_allow_revision'), 5, 4);
		add_filter('shortcode_atts_engage-form_modal', array('Engage_Forms_Shortcode_Atts', 'maybe_allow_revision'),
			5, 4);

		//email settings
		add_action('engage_forms_core_init', array('Engage_Forms_Email_Settings', 'maybe_add_hooks'));
		add_filter('pre_update_option__engage_forms_email_api_settings', array(
			'Engage_Forms_Email_Settings',
			'sanitize_save'
		));

		if (current_user_can(Engage_Forms::get_manage_cap('admin'))) {
			add_action('wp_ajax_ef_email_save', array('Engage_Forms_Email_Settings', 'save'));
		}

		// Add cache plugin support
		if (method_exists('LiteSpeed_Cache_API', 'esi_enabled') && LiteSpeed_Cache_API::esi_enabled()) {
			LiteSpeed_Cache_API::hook_tpl_esi('engage_forms', 'Engage_Forms_Render_Nonce::hook_esi');
		}

		//auto-population via Easy Pods/ Easy Queries
		add_action('engage_forms_render_start', array(__CLASS__, 'easy_pods_queries_setup'));

		//delete file uploads that are not going in media library
		add_action('engage_forms_submit_complete', array('Engage_Forms_Files', 'cleanup'));
		add_action(Engage_Forms_Files::CRON_ACTION, array('Engage_Forms_Files', 'cleanup_via_cron'));

		//entry permission
		add_filter('engage_forms_manage_cap', array('Engage_Forms_Entry_UI', 'permissions_filter'), 9, 3);

		add_filter('engage_forms_send_email', array(Engage_Forms_Admin::get_instance(), 'block_email_on_edit'), 9);

		if (current_user_can(Engage_Forms::get_manage_cap('admin'))) {
			$id = null;

			$view = false;
			if (isset($_GET['ef-email-preview'], $_GET['ef-email-preview-form'])) {
				if (wp_verify_nonce($_GET['ef-email-preview'], $_GET['ef-email-preview-form'])) {
					$id = $_GET['ef-email-preview-form'];
					$view = true;
				}
			}

			new Engage_Forms_Email_Previews($id, $view);
		...
		add_filter('engage_forms_mailer', array('Engage_Forms_Email_Filters', 'mail_attachment_check'), 10, 3);
		//format email - using old method in case anyone removed the hook
		add_filter('engage_forms_mailer', array($this, 'format_message'));
		//format autoresponder email
		add_filter('engage_forms_autoresponse_mail',
			array('Engage_Forms_Email_Filters', 'format_autoresponse_message'));
		add_filter('engage_forms_mailer', array('Engage_Forms_Email_Filters', 'prepare_headers'));
		add_filter('engage_forms_autoresponse_mail', array('Engage_Forms_Email_Filters', 'prepare_headers'));

		/** Entry Viewer v1 */
		add_action('wp_ajax_browse_entries', array(Engage_Forms_Entry_UI::get_instance(), 'view_entries'));
		add_action('wp_ajax_get_entry', array(Engage_Forms_Entry_UI::get_instance(), 'view_entry'));
		add_action('engage_forms_entry_actions', array(Engage_Forms_Entry_UI::get_instance(), 'get_entry_actions'),
			1);

		add_action('rest_api_init', array(__CLASS__, 'init_rest_api'));

		//entry viewer shortcode
		add_shortcode(Engage_Forms_Entry_Shortcode::get_shortcode_name(),
			array('Engage_Forms_Entry_Shortcode', 'shortcode_callback'));

		//init Credit card # hash class here, not on hook, so it can't be unhooked
		new Engage_Forms_Field_Credit;

		/** Load magic tag system */
		new Engage_Forms_Magic();

		//init utm tag field handler
		$utm = new Engage_Forms_Field_Utm();
		$utm->add_hooks();

		//clear syncer cache on form update
		add_action('engage_forms_save_form', array('Engage_Forms_Sync_Factory', 'clear_cache'));

		//initialize settings
		Engage_Forms_Settings_Init::load();

		//CRON callback for deleting our fake transients
		add_action(Engage_Forms_Transient::CRON_ACTION, array('Engage_Forms_Transient', 'cron_callback'));
		add_action('engage_forms_submit_complete', array('Engage_Forms_Transient', 'cron_callback'));

		//Init GDPR exporters/erasers
        add_action( 'init', [ Engage_Forms_GDPR::class, 'register_gdpr' ] );

		/**
		 * Runs after Engage Forms core is initialized
		 *
		 * @since 1.3.5.3
		 */
		do_action('engage_forms_core_init');

		/** Adding anything to this constructor after engage_forms_core_init action is a violation of intergalactic law */
	...
	{
		return Engage_Forms_Forms::get_form($id_name);
	}
	...
	{
		return Engage_Forms_Forms::get_forms(true, $internal);
	...
	{
		/**
		 * Use this filter to force Engage Forms styles to enqueue early -- IE force them into header
		 *
		 * @since 1.5.0
		 *
		 * @param bool $use To use or not
		 */
		$use = apply_filters('engage_forms_force_enqueue_styles_early', false);
		$on_page = self::check_for_forms_on_page();
		if ($use || $on_page) {
			add_action('wp_enqueue_scripts', array('Engage_Forms_Render_Assets', 'optional_style_includes'));
	...
	public static function check_for_forms_on_page($post = null)
	{
		...
		$has_shortcode = has_shortcode($post->post_content, 'engage-form');
		if ($has_shortcode) {
			return true;
		}

		$has_shortcode = has_shortcode($post->post_content, 'engage-form_modal');
		if ($has_shortcode) {
			return true;
		}
		...
		if (is_array($sidebars) && !empty($sidebars)) {
			$form_widgets = get_option('widget_engage_forms_widget');
			unset($sidebars['wp_inactive_widgets']);
			foreach ($sidebars as $sidebar => $set) {
				if (is_active_sidebar($sidebar)) {
					foreach ($set as $setup) {
						if (false !== strpos($setup, 'engage_forms_widget-')) {
							$widget_instance = str_replace('engage_forms_widget-', '', $setup);
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Engage_Forms_Render_Modals::modal_button');
		return Engage_Forms_Render_Modals::modal_button($atts, $content, $form, $modal_id);
	}
	...
	public static function load_field($form, $field_base_id)
	{
		if (!empty($form['fields'][$field_base_id])) {
			...
			//probably bad, but opportunity to defined using "engage_forms_render_setup_field"
			$field = array();
		}
		/**
		 ...
		 */
		$field = apply_filters('engage_forms_render_setup_field', $field, $form);
	...
	public function load_plugin_textdomain()
	{
		...
		load_plugin_textdomain($this->plugin_slug, false, basename(EFCORE_PATH) . '/languages');
	}
	 ...
	 *
	 */
	public static function init_ef_internal()
	{

		if (false == self::$internal_init) {
			add_rewrite_tag('%ef_api%', '([^&]+)');
			add_rewrite_tag('%ef_entry%', '([^&]+)');
			// INIT API
			add_rewrite_rule('^ef-api/([^/]*)/([^/]*)/?', 'index.php?ef_api=$matches[1]&ef_entry=$matches[2]', 'top');
			add_rewrite_rule('^ef-api/([^/]*)/?', 'index.php?ef_api=$matches[1]', 'top');

			self::$internal_init = true;


			// check update version
			$db_version = get_option('EF_DB', 0);
			update_option( 'EF_DB', 7 );
			$force_update = false;

			// ensure that admin can only force update
			if (Engage_Forms::get_manage_cap('admin') && isset($_GET['cal_db_update'])) {
				$force_update = (bool)wp_verify_nonce($_GET['cal_db_update']);
			}

			include_once EFCORE_PATH . 'includes/updater.php';

			if ( EF_DB > $db_version || $force_update) {
				self::check_tables();
				if ($db_version < 2 || $force_update) {
					engage_forms_db_v2_update();
				}

				if ( $db_version < 4 || $force_update) {
					self::activate_engage_forms(true);
					engage_forms_write_db_flag(4);
				}

				if (($db_version < 6 || $force_update) && class_exists('Engage_Forms_Forms')) {
					engage_forms_db_v6_update();
				}

                if (($db_version < 7 || $force_update) ) {
                    engage_forms_db_v7_update();
                }

				if (($db_version < 8 || $force_update) ) {
					Engage_Forms_Transient::clear_wpcron();
				}

				engage_forms_write_db_flag(EF_DB );

			} else {
				$version = engage_forms_get_last_update_version();
				if (empty($version) || version_compare($version, EFCORE_VER) !== 0) {
					update_option('_engageforms_lastupdate', EFCORE_VER);
				}
	 ...
	 */
	public static function activate_engage_forms($force = false)
	{

		//if running on activation hook, load order is wrong.
		if( ! did_action( 'engage_forms_includes_complete' ) ){
			engage_forms_load();
		}
		...
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "ef_form_entry_values`", ARRAY_A);
		$fields = array();
		...
		if (!in_array('field_id', $fields)) {
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "ef_form_entry_values` ADD `field_id` varchar(20) NOT NULL AFTER `entry_id`;");
			$wpdb->query("CREATE INDEX `field_id` ON `" . $wpdb->prefix . "ef_form_entry_values` (`field_id`); ");
			// update all entries
			$forms = $wpdb->get_results("SELECT `id`,`form_id` FROM `" . $wpdb->prefix . "ef_form_entries`", ARRAY_A);
			$known = array();
			if (!empty($forms)) {
				foreach ($forms as $form) {
					if (!isset($known[$form['form_id']])) {
						$config = Engage_Forms_Forms::get_form($form['form_id']);
						if (empty($config)) {
							continue;
						}
					...
					foreach ($config['fields'] as $field_id => $field) {
						$wpdb->update($wpdb->prefix . "ef_form_entry_values", array('field_id' => $field_id), array(
							'entry_id' => $form['id'],
							'slug' => $field['slug']
						));
					}
		...
		// add status
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "ef_form_entries`", ARRAY_A);
		$fields = array();
		...
		if (!in_array('status', $fields) && $version < '1.2.0') {
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "ef_form_entries` ADD `status` varchar(20) NOT NULL DEFAULT 'active' AFTER `datestamp`;");
			$wpdb->query("CREATE INDEX `status` ON `" . $wpdb->prefix . "ef_form_entries` (`status`); ");
		}
	...
	{
		if (!did_action('engage_forms_checked_tables')) {
			global $wpdb;
			if (!class_exists('Engage_Forms_DB_Tables')) {
				include_once EFCORE_PATH . 'classes/db/tables.php';
			}

			self::$tables = new Engage_Forms_DB_Tables($wpdb);
			 ...
			 * @since 1.5.1
			 *
			 * @param Engage_Forms_DB_Tables $tables Engage_Forms_DB_Tables object
			 */
			do_action('engage_forms_checked_tables', self::$tables);
	...
	{
		_deprecated_function(__METHOD__, '1.5.9', 'Engage_Forms_Email_Filters::mail_attachment_check');
		return Engage_Forms_Email_Filters::mail_attachment_check($mail, $data, $form);
	}
	...
	public static function captcha_check($value, $field, $form)
	{
		...
		if (empty($result->success)) {
			return new WP_Error('error', __("The captcha wasn't entered correctly.",
					'engage-forms') . ' <a href="#" class="reset_' . sanitize_text_field($_POST[$field['ID']]) . '">' . __('Reset',
					'engage-forms') . '<a>.');
		}
	...
	public static function update_field_data($field, $entry_id, $form)
	{
		...
		// is capture?
		$field_type = Engage_Forms_Field_Util::get_type($field, $form);
		$not_support = Engage_Forms_Fields::not_support($field_type, 'entry_list');
		if ($not_support) {
			return;
		}
		...
		if (has_filter('engage_forms_update_field')) {
			$new_data = apply_filters('engage_forms_update_field', $new_data, $field, $form);
		}

		if (has_filter('engage_forms_update_field_' . $field['type'])) {
			$new_data = apply_filters('engage_forms_update_field_' . $field['type'], $new_data, $field, $form);
		}

		if ($original_data !== null) {
			$wpdb->delete($wpdb->prefix . 'ef_form_entry_values', array(
				'entry_id' => $entry_id,
				'field_id' => $field['ID']
			));
		}
		...
		foreach ((array)$new_data as $entry_data) {
			...
			Engage_Forms_Entry_Field::insert($new_entry);
		}
	...
	public static function save_field_data($field, $entry_id, $form)
	{
		...
		foreach ((array)$data as $key => $raw_entry) {
			...
			$entry = Engage_Forms_Sanitize::sanitize($raw_entry);

			if (has_filter('engage_forms_save_field')) {
				/**
				 ...
				 */
				$entry = apply_filters('engage_forms_save_field', $entry, $field, $form, $entry_id);
			}

			if (has_filter('engage_forms_save_field_' . $field['type'])) {
				/**
				 ...
				 * @param int $entry_id ID of entry to save for  @since 1.5.2
				 */
				$entry = apply_filters('engage_forms_save_field_' . $field['type'], $entry, $field, $form, $entry_id);
			}
			...
			// Save
			Engage_Forms_Entry_Field::insert($field_item);
		}

		if (!empty($keyed)) {

			if (has_filter('engage_forms_save_field_combined' . $field['type'])) {
				$data = apply_filters('engage_forms_save_field_combined' . $field['type'], $entry, $field);
			}
	...
	public static function save_final_form($form)
	{
		...
		if (isset($_POST['_ef_frm_edt'])) {
			...
			$details = Engage_Forms::get_entry_detail($_POST['_ef_frm_edt'], $form);

			// check token
			if (is_array($details) && isset($_POST['_ef_frm_edt_tkn'])) {

				$validated = Engage_Forms_Entry_Token::verify_token($_POST['_ef_frm_edt_tkn'], $details['id'],
					$form['ID']);
				if (is_wp_error($validated)) {
					return $validated;
				} else {
					$entry_id = $details['id'];
					$edit_token = Engage_Forms_Entry_Token::create_entry_token($entry_id, $form);
				}
				...
				if (!empty($user_id)) {
					if (!empty($details)) {
						...
						if (current_user_can('edit_posts') || absint($details['user_id'] ) === $user_id) {
							$entryid = $_POST['_ef_frm_edt'];
						} else {
							return new WP_Error('error', __("Permission denied.", 'engage-forms'));
						}
					}
		...
		if (!empty($form['db_support'])) {
			Engage_Forms_Save_Final::save_in_db($form, $entryid);
		}


		if (self::should_send_mail($form, $transdata)) {
			Engage_Forms_Save_Final::do_mailer($form, $entryid);
		}
	...
	public static function debug_mail_send($phpmailer)
	{
		...
		$wpdb->insert($wpdb->prefix . 'ef_form_entry_meta', $meta_entry);
			...
			foreach ($form['processors'] as $processor) {
				if ($processor['type'] == 'form_redirect') {
					...
					if (!empty($processor['config']['url'])) {
						...
						// set message
						add_filter('engage_forms_render_notices', array(
							'Engage_Forms',
						...
						if (!empty($referrer['query'])) {
							...
							if (isset($referrer['query']['ef_su'])) {
								unset($referrer['query']['ef_su']);
							}
	...
	public static function send_auto_response($config, $form)
	{
		...
		$config = apply_filters('engage_forms_autoresponse_config', $config, $form);
		...
		if (Engage_Forms_Email_Prepare::is_list($config['recipient_email'])) {
			$recipients = Engage_Forms_Email_Prepare::prepare_email_array(explode(',', $config['recipient_email']));
			if (!engage_forms_pro_is_active()) {
				$send_local = true;
			} else {
				$send_local = \engagewp\engageforms\pro\container::get_instance()->get_settings()->get_form($form['ID'])->should_send_local();
			}
			if ($send_local) {
		...
		if (!is_email($config['sender_email'])) {
			$config['sender_email'] = Engage_Forms_Email_Fallback::get_fallback($form);
		}

		if (!empty($config['reply_to'])) {
			$email_message['replyto'] = Engage_Forms::do_magic_tags($config['reply_to']);
		...
		if (!empty($config['cc'])) {
			$email_message['cc'] = Engage_Forms::do_magic_tags($config['cc']);

			$cc_array = array_map('trim', preg_split('/[;,]/', Engage_Forms::do_magic_tags($config['cc'])));
		...
		if (!empty($config['bcc'])) {
			$email_message['bcc'] = Engage_Forms::do_magic_tags($config['bcc']);

			$bcc_array = array_map('trim', preg_split('/[;,]/', Engage_Forms::do_magic_tags($config['bcc'])));
		 ...
		 *
		 */
		$email_message = apply_filters('engage_forms_autoresponse_mail', $email_message, $config, $form, $entry_id);
		if (null === $email_message) {
			return;
		}

		if ('wp' !== Engage_Forms_Email_Settings::get_method()) {
			...
			Engage_Forms_Save_Final::do_mailer($form, null, null, $email_message);

			return;
		}

		do_action('engage_forms_do_autoresponse', $config, $form);
		...
		if (!$sent) {
			 ...
			 */
			do_action('engage_forms_autoresponder_failed', $email_message, $config, $form);
		}
	...
	public function get_form_processors($processors)
	{
		$internal_processors = array(
			'auto_responder' => array(
				"name" => __('Auto Responder', 'engage-forms'),
				"description" => __('Sends out an auto response e-mail', 'engage-forms'),
				"post_processor" => array($this, 'send_auto_response'),
				"template" => EFCORE_PATH . "processors/auto_responder/config.php",
				"default" => array(
					'subject' => __('Thank you for contacting us', 'engage-forms')
				),
			),
			'form_redirect' => array(
				"name" => __('Redirect', 'engage-forms'),
				"description" => __('Redirects user to URL on successful submit', 'engage-forms'),
				"template" => EFCORE_PATH . "processors/redirect/config.php",
				"single" => false
			),
			'conditional_recipient' => array(
				"name" => __('Conditional Recipient', 'engage-forms'),
				"description" => __('Send email to different recipients depending on conditions', 'engage-forms'),
				"template" => EFCORE_PATH . "processors/conditional_recipient/config.php",
				'post_processor' => array('Engage_Forms_Processor_Conditional_Recipient', 'post_processor'),
				"single" => false
			),
			'increment_capture' => array(
				"name" => __('Increment Value', 'engage-forms'),
				"description" => __('Increment a value per entry.', 'engage-forms'),
				"processor" => array($this, 'increment_value'),
				"template" => EFCORE_PATH . "processors/increment/config.php",
				"single" => true,
				"conditionals" => false,
				"magic_tags" => array(
					'increment_value'
		...
		if (!empty($wp_api_key)) {
			$internal_processors['akismet'] = array(
				"name" => __('Akismet', 'engage-forms'),
				"description" => __('Anti-spam filtering', 'engage-forms'),
				"pre_processor" => array($this, 'akismet_scanner'),
				"template" => EFCORE_PATH . "processors/akismet/config.php",
				"single" => false,
			);
		}
	...
	public function increment_value($config, $form)
	{
		...
		if (!empty($config['field'])) {
			...
			$field_value = Engage_Forms::get_field_data($field_id, $form);
		}
		 ...
		 * @param array $form Form config
		 */
		$increment_value = apply_filters('engage_forms_incremental_value', $increment_value, $saved_value, $config,
			$form);
	...
	/**
	 * Catch known functions that were supported at https://engageforms.com/doc/calculation-fields/ and are not sipported by parser https://github.com/denissimon/formula-parser
	 *
	 * @since 1.8.5
	 *
	...
	static public function calculation_deprecated_caught_process( $formula, $deprecated )
	{
		if( current_user_can( Engage_Forms::get_manage_cap('admin') ) ) {
			...
			$message = sprintf(
				'"%s" %s',
				$deprecated,
				__('has been deprecated in calculation fields for compatibility with PHP 7.2 or later', 'engage-forms')
			);
			 ...
			 */
			add_filter('engage_forms_render_notices', function( $out ) use ( $message ){
	...
	{
		if (false === Engage_Forms_Field_Util::check_conditional($field, $form)) {
			return;
		}
		...
		if (!empty($field['config']['manual'])) {
			...
			if (!empty($hastags[1])) {
				...
				foreach ($hastags[1] as $tag_key => $tag) {
					...
					foreach (Engage_Forms_Forms::get_fields($form, false) as $key_id => $fefg) {
						if ($fefg['slug'] === $tag) {
							$binds[] = '#' . $key_id;
							$bindfields[] = '"' . $key_id . '"';
		...
		foreach (Engage_Forms_Forms::get_fields($form, false) as $fid => $form_field) {
			if (false !== strpos($formula, $fid)) {
				$number = Engage_Forms_Field_Calculation::get_value($form_field, $form);
		...
		if (false !== strpos($formula, '/0')) {
			return new WP_Error($field['ID'] . '-calculation',
				__('Calculation is invalid (division by zero)', 'engage-forms'));
		}
		...
		if(version_compare(PHP_VERSION, '7.2.0', '<')){
			...
			//Check if the formula use a symbol or function not supported by the parser but declared as supported at https://engageforms.com/doc/calculation-fields/
			...
			if( $deprecated === false ){
				...
				try {
					...
					$total = self::calculation_deprecated_caught_process( $formula, __('A symbol in the formula', 'engage_forms') );
				}
		...
		if (is_infinite($total) || !is_numeric($total)) {
			return new WP_Error($field['ID'] . '-calculation', __('Calculation is invalid', 'engage-forms'));
		}

		$total = Engage_Forms_Field_Util::format_calc_field($field, $total);
	...
	{
		include EFCORE_PATH . "fields/calculation/line-templates.php";
	}
		...
		if (!empty($field['config']['auto'])) {
			...
			switch ($field['config']['auto_type']) {
				...
				case 'easy-query' :
					 ...
					 * @param array $form Form config
					 */
					$args = apply_filters('engage_forms_autopopulate_post_type_args', $args, $field);
					 ...
					 */
					$field_for_value = apply_filters('engage_forms_autopopulate_options_post_value_field',
					 ...
					 * @param array $posts Current post collection.
					 */
					$field_for_label = apply_filters('engage_forms_autopopulate_options_post_label_field',
				...
				case 'taxonomy':
					 ...
					 * @param array $form Form config
					 */
					$args = apply_filters('engage_forms_autopopulate_taxonomy_args', $args);
					 ...
					 */
					$field_for_value = apply_filters('engage_forms_autopopulate_options_taxonomy_value_field',
					 ...
					 * @param array $posts Current term collection.
					 */
					$field_for_label = apply_filters('engage_forms_autopopulate_options_taxonomy_label_field', 'name',
						$field, $form, $terms);
	...
	static public function check_condition($conditions, $form, $entry_id = null)
	{
		 ...
		 */
		$result = apply_filters('engage_forms_pre_check_condition', null, $conditions, $form, $entry_id);
		 ...
		 * @param int|null $entry_id Entry ID. May be null.
		 */
		return apply_filters('engage_forms_post_check_condition', $result, $conditions, $form, $entry_id);
	}
	...
	static public function form_redirect($type, $url, $form, $processid)
	{
		...
		$url = apply_filters('engage_forms_redirect_url', $url, $form, $processid);
		$url = apply_filters('engage_forms_redirect_url_' . $type, $url, $form, $processid);

		do_action('engage_forms_redirect', $type, $url, $form, $processid);
		do_action('engage_forms_redirect_' . $type, $url, $form, $processid);
	...
	public function set_magic_tags($tags)
	{
		...
		$tags['system'] = array(
			'type' => __('System Tags', 'engage-forms'),
			'tags' => $system_tags,
			'wrap' => array('{', '}')
		);

		// get processor tags
		$processors = Engage_Forms_Processor_Load::get_instance()->get_processors();
		if (!empty($processors)) {
	...
	static public function do_magic_tags($value, $entry_id = null, $magic_caller = array())
	{
		...
		if (null !== $entry_id) {
			$entry_details = Engage_Forms_Magic_Doer::magic_tag_meta_prepare($entry_id);

		}

		if (is_string($value)) {
			if (!empty($entry_details)) {
				$value = Engage_Forms_Magic_Doer::do_processor_magic($value, $entry_details);
			}

			// check for magics
			$value = Engage_Forms_Magic_Doer::do_bracket_magic($value, $this_form, $entry_id, $magic_caller,
				$referrer);

			// fields
			$value = Engage_Forms_Magic_Doer::do_field_magic($value, $entry_id, $this_form);
		}
	...
	static public function get_field_types()
	{
		...
		return Engage_Forms_Fields::get_all();
	...
	{
		if (is_string($form)) {
			$form_efg = Engage_Forms_Forms::get_form($form);
			if (!empty($form_efg['ID'])) {
				if ($form_efg['ID'] !== $form || empty($form_efg['processors'])) {
					return false;
				}
			}
			$form = $form_efg;
	...
	static public function set_field_data($field_id, $data, $form, $entry_id = false)
	{
		...
		if (is_string($form)) {
			$form = Engage_Forms_Forms::get_form($form);
		}
		...
		$field = Engage_Forms_Field_Util::get_field($field_id, $form);
		if (is_array($field) && false === Engage_Forms_Field_Util::check_conditional($field, $form)) {
			return;
		}
	...
	static public function get_field_data($field_id, $form, $entry_id = false, $check_conditionals = true)
	{
		...
		if (is_string($form)) {
			$form_obj = Engage_Forms_Forms::get_form($form);
		...
		$field = Engage_Forms_Field_Util::get_field($field_id, $form);
        if ($check_conditionals && is_array($field) && false === Engage_Forms_Field_Util::check_conditional(
                $field,
                $form,
                $entry_id
            )
		...
		if (!empty($entry_id) && isset($form['fields'][$field_id])) {
			...
			if (!$entry) {
				...
				$entry = $wpdb->get_results($wpdb->prepare(
					"SELECT `value` FROM `" . $wpdb->prefix . "ef_form_entry_values` WHERE `entry_id` = %d AND `field_id` = %s",
					$entry_id, $field_id
				), ARRAY_A);

				//allow plugins to alter the value
				$entry = apply_filters('engage_forms_get_field_entry', $entry, $field_id, $form, $entry_id);
		...
		if (isset($form['fields'][$field_id])) {
			...
			// get field
			$field = apply_filters('engage_forms_render_setup_field', $form['fields'][$field_id], $form);

			if (empty($field) || !isset($field['ID'])) {
				return null;
			}
			// get field types
			$field_types = Engage_Forms_Fields::get_all();
			...
			// apply field filter
			if (has_filter('engage_forms_process_field_' . $field['type'])) {
				$entry = apply_filters('engage_forms_process_field_' . $field['type'], $entry, $field, $form);
	...
	static public function get_field_by_slug($slug, $form)
	{
		...
		return Engage_Forms_Field_Util::get_field_by_slug($slug, $form);
	...
	static public function get_entry_detail($entry_id, $form = null)
	{
		...
		$entry = $wpdb->get_row($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "ef_form_entries` WHERE `id` = %d",
			$entry_id), ARRAY_A);
		if (!empty($entry)) {
			if (null === $form) {
				$form = Engage_Forms_Forms::get_form($entry['form_id']);
				if (empty($form)) {
					return null;
				}
			}
		...
		$entry = apply_filters('engage_forms_get_entry_detail', $entry, $entry_id, $form);
	...
	static public function get_entry_meta($entry_id, $form, $type = null)
	{
		...
		$entry_meta_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "ef_form_entry_meta` WHERE `entry_id` = %d",
			$entry_id), ARRAY_A);
		...
		if (!empty($entry_meta_data)) {
			...
			foreach ($entry_meta_data as $meta_index => $meta) {
				...
				$group = 'meta';
				$meta = apply_filters('engage_forms_get_entry_meta', $meta, $form);
				...
				if (isset($form['processors'][$meta['process_id']]) || $meta['process_id'] == '_debug_log') {
					...
					if ($meta['process_id'] == '_debug_log') {
						...
						$entry_meta['debug'] = array(
							'name' => __('Mailer Debug', 'engage-forms'),
							'data' => array(
								'_debug_log' => array(
									'entry' => array(
										'log' => Engage_Forms_Sanitize::remove_scripts($meta)
									)
								)
							)
						);
					...
					$meta = apply_filters('engage_forms_get_entry_meta_' . $form['processors'][$meta['process_id']]['type'],
					...
					if (!empty($meta)) {
						if (!isset($entry_meta[$group])) {
							...
							if (isset($form['processors'][$meta['process_id']]['type']) && isset($processors[$form['processors'][$meta['process_id']]['type']])) {
								...
								if ($meta['process_id'] == '_debug_log') {
									$meta_name = __('Mailer Debug', 'engage-forms');
								} else {
	...
	static public function get_submission_data($form, $entry_id = false, $check_conditionals = true)
	{
		...
		if (is_string($form)) {
			...
			if (!isset($form['ID']) || $form['ID'] !== $form_id) {
				return new WP_Error('fail', __('Invalid form ID', 'engage-forms'));
			}
		}
	...
	{
		/** You MUST not add anything before engage_forms_submit_process_before action.  */

		/**
		 * Runs before Engage_Forms::process_submission() does anything
		 *
		 * @since 1.5.7
		 */
		do_action('engage_forms_submit_process_before');
		...
		if (!empty($referrer['query'])) {
			parse_str($referrer['query'], $referrer['query']);
			if (isset($referrer['query']['ef_er'])) {
				unset($referrer['query']['ef_er']);
			}
			if (isset($referrer['query']['ef_su'])) {
				unset($referrer['query']['ef_su']);
			}
		}
		if ((isset($_POST['_ef_cr_pst']) && !is_object($post)) || (isset($_POST['_ef_cr_pst']) && $post->ID !== (int)$_POST['_ef_cr_pst'])) {
			$post = get_post((int)$_POST['_ef_cr_pst']);
		}
		// get form and check
		$form = Engage_Forms_Forms::get_form($_POST['_ef_frm_id']);
		if (empty($form['ID']) || $form['ID'] != $_POST['_ef_frm_id']) {
			return;
		}

		// instance number
		$form_instance_number = 1;
		if (isset($_POST['_ef_frm_ct'])) {
			$form_instance_number = $_POST['_ef_frm_ct'];
		}

		// check honeypot
		if (Engage_Forms_Field_Honey::active($form)) {
			$passed = Engage_Forms_Field_Honey::check($_POST, $form);
			if (!$passed) {
				$url = Engage_Forms_Field_Honey::redirect_url($referrer, $form_instance_number, $process_id);

				self::form_redirect('complete', $url, $form, uniqid('_ef_bliss_'));
			}

		}

		// init filter
		$form = apply_filters('engage_forms_submit_get_form', $form);
		 ...
		 *
		 */
		do_action('engage_forms_submit_start', $form, $process_id);


		if (!empty($form['fields'])) {
			foreach ($form['fields'] as $field_id => $field) {
				$field = Engage_Forms_Field_Util::get_field($field, $form, true);
		...
		if (!empty($_POST['efajax']) && $_POST['efajax'] == $form['ID']) {
			$form['form_ajax'] = 1;
		}
		...
		if (!empty($field_types)) {
			foreach ($field_types as $fieldType => $fieldConfig) {
				...
				if (isset($fieldConfig['handler'])) {
					add_filter('engage_forms_process_field_' . $fieldType, $fieldConfig['handler'], 10, 3);
				}
				// check for a hash
				if (isset($fieldConfig['save'])) {
					add_filter('engage_forms_save_field_' . $fieldType, $fieldConfig['save'], 10, 4);
				}
				// check for a hash
				if (isset($fieldConfig['validate'])) {
					add_filter('engage_forms_validate_field_' . $fieldType, $fieldConfig['validate'], 10, 3);
				}
			}
		}
		...
		// SET process ID
		if (isset($_GET['ef_er'])) {
			$_POST['_ef_frm_tr'] = $_GET['ef_er'];
		}

		if (isset($_POST['_ef_frm_tr'])) {
			$pretransient = Engage_Forms_Transient::get_transient($_POST['_ef_frm_tr']);
			if (!empty($pretransient['transient']) && $pretransient['transient'] === $_POST['_ef_frm_tr']) {
		...
		if (empty($process_id)) {
			$process_id = uniqid('_ef_process_');
		}

		// initialize data
		$entry_id = false;
		if (isset($_POST['_ef_frm_edt'])) {
			$entry_id = (int)$_POST['_ef_frm_edt'];
		}
		...
		if (isset($transdata['data']['efajax'])) {
			unset($transdata['data']['efajax']);
		}
		// setup transient data
		$transdata = apply_filters('engage_forms_submit_transient_setup', $transdata);
		...
		if (isset($_POST['_ef_frm_edt'])) {
			// is edit
			//check user can edit this item.
			$transdata['edit'] = (int)$_POST['_ef_frm_edt'];
			// set entry_id
			self::set_field_data('_entry_id', $transdata['edit'], $form);
			$details = self::get_entry_detail($_POST['_ef_frm_edt'], $form);
			$user_id = get_current_user_id();

			// check token
			if (isset($_POST['_ef_frm_edt_tkn'])) {

				$validated = Engage_Forms_Entry_Token::verify_token($_POST['_ef_frm_edt_tkn'], $entry_id, $form['ID']);
				if (is_wp_error($validated)) {
					return $validated;
				} else {
					$edit_token = Engage_Forms_Entry_Token::create_entry_token($entry_id, $form);
				...
				if (empty($user_id)) {
					...
					$transdata['note'] = __('Permission denied or entry does not exist.', 'engage-forms');
				} else {
		...
		//@see https://github.com/EngageWP/Engage-Forms/issues/1579
		if (empty($_POST['_ef_future'])) {
			$future_fields = '';
		} else {
			$future_fields = $_POST['_ef_future'];
		}
		...
		foreach ($form['fields'] as $field_id => $field) {
			...
			if (is_wp_error($entry)) {
				...
				$failed = false;
				// run validators
				if (has_filter('engage_forms_validate_field_' . $field['type'])) {
					/**
					 ...
					 * @param array $form Form config
					 */
					$entry = apply_filters('engage_forms_validate_field_' . $field['type'], $entry, $field, $form);
				}
				 ...
				 * @param array $form Form config
				 */
				$entry = apply_filters('engage_forms_validate_field_' . $field['ID'], $entry, $field, $form);
				...
				if (!empty($field['required'])) {
					...
					if (is_wp_error($entry)) {
						...
						$transdata['fields'][$field_id] = $field['label'] . ' ' . __('is required', 'engage-forms');
					}
				}
			}
		...
		if (!empty($transdata['fields']) || !empty($transdata['error'])) {
			...
			// set error transient
			$transdata = apply_filters('engage_forms_submit_return_transient', $transdata, $form, $referrer,
				$process_id);
			$transdata = apply_filters('engage_forms_submit_return_transient_required', $transdata, $form, $referrer,
				$process_id);
			...
			$referrer = apply_filters('engage_forms_submit_return_redirect', $referrer, $form, $process_id);
			$referrer = apply_filters('engage_forms_submit_return_redirect_required', $referrer, $form, $process_id);

			Engage_Forms_Transient::set_transient($process_id, $transdata, $transdata['expire']);
		...
		// has processors
		do_action('engage_forms_submit_start_processors', $form, $referrer, $process_id);
		if (!isset($form['processors'])) {
			$form['processors'] = array();
		}

		// get all form processors
		$form_processors = Engage_Forms_Processor_Load::get_instance()->get_processors();
		 ...
		 */
		do_action('engage_forms_submit_pre_process_start', $form, $referrer, $process_id);
		 ...
		 */
		foreach ($form['processors'] as $processor_id => $processor) {
			// the ef_version value in the form was introduced in 1.3.2
			// so if its set, its safe to asume the runtimes are set.
			if (!isset($form['ef_version'])) {
				// nope
			...
			if (isset($form_processors[$processor['type']])) {
				...
				if (!empty($process_line_data)) {
					if (is_array($process_line_data)) {
						...
						// set error transient
						$transdata = apply_filters('engage_forms_submit_return_transient', $transdata, $form,
							$referrer, $process_id);
						$transdata = apply_filters('engage_forms_submit_return_transient_pre_process', $transdata,
						...
						$referrer = $referrer['path'] . '?' . http_build_query($query_str);
						$referrer = apply_filters('engage_forms_submit_return_redirect', $referrer, $form,
							$process_id);
						$referrer = apply_filters('engage_forms_submit_return_redirect-' . $processor['type'],
		 ...
		 * @param string $process_id Unique ID for this processing
		 */
		do_action('engage_forms_submit_pre_process_end', $form, $referrer, $process_id);
		...
		if (empty($transdata['edit']) && !empty($form['db_support'])) {
			...
			$wpdb->insert($wpdb->prefix . 'ef_form_entries', $new_entry);
			 ...
			 */
			do_action('engage_forms_entry_saved', $entryid, $new_entry, $form);

			// save entry_id
			self::set_field_data('_entry_id', $entryid, $form);
			$token = Engage_Forms_Entry_Token::create_entry_token($entryid, $form);
		 ...
		 */
		do_action('engage_forms_submit_process_start', $form, $referrer, $process_id, $entryid);
		/// PROCESS
		foreach ($form['processors'] as $processor_id => $processor) {
			if (isset($form_processors[$processor['type']])) {
				// has processor
				...
				if (!empty($transdata['error'])) {
					...
					if (!empty($entryid) && !empty($new_entry) && $new_entry['status'] == 'pending') {
						// kill it with fire
						$wpdb->delete($wpdb->prefix . 'ef_form_entries', array('id' => $entryid));
					}
					// set error transient
					$transdata = apply_filters('engage_forms_submit_error_transient', $transdata, $form, $referrer,
						$process_id);
					$transdata = apply_filters('engage_forms_submit_error_transient_pre_process', $transdata, $form,
						$referrer, $process_id);
					...
					$referrer = $referrer['path'] . '?' . http_build_query($query_str);
					$referrer = apply_filters('engage_forms_submit_error_redirect', $referrer, $form, $process_id);
					$referrer = apply_filters('engage_forms_submit_error_redirect_pre_process', $referrer, $form,
						$process_id);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('engage_forms_submit_process_end', $form, $referrer, $process_id, $entryid);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('engage_forms_submit_post_process', $form, $referrer, $process_id, $entryid);
		// POST PROCESS
		foreach ($form['processors'] as $processor_id => $processor) {
			if (isset($form_processors[$processor['type']])) {
				// has processor
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('engage_forms_submit_post_process_end', $form, $referrer, $process_id, $entryid);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('engage_forms_submit_complete', $form, $referrer, $process_id, $entryid);

		// redirect back or to result page
		$referrer['query']['ef_su'] = $form_instance_number;

		// last entry id - new only
		if (empty($transdata['edit'])) {
			$ef_id = self::do_magic_tags('{entry_id}');
			if (!empty($ef_id)) {
				$referrer['query']['ef_id'] = self::do_magic_tags('{entry_id}');
			}
		}
		...
		// filter refer
		$referrer = apply_filters('engage_forms_submit_redirect', $referrer, $form, $process_id);
		$referrer = apply_filters('engage_forms_submit_redirect_complete', $referrer, $form, $process_id);

		// kill transient data
		Engage_Forms_Transient::delete_transient($process_id);
	...
	/**
	 * Makes Engage Forms load the preview
	 */
	static public function ef_init_preview()
	{
		if (!isset($_GET, $_GET[Engage_Forms_Admin::PREVIEW_KEY])) {
			return;
		}
		...
		$preview_id = trim($_GET[Engage_Forms_Admin::PREVIEW_KEY]);
		if (!empty($preview_id)) {
			$form = Engage_Forms_Forms::get_form($preview_id);
			...
			if (!empty($userid)) {
				...
				if (empty($post) || $post->post_title !== 'Engage Forms Preview') {
                    // Using WP_Query to find the page by title
                    $args = array(
                        'post_type' => 'page',
                        'post_status' => 'any',
                        'title' => 'Engage Forms Preview',
                        'posts_per_page' => 1
                    );
                    ...
                    if (!$query->have_posts()) {
                        ...
                        $post_arr = array(
                            'post_content' => '',
                            'post_name' => 'engage_forms_preview',
                            'post_title' => 'Engage Forms Preview',
                            'post_status' => 'draft',
                            'post_type' => 'page',
                            'ping_status' => 'closed',
                        ...
                        // Construct URL with the newly created page
                        $url = add_query_arg(array(
                            'page_id' => $page_id,
                            'preview' => true,
                            Engage_Forms_Admin::PREVIEW_KEY => $preview_id
                        ), home_url());
                    } else {
                        // If the page exists, ensure it's a draft
                        $temp_page = $query->posts[0];
                        if ($temp_page->post_status !== 'draft') {
                            wp_update_post(array('ID' => $temp_page->ID, 'post_status' => 'draft'));
                        }
                        // Construct URL with the found page
                        $url = add_query_arg(array(
                    ...
                    // Common redirect for both created and found page
                    if (isset($_GET[Engage_Forms_Admin::REVISION_KEY])) {
                        $url = add_query_arg(Engage_Forms_Admin::REVISION_KEY, absint($_GET[Engage_Forms_Admin::REVISION_KEY]), $url);
                    }
				...
				if (isset($_GET[Engage_Forms_Admin::REVISION_KEY])) {
					$post->post_content = '[engage-form id="' . $_GET[Engage_Forms_Admin::PREVIEW_KEY] . ' revision="' . absint($_GET[Engage_Forms_Admin::REVISION_KEY]) . '"]';
				} else {
					$post->post_content = '[engage-form id="' . $_GET[Engage_Forms_Admin::PREVIEW_KEY] . '"]';
				}
			}
		}
	...
	public function api_handler()
	{
		...
		if (isset($wp_query->query_vars[ 'ef_api' ])) {
			$form_id = engage_forms_very_safe_string($wp_query->query_vars[ 'ef_api' ]);
		} elseif (isset($_GET, $_GET[ 'ef-api' ])) {
			$form_id = engage_forms_very_safe_string($_GET[ 'ef-api' ]);
		} else {
			$form_id = null;
		}
		...
		if (!$form_id) {
			...
			if (!empty($_REQUEST[ 'ef_instance' ])) {
				$atts[ 'instance' ] = $_REQUEST[ 'ef_instance' ];
			}

			//start buffering
			engage_forms_start_buffer();
			// push 200 status. in some cases plugins or permalink config may cause a 404 before going out
			header("HTTP/1.1 200 OK", true);
			if (!empty($form[ 'ID' ])) {

				if ($form[ 'ID' ] === $form_id) {
					$entry_id = ( !empty($wp_query->query_vars[ 'ef_entry' ]) ? $wp_query->query_vars[ 'ef_entry' ]
					...
					if ($_SERVER[ 'REQUEST_METHOD' ] === 'POST') {
						...
						if (!empty($_POST[ 'control' ])) {
							...
							$transdata = Engage_Forms_Transient::get_transient($transient_name);
							if (false === $transdata) {
								$transdata = [];
							}
							if (!empty($_FILES) && !empty($_POST[ 'field' ])) {
								$field = Engage_Forms_Field_Util::get_field($form[ 'fields' ][ $_POST[ 'field' ] ],
									$form, true);
								$data = ef_handle_file_upload(true, $field, $form);
								if (is_wp_error($data)) {
									engage_forms_send_json($data->get_error_message(), true);
								}
								$transdata[] = $data;
								Engage_Forms_Transient::set_transient($transient_name, $transdata, DAY_IN_SECONDS);
								engage_forms_send_json(['success' => true]);
							}
						}

						$_POST[ '_wp_http_referer_true' ] = 'api';
						$_POST[ '_ef_frm_id' ] = $_POST[ 'efajax' ] = $form_id;
	...
	static function get_submit_url($form_id = '')
	{
		...
		if (is_multisite() && get_blog_option(null, 'permalink_structure') || get_option('permalink_structure')) {
			$url = get_home_url();
			$url .= '/ef-api/' . ltrim($form_id, '/');
		} else {
			$url = trailingslashit(get_home_url());
			$url = add_query_arg('ef_api', $form_id, $url);
		}
		if (is_ssl()) {
		...
		/**
		 * Filter the Engage Forms APU url
		 *
		 * @since 1.3.2
		 *
		 ...
		 */
		return apply_filters('engage_forms_submission_url', $url, $form_id);

	}

	/**
	 * Makes Engage Forms go in front-end!
	 */
	static public function ef_init_system()
	{

		if (!empty($_GET['ef_tp'])) {

			// process a transient stored entry
			$data = Engage_Forms_Transient::get_transient($_GET['ef_tp']);
			if (!empty($data) && $data['transient'] === $_GET['ef_tp'] && isset($data['data'])) {
				// create post values
				$_POST = array_merge($_POST, $data['data']);
				// set transient id
				$_POST['_ef_frm_tr'] = $data['transient'];
			}
		}
	...
	static public function get_entry($entry_id, $form)
	{
		if (is_string($form)) {
			...
			if (!is_array($form) || !isset($form['ID']) || $form['ID'] !== $form_id) {
				return new WP_Error('fail', esc_html__('Invalid form ID', 'engage-forms'));
			}
		}
		...
		//See: https://github.com/EngageWP/Engage-Forms/issues/1494
		...
		foreach ($entry as $field_id => $field_value) {
			...
			$type = Engage_Forms_Field_Util::get_type(Engage_Forms_Field_Util::get_field($field_id, $form));
			if (Engage_Forms_Fields::not_support($type, 'entry_list')) {
				continue;
			}
			...
			// set view
			$field_view = apply_filters('engage_forms_view_field_' . $field['type'], $field_value, $field, $form);
		...
		if (!empty($form['variables']['types'])) {
			foreach ($form['variables']['types'] as $var_key => $var_type) {
				if ($var_type == 'entryitem') {
					$var_val = Engage_Forms::do_magic_tags($form['variables']['values'][$var_key], $entry_id);
		 ...
		 * @param array $form Form config
		 */
		$data['user'] = apply_filters('engage_forms_get_entry_user', $data['user'], $entry_id, $form);
		 ...
		 * @param array $form Form config
		 */
		$data = apply_filters('engage_forms_get_entry', $data, $entry_id, $form);
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Engage_Forms_Render_Modals::render_modal_form');
		return Engage_Forms_Render_Modals::modal_form($atts, $content);
	}
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Engage_Forms_Render_Modals::render_footer_modals');
		Engage_Forms_Render_Modals::render_footer_modals();
	}
	...
	static public function render_field($field, $form = null, $entry_data = array(), $field_errors = array())
	{
        ...
        $field_id_attr = Engage_Forms_Field_Util::get_base_id($field, $current_form_count, $form);
        if( Engage_Forms_Field_Util::is_ef2_field_type($type)){
            ...
            $renderer = new \engagewp\engageforms\ef2\Fields\RenderField($form_id_attribute,
                array_merge( $field, [
		...
		$field_classes = Engage_Forms_Field_Util::prepare_field_classes($field, $form);
		...
		$field_structure = apply_filters('engage_forms_render_field_structure', $field_structure, $form);
		$field_structure = apply_filters('engage_forms_render_field_structure_type-' . $field['type'],
			$field_structure, $form);
		$field_structure = apply_filters('engage_forms_render_field_structure_slug-' . $field['slug'],
			$field_structure, $form);
		 ...
		 */
		$field_file = apply_filters('engage_forms_render_field_file', $field_file, $field['type'], $field['ID'],
			$field_file, $field_structure, $form);

		ob_start();
		include $field_file;
		$field_html = apply_filters('engage_forms_render_field', ob_get_clean(), $form);
		$field_html = apply_filters('engage_forms_render_field_type-' . $field['type'], $field_html, $form);
		$field_html = apply_filters('engage_forms_render_field_slug-' . $field['slug'], $field_html, $form);
	...
	static public function render_form($atts, $entry_id = null, $shortcode = null)
	{
		...
		if (is_string($atts)) {
			...
			$form = Engage_Forms_Forms::get_form($atts);
			$atts = array();

		} elseif (is_array($atts) && Engage_Forms_Forms::is_revision($atts)) {
			$form = $atts;
		...
		if (!empty($form['form_draft'])) {
			if (!isset($_GET['ef_preview']) || $_GET['ef_preview'] != $form['ID']) {
				if (isset($_POST['action']) && $_POST['action'] == 'ef_get_form_preview') {
					echo '<p style="color: #ef0000;">' . __('Form is currently not active.', 'engage-forms') . '</p>';
				} else {
					return;
				}

			} else {
				echo '<div class="engage-grid"><p class="alert alert-error alert-danger">' . __('Form is currently not active.',
						'engage-forms') . '</p></div>';
			}
		}

		Engage_Forms_Render_Assets::optional_style_includes();
		 ...
		 * @param array $form Form config.
		 */
		$form = apply_filters('engage_forms_render_get_form', $form);
		 ...
		 * @param array $form Form config.
		 */
		$entry_id = apply_filters('engage_forms_render_entry_id', $entry_id, $form);
		 ...
		 */
		$html = apply_filters('engage_forms_pre_render_form', null, $entry_id, $form, $atts);
		if (is_string($html)) {
			return $html;
		...
		do_action('engage_forms_render_start', $form);
		...
		if (!empty($form['form_ajax'])) {
			add_filter('engage_forms_render_form_attributes', 'ef_ajax_setatts', 10, 2);
		 ...
		 * @param array $config Form config
		 */
		$form_attributes = apply_filters('engage_forms_render_form_attributes', $form_attributes, $form);


		include_once EFCORE_PATH . "classes/engage-grid.php";
		 ...
		 */
		$gridsize = apply_filters('engage_forms_render_set_grid_size', $gridsize);
		...
		// filter settings
		$grid_settings = apply_filters('engage_forms_render_grid_settings', $grid_settings, $form);

		$form['grid_object'] = new Engage_Form_Grid($grid_settings);
		...
		if (!empty($entry_id)) {
			...
			if (!empty($_GET['ef_et'])) {
				// build token
				$validated = Engage_Forms_Entry_Token::verify_token(trim($_GET['ef_et']), $entry_id, $form['ID']);
				if (is_wp_error($validated)) {
					$notices['error']['note'] = __('Permission denied or entry does not exist.', 'engage-forms');
				} else {
					$edit_token = Engage_Forms_Entry_Token::create_entry_token($entry_id, $form);
				}
		...
		// check for prev post
		$prev_data = apply_filters('engage_forms_render_pre_get_entry', array(), $form, $entry_id);
		...
		if (!empty($entry_id)) {
			...
			$prev_data = apply_filters('engage_forms_render_get_entry', $prev_data, $form, $entry_id);
		}


		if(!empty($_GET['ef_er'])){
			$prev_post = Engage_Forms_Transient::get_transient( $_GET['ef_er'] );
			if( !empty( $prev_post['transient'] ) && $prev_post['data']['_ef_frm_id'] === $form['ID'] ){

				if ($prev_post['transient'] === $_GET['ef_er']) {
			...
			// filter transient
			$prev_post = apply_filters('engage_forms_render_get_transient', $prev_post, $form);

		}
		if (!empty($_GET['ef_su']) && $current_form_count == $_GET['ef_su']) {
		...
		if (!empty($page_breaks)) {
			$currentpage = 1;
			if (isset($_GET['ef_pg']) && !isset($prev_post['page'])) {
				$currentpage = (int)$_GET['ef_pg'];
			...
			$form['grid_object']->before('<div id="form_page_' . $current_form_count . '_pg_1" data-formpage="1" class="engage-form-page" style="display:' . $display . ';" role="region" aria-labelledby="breadcrumb_' . $current_form_count . '_pg_1" aria-hidden="' . $hidden . '">',
				1);
			...
			foreach ($page_breaks as $page => $break) {
				...
				if ($break + 1 <= $total_rows) {
					...
					$form['grid_object']->before('<div id="form_page_' . $current_form_count . '_pg_' . ($page + 2) . '" data-formpage="' . ($page + 2) . '" role="region" aria-labelledby="breadcrumb_' . $current_form_count . '_pg_' . ($page + 2) . '" aria-hidden="' . $hidden . '" class="engage-form-page" style="display:' . $display . ';">',
						$break + 1);
				}
			}
		...
		if (!empty($form['fields'])) {
			// prepare fields
			foreach ($form['fields'] as $field_id => $field) {
				$field = apply_filters('engage_forms_render_get_field', $field, $form);
				$field = apply_filters('engage_forms_render_get_field_type-' . $field['type'], $field, $form);
				$field = apply_filters('engage_forms_render_get_field_slug-' . $field['slug'], $field, $form);
				$form['fields'][$field_id] = $field;
			}
		}
			...
			foreach ($form['layout_grid']['fields'] as $field_base_id => $location) {
				...
				if (isset($form['fields'][$field_base_id])) {
					...
					if (empty($field) || !isset($field_types[$field['type']]['file']) || !file_exists($field_types[$field['type']]['file'])) {
                        if (!Engage_Forms_Field_Util::is_ef2_field_type($field['type'])) {
                            continue;
                        }
                    }
					...
					Engage_Forms_Render_Assets::enqueue_field_scripts($field_types, $field);
					...
					if (!empty($field['conditions']['group']) && !empty($field['conditions']['type'])) {
                        $is_ef2_field = Engage_Forms_Field_Util::is_ef2_field_type( Engage_Forms_Field_Util::get_type( $field, $form ) );
						...
						// wrapp it up
						$field_html = '<span class="engage-forms-conditional-field" role="region" aria-live="polite" id="conditional_' . $field_base_id . '" data-field-id="' . $field_base_id . '">' . $field_html . '</span>';
					}
		...
		// form object strings
		wp_localize_script('ef-dynamic', $form['ID'] . '_' . $current_form_count, $form_field_strings);

		// do grid
		$form['grid_object'] = apply_filters('engage_forms_render_grid_structure', $form['grid_object'], $form);
		// wrapper classes
		 ...
		 * @param array $config Form config
		 */
		$form_wrapper_classes = apply_filters('engage_forms_render_form_wrapper_classes', $form_wrapper_classes,
			$form);

		$form_wrap_id = Engage_Forms_Render_Util::form_id_attr($current_form_count);
		$out = sprintf('<div class="%s" id="%s" data-ef-ver="%s" data-ef-form-id="%s">',
			esc_attr(implode(' ', $form_wrapper_classes)), esc_attr($form_wrap_id), esc_attr(EFCORE_VER),
			esc_attr($form['ID']));
		...
		if (!empty($form['mailer']['enable_mailer']) && !empty($form['debug_mailer'])) {
			$notices['error'] = array(
				'note' => __('WARNING: Form is in Mailer Debug mode. Disable before going live.', 'engage-forms')
			);
		}

		$out .= '<div id="engage_notices_' . $current_form_count . '" data-spinner="' . admin_url('images/spinner.gif') . '">';
		if (!empty($notices)) {
			// do notices
			// entry id
			if (isset($_GET['ef_id'])) {
				$notice_entry_id = (int)$_GET['ef_id'];
			...
			foreach ($notices as $note_type => $notice) {
				if (!empty($notice['note'])) {
					$out .= '<div class=" ' . implode(' ', $note_classes[$note_type]) . '">' . Engage_Forms_Sanitize::remove_scripts(self::do_magic_tags($notice['note'], $notice_entry_id) ) . '</div>';
				}
			}
		...
		if ((empty($notices['success']) || empty($form['hide_form'])) && empty($halt_render)) {
			 ...
			 * @param array $config Form config
			 */
			$form_element = apply_filters('engage_forms_render_form_element', $form_element, $form);
			 ...
			 * @param array $config Form config
			 */
			$form_classes = apply_filters('engage_forms_render_form_classes', $form_classes, $form);
			...
			$out .= Engage_Forms_Render_Nonce::nonce_field($form['ID']);
			$out .= sprintf( '<div id="%s"></div>', esc_attr( "ef2-$form_id_attribute") );
			$out .= "<input type=\"hidden\" name=\"_ef_frm_id\" value=\"" . $form['ID'] . "\">\r\n";
			$out .= "<input type=\"hidden\" name=\"_ef_frm_ct\" value=\"" . $current_form_count . "\">\r\n";
			if (!empty($form['form_ajax'])) {
				$out .= "<input type=\"hidden\" name=\"efajax\" value=\"" . $form['ID'] . "\">\r\n";
			}
			if (is_object($post)) {
				$out .= "<input type=\"hidden\" name=\"_ef_cr_pst\" value=\"" . $post->ID . "\">\r\n";
			}

			// user transient for continuation
			if (!empty($prev_post['transient'])) {
				$out .= "<input type=\"hidden\" name=\"_ef_frm_tr\" value=\"" . $prev_post['transient'] . "\">\r\n";
			}
			// is edit?
			if (!empty($entry_id)) {
				$out .= "<input type=\"hidden\" name=\"_ef_frm_edt\" value=\"" . $entry_id . "\">\r\n";
			}

			// is edit via token?
			if (!empty($edit_token)) {
				$out .= "<input type=\"hidden\" name=\"_ef_frm_edt_tkn\" value=\"" . $edit_token . "\">\r\n";
			}

			//setup fieldjs
			Engage_Forms_Field_Localizer::add_form($form, Engage_Forms_Render_Util::get_current_form_count());
			...
			if (!empty($form['auto_progress']) && count($form['page_names']) > 1) {
				...
				$out .= "<span class=\"engage-grid\"><ol class=\"breadcrumb\" data-form=\"engage-form_" . $current_form_count . "\" id=\"engage-forms-breadcrumb_" . $current_form_count . "\">\r\n";
				$current_page = 1;
				if (!empty($_GET['ef_pg'])) {
					$current_page = $_GET['ef_pg'];
				}
				foreach ($form['page_names'] as $page_key => $page_name) {
					...
					$qurystr['ef_pg'] = $page_key + 1;
					$out .= "<li" . $tabclass . "><a aria-controls=\"form_page_" . $current_form_count . "_pg_" . ($page_key + 1) . "\" aria-expanded=\"" . $expanded . "\" id=\"breadcrumb_" . $current_form_count . "_pg_" . ($page_key + 1) . "\" href=\"?" . http_build_query($qurystr) . "\" data-page=\"" . ($page_key + 1) . "\" data-pagenav=\"engage-form_" . $current_form_count . "\" title=\"" . sprintf(__('Navigate to %s',
							'engage-forms'), $page_name) . "\">" . $page_name . "</a></li>\r\n";
				}
				$out .= "</ol></span>\r\n";
			}
			...
			// sticky sticky honey
			if (Engage_Forms_Field_Honey::active($form)) {
				$out .= Engage_Forms_Field_Honey::field($form);
			}
		...
		if (!empty($conditions_configs)) {
			...
			$hastags = Engage_Forms_Magic_Util::explode_field_magic($conditions_str);
			if (!empty($hastags[1])) {
			...
			$out .= 'if( typeof engage_conditionals === "undefined" ){ var engage_conditionals = {}; }';
			$out .= "engage_conditionals." . $form['ID'] . '_' . $current_form_count . " = " . $conditions_str . ";\r\n";
			$out .= "</script>\r\n";
			...
			Engage_Forms_Render_Assets::enqueue_script('conditionals');
		}
		 ...
		 * @param array $config Form config
		 */
		do_action('engage_forms_render_end', $form);

		Engage_Forms_Render_Assets::enqueue_form_assets();
		 ...
		 * @param array $config Form config
		 */
		return apply_filters('engage_forms_render_form', $out, $form);

	}

	/**
	 * Returns the capability to manage Engage Forms
	 *
	 * By default, returns "manage_options" can be filtered with "engage_forms_manage_cap"
	 *
	 * @since 1.3.1
	 *
	...
	public static function get_manage_cap($context = 'admin', $form = false)
	{
		...
		/**
		 * Change capability for managing Engage Forms
		 *
		 * @since 1.3.1
		 *
		 ...
		 */
		return apply_filters('engage_forms_manage_cap', 'manage_options', $context, $form);
	 ...
	 * @param string $shortcode Shortcode type engage-form|engage_forms_modal
	 *
	 * @return string|void
	 */
	public static function shortcode_handler($atts, $content, $shortcode)
	{
		...
		if (!empty($atts['revision'])) {
			$revision = Engage_Forms_Forms::get_revision($atts['revision']);
		...
		if ($shortcode === 'engage-form_modal' || (!empty($atts['modal']) && $atts['modal'])) {
			return Engage_Forms_Render_Modals::modal_form($atts, $content);
		}
	...
	/**
	 * Load the Engage Forms REST API
	 *
	 * @since 1.4.4
	 *
	...
	public static function init_rest_api()
	{
		...
		self::$api = new Engage_Forms_API_Load(Engage_Forms_API_Util::api_namespace());
		 ...
		 *
		 * do_action( 'engage_forms_rest_api_pre_init', function( $api ){ $api->add_route(...
		 *
		 * @since 1.4.4
		 *
		 * @param Engage_Forms_API_Load $api API Load class
		 */
		do_action('engage_forms_rest_api_pre_init', self::$api);

		self::$api->add_route(new Engage_Forms_API_Entries());
		self::$api->add_route(new Engage_Forms_API_Forms());
		self::$api->add_route(new Engage_Forms_API_Settings());
		 ...
		 * @since 1.4.4
         *
         * @param Engage_Forms_API_Load $api_v2 Engage Forms REST API v2
         * @param \engagewp\engageforms\ef2\RestApi\Register $api_v3 Engage Forms REST API v2
		 */
		do_action('engage_forms_rest_api_init',
            self::$api,
            (new \engagewp\engageforms\ef2\RestApi\Register(Engage_Forms_API_Util::api_namespace('v3' ) ) )
                ->initEndpoints()
        );
	...
	public static function should_send_mail($form, $transadata = array())
	{
		 ...
		 * @param array $form Form config
		 */
		return apply_filters('engage_forms_send_email', $send, $form);
	...
	{
		if (isset($_POST['_ef_frm_id'])) {
			engage_forms_start_buffer();
			if (isset($_POST['_ef_verify']) && Engage_Forms_Render_Nonce::verify_nonce($_POST['_ef_verify'],
					$_POST['_ef_frm_id'])) {
				$submission = Engage_Forms::process_submission();
				engage_forms_send_json($submission);
				exit;
			}
			...
			$form = Engage_Forms_Forms::get_form($_POST['_ef_frm_id']);
			$notices = array();
			$notices['error']['note'] = __('Submission rejected, token invalid', 'engage-forms');
			...
			engage_forms_send_json($out,true);
			exit;
	...
	public static function format_message($mail)
	{
		...
		return Engage_Forms_Email_Filters::format_message($mail);

	}

	/**
	 * Get main instance of Engage_Forms_Settings class
	 *
	 * @since 1.5.3
	 *
	 * @return Engage_Forms_Settings
	 */
	public static function settings()
	{
		if (!self::$settings) {
			self::$settings = new Engage_Forms_Settings();
			/**
			 * Runs after main instance of Engage_Forms_Settings is created
			 *
			 * Access via Engage_Forms::settings()
			 *
			 * @since 1.5.3
			 */
			do_action('engage_forms_settings_registered');
		}
