 ...
 */
class Engage_Forms_Entry_Entries {
	...
	/**
	 * Engage_Forms_Entry_Entries constructor.
	 *
	 * @since 1.4.0
	 *
	 ...
	 *
	 * This is for use inside of Engage_Forms_Admin::get_entries() for backwards compat reasons
	 *
	 * @since 1.4.0
	 *
	...
	public function get_rows( $page, $entry_id, $status = 'active' ) {
		...
		if ( ! empty( $field_values ) ) {
			/** @var Engage_Forms_Entry_Field $_field */
	...
	protected function query_page( $page, $status ) {
		...
		if ( ! empty( $_entries ) ) {
			...
			foreach ( $_entries as $_entry ) {
				$entry                                                  = new Engage_Forms_Entry_Entry( $_entry );
				$this->entries[ $status ][ $page ][ (int) $_entry->id ] = new Engage_Forms_Entry( $this->form, $entry->id, $entry );
			}
	...
	protected function query_total( $status ) {
		return Engage_Forms_Entry_Bulk::count( $this->form[ 'ID' ], $status );
	}
