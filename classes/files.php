 ...
 */
class Engage_Forms_Files{

    const CRON_ACTION = 'engage_forms_delete_files';
    ...
    public static function add_to_media_library( $upload, $field ){
	    ...
	    /**
	     * Runs after file is uploaded to media library by Engage Forms
	     *
	     * @since 1.5.1
	     *
	     ...
	     */
		do_action( 'engage_forms_file_added_to_media_library', $media_id, $field );
	    return $media_id;
    }
    ...
    public static function add_upload_filter( $field_id , $form_id, $private = true, $transient_id = null ){
	    if ( $private ) {
		    self::$dir = apply_filters( 'engage_forms_private_upload_directory', self::secret_dir( $field_id, $form_id, $transient_id), $field_id, $form_id, $transient_id );
	    }else{
		    /**
			 ...
			 */
	    	$dir = apply_filters( 'engage_forms_upload_directory', null, $field_id, $form_id );
		    if( null != $dir ){
		    	self::$dir = $dir;
		    }
	    }
    ...
    public static function cleanup( $form, $second_run = false ){
        if( false === $second_run && Engage_Forms::should_send_mail( $form ) ) {
            add_action( 'engage_forms_mailer_complete', array( __CLASS__, 'delete_after_mail' ), 10, 3 );
            add_action( 'engage_forms_mailer_failed', array( __CLASS__, 'delete_after_mail' ), 10, 3 );
            return;
        }
     ...
     *
     * We use "engage_forms_submit_complete" to start the clean up, but that is too soon, if using mailer.
     *
     * @since 1.4.4
     *
     * @param $mail
     ...
     *
     * This is needed because if a form never completed submission, files are not deleted at engage_forms_submit_complete
     *
     * @since 1.4.4
     *
     * @param array $args
    ...
    public static function is_private( array  $field ){
    	...
    	return Engage_Forms_Field_Util::is_file_field( $field ) && ! self::should_add_to_media_library($field);
    }
    ...
    public static function get_upload_handler( $form, $field ){
	     ...
	     * @param array $field Field config
	     */
    	return apply_filters( 'engage_forms_file_upload_handler', array( 'Engage_Forms_Files', 'upload' ), $form, $field );
    }
    ...
    public static function types(){
    	return array(
    	    'advanced_file',
            'file',
            \engagewp\engageforms\ef2\Fields\FieldTypes\FileFieldType::getCf1Identifier(),
            \engagewp\engageforms\ef2\Fields\FieldTypes\FileFieldType::getType()
        );
    }
	...
	public static function schedule_delete($field_id, $form_id, $file )
	{
		...
		if ( is_array($form) ) {
			...
			if ( is_array($field) && !self::is_persistent($field) ) {
				engage_forms_schedule_job(new \engagewp\engageforms\ef2\Jobs\DeleteFileJob($file));
				return true;
			}
		}
