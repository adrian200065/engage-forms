...
class Engage_Forms_API_Token {
	...
	public static function make_token( $lowest_role, $form_id ){
		 ...
		 */
		$secret = apply_filters( 'engage_forms_api_token_secret', get_option( 'engage_forms_api_token_secret', NONCE_SALT . md5_file( __FILE__ ) ), $form_id  );
		return sha1( 'ef_viewer_' . $lowest_role . $secret  . $form_id );
	...
	public static function check_token( $token, $form_id, WP_User $user = null ){
		...
		foreach( array_merge( array_keys( engage_forms_get_roles() ), array('public') ) as $role ){
