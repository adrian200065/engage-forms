 ...
 */
class Engage_Forms_API_Entries extends Engage_Forms_API_CRUD {
	 ...
	 * Get an entry
	 *
	 * GET /ef-api/v2/entries/form-id/entry-id
	 *
	 * @since 1.5.0
	 *
	...
	public function get_item( WP_REST_Request $request ) {
		...
		$entry = new Engage_Forms_Entry( $this->form->toArray(), $request[ 'entry_id' ] );

		if( null == $entry->get_entry() ){
			return Engage_Forms_API_Response_Factory::error_entry_not_found();
		}

		$data = $this->add_entry_to_response( $entry, array() );
		$data = $data[ $request[ 'entry_id' ] ];
		return Engage_Forms_API_Response_Factory::entry_data( $data, 1, 1 );
	...
	public function get_items( WP_REST_Request $request ) {
		...
		$entries = new Engage_Forms_Entry_Entries( $this->form->toArray(), $per_page );
		...
		return Engage_Forms_API_Response_Factory::entry_data(
			$data,
			count( $data ),
			$pages
		);
	 ...
	 * Delete an entry
	 *
	 * DELETE /ef-api/v2/entries/form-id/entry-id
	 *
	 * @since 1.5.0
	 *
	 * @param WP_REST_Request $request
	...
	public function delete_item( WP_REST_Request $request ) {
		...
		$result = Engage_Forms_Entry_Bulk::delete_entries( array( $request[ 'entry_id' ] ) );
		if( $result ){
			return new Engage_Forms_API_Response( __( 'Entry Deleted', 'engage-forms' ), 200, array() );
		}else{
			return new Engage_Forms_API_Error( 'not-deleted', __( 'Entry Not Deleted', 'engage-forms' ) );
		}
	}
     ...
     *
     * GET /ef-api/v2/entries/form-id/delete
     *
     * @since 1.7.0
     *
     * @param WP_REST_Request $request
    ...
    public function delete_entries( WP_REST_Request $request ) {
        ...
        if( false === Engage_Forms_Forms::is_internal_form( $formID ) ){
            $data = array(
                'deleted' => false,
                'message' =>  __( 'Form not found', 'engage-forms')
            );
            return new Engage_Forms_API_Response( $data, 404, array() );
        }

        $entries = \engagewp\EngageFormsQueries\EngageFormsQueries()->selectByFormId(  $formID, false );
        ...
        if( null != $entries ) {
            ...
            Engage_Forms_Entry_Bulk::delete_entries( $entryIds );
            $data = array(
                'deleted' => true,
                'message' =>  __( 'Entries deleted', 'engage-forms')
            );
            ...
            $data = array(
                'deleted' => false,
                'message' =>  __( 'No entries found', 'engage-forms')
            );
            return new Engage_Forms_API_Response( $data, 404, array() );
        }
	 ...
	 * @since 1.5.0
	 *
	 * @param array $entries Array of found Engage_Forms_Entry objects
	 *
	 * @return array
	 */
	protected function prepare_entries_for_response( $entries ){
		...
		if ( ! empty( $entries ) ) {
			/** @var Engage_Forms_Entry $entry Entry Object */
	 ...
	 * @since 1.5.0
	 *
	 * @param Engage_Forms_Entry $entry Entry object
	 * @param array $response_data Current response collection
	 *
	 * @return array
	 */
	protected function add_entry_to_response( Engage_Forms_Entry $entry,  array $response_data ){
		...
		if( ! empty( $fields ) ){
			/** @var Engage_Forms_Entry_Field $field */
		...
		if( ! empty( $metas ) ){
			/** @var Engage_Forms_Entry_Meta $meta */
		 ...
		 * @param Engage_Forms_Entry $entry Entry data
		 */
		$response_data[ $id ] = apply_filters( 'engage_forms_api_entry_data', $response_data[ $id ], $entry );
		return $response_data;
	}
	...
	public function get_items_permissions_check( WP_REST_Request $request ){
		$form_id =  $request[ 'form_id' ];
		$allowed = current_user_can( Engage_Forms::get_manage_cap( 'entry-view' ), $form_id );
		...
		/**
		 * Filter permissions for viewing entries via Engage Forms REST API
		 *
		 * @since 1.5.0
		 *
		 ...
		 */
		return apply_filters( 'engage_forms_api_allow_entry_view', $allowed, $form_id, $request );
	...
	public function create_item_permissions_check( WP_REST_Request $request ){
		...
		/**
		 * Filter permissions for creating, updating or deleting entries via Engage Forms REST API
		 *
		 * @since 1.5.0
		 *
		 * @param bool $allowed Is request authorized?
		 ...
		 * @param WP_REST_Request $request The current request
		 */
		return apply_filters( 'engage_forms_api_allow_entry_edit', $allowed, $request[ 'form_id' ], $request );
