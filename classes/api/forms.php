 ...
 */
class Engage_Forms_API_Forms extends  Engage_Forms_API_CRUD {
	...
	public function add_routes( $namespace ) {
		...
		$id_route[ \WP_REST_Server::EDITABLE ] = [
            ...
            'args'                => [
                'ef_edit_nonce' => [
                    'type' => 'string',
                    'description' => __('Engage Forms editor nonce', 'engage-forms'),
                    'required' => 'true'
                ],
                'config' => [
                    'type' => 'object',
                ...
                'form' => [
                    'type' => 'string',
                    'description' => __('ID of form', 'engage-forms'),
                    'required' => 'true'
                ],
            ]
        ];
     ...
     * 
     * Secures for POST /ef-api/v2/forms
     * 
     * @since 1.9.0
     */
	public function save_form_permissions_check(\WP_REST_Request $request){
        //Is allowed?
	    if( current_user_can( Engage_Forms::get_manage_cap( 'manage' ) )  ){
            //Allow if nonce valid
	        return wp_verify_nonce( $request['ef_edit_nonce'],'ef_edit_element');
        }
	    return false;
    }
     ...
     * 
     * Handler for POST /ef-api/v2/forms
     * 
     * @since 1.9.0
     */
	public function save_form(\WP_REST_Request $request){
        $saved = Engage_Forms_Admin::save_a_form(array_merge(['ID' => $request['form_id']],$request['config']));
        if( ! $saved ){
           return new \WP_Error(500,__('Not saved', 'engage-forms'));
        }
        return new WP_REST_Response(['form_id' => $saved,'form' => \Engage_Forms_Forms::get_form($saved)], 201 );
    }
    ...
    protected function privacy_route_args()
    {
        return array(
            'emailIdentifiers' => array(
                'type' => 'array',
                'required' => false,
                'description' => esc_html__( 'Array of fields that can be used to find personally identifying information saved with this form.', 'engage-forms' ),
                'sanitize_callback' => array( 'Engage_Forms_API_Util', 'validate_array_of_field_ids' )
            ),
            'piiFields' => array(
                'type' => 'array',
                'required' => false,
                'description' => esc_html__( 'Array of fields that contain personally identifying information', 'engage-forms' ),
                'sanitize_callback' => array( 'Engage_Forms_API_Util', 'validate_array_of_field_ids' )
            ),
     ...
     *
     * /ef-api/v2/forms/<form_id>
     *
     * @since 1.5.0
     *
     * @param WP_REST_Request $request
     *
     * @return Engage_Forms_API_Error|Engage_Forms_API_Response
     */
    public function get_item(WP_REST_Request $request){
	    ...
	    try{
		    ...
		    return Engage_Forms_API_Response_Factory::error_form_not_found();
	    }
        ...
        return new Engage_Forms_API_Response( $response_form, 200, array( ) );
     ...
     *
     * /ef-api/v2/forms/<form_id>/preview
     *
     * @since 1.5.8
     *
     ...
     *
     * @param WP_REST_Request $request
     * @return Engage_Forms_API_Response|Engage_Forms_API_Error
     */
    public function get_privacy_settings( WP_REST_Request $request )
    {
        ...
        return new Engage_Forms_API_Response( $this->form->toArray() );
    ...
    public function get_revisions( WP_REST_Request $request ){
    	...
    	$revisions = Engage_Forms_Forms::get_revisions( $request[ 'form_id' ], true );
	    $response_data = array();
	    foreach ( $revisions as $revision ){
		    $response_data[] = array(
		    	'id' => $revision,
			    'edit' => Engage_Forms_Admin::form_edit_link( $form_id, $revision )
		    );
	    }
	    if( empty( $response_data ) ){
		    return new Engage_Forms_API_Response( array( 'message' => __( 'No Revisions Found For This Form', 'engage-forms' ) ) );

	    }
	    return new Engage_Forms_API_Response( $response_data );
    }
    ...
    public function get_items(WP_REST_Request $request){
        ...
        $forms = Engage_Forms_Forms::get_forms( $request[ 'details' ] );
	    $prepared = array();
        ...
        $response = new Engage_Forms_API_Response( $prepared, 200, array( ) );
    ...
    public function get_items_permissions_check( WP_REST_Request $request ){
        $allowed = current_user_can( Engage_Forms::get_manage_cap( 'entry-view' ), $request[ 'form_id' ] );
	    if( ! $allowed ){
		    $allowed = Engage_Forms_API_Util::check_api_token( $request );
	    }
        /**
         * Filter permissions for viewing form config via Engage Forms REST API
         *
         * @since 1.5.0
         *
         ...
         */
        return apply_filters( 'engage_forms_api_allow_form_view', $allowed, $request[ 'form_id' ], $request );
    ...
    public function create_item_permissions_check( WP_REST_Request $request ){
        $allowed = current_user_can( Engage_Forms::get_manage_cap( 'entry-edit' ), $request[ 'form_id' ] );

        /**
         * Filter permissions for creating, updating or deleting forms via Engage Forms REST API
         *
         * @since 1.5.0
         *
         * @param bool $allowed Is request authorized?
         ...
         * @param WP_REST_Request $request The current request
         */
        return apply_filters( 'engage_forms_api_allow_form_edit', $allowed, $request[ 'form_id' ], $request );
    ...
    public function create_item( WP_REST_Request $request ) {
        ...
        $new_form = Engage_Forms_Forms::create_form( $data );
        if( ! empty($new_form ) ){
            try{
        ...
        return Engage_Forms_API_Response_Factory::error_form_not_created();
    ...
    public function args_for_create(){
        $templates = Engage_Forms_Admin::internal_form_templates();
        return [
            'ID' => [
                'type' => 'string',
                'description' => __( 'The desired form ID', 'engage-forms' ),
                'required' => false,
                'default' => '',
                'sanitize_callback' => 'engage_forms_very_safe_string'
            ],
            'name' => [
                'type' => 'string',
                'description' => __( 'The name for the form', 'engage-forms' ),
                'required' => false,
                'default' => '',
                'sanitize_callback' => 'engage_forms_very_safe_string'
            ],
            'type' => [
                'type' => 'string',
                'description' => __( 'The type of form to create', 'engage-forms' ),
                'required' => true,
                'default' => 'primary',
                'enum' => [
                    'primary',
            ...
            'template' => [
                'description' => __( 'The form template to use', 'engage-forms' ),
                'type' => 'string',
                'required' => false,
                'enum' => array_keys($templates)
            ],
            'clone' => [
                'description' => __( 'The ID of a form to clone', 'engage-forms' ),
                'type' => 'string',
                'required' => false,
            ]
        ];
	 ...
	 *
	 * @param Engage_Forms_API_Form $form
	 * @param WP_REST_Request $request
	 *
	 * @return array|mixed
	 */
    protected function prepare_form_for_response( Engage_Forms_API_Form $form, WP_REST_Request $request ){
	 ...
	 * @since 1.5.0
	 *
	 * @param Engage_Forms_API_Form $form Form config
	 *
	 * @return array
	 */
    protected function prepare_field_details( Engage_Forms_API_Form $form, WP_REST_Request $request ){
	    ...
	    $entry_list_defaults = array(
	    	'id' => array(
	    		'id' => 'id',
			    'label' => __( 'ID', 'engage-forms' )
		    ),
		    'datestamp' => array(
			    'id' => 'datestamp',
			    'label' => __( 'Submitted', 'engage-forms' )
		    ),
	    );
    ...
    protected function prepare_field(  &$field ){
		...
		$field = array(
			'id' => engage_forms_very_safe_string( $field[ 'ID' ] ),
			'label' =>  $label
        );
    }
    ...
    protected function preview_response(){
        $html = Engage_Forms::render_form($this->form->toArray());

        $css = array_merge(
            Engage_Forms_Render_Assets::get_core_styles(),
            Engage_Forms_Render_Assets::get_field_styles()
        );
        $js = array_merge(
            Engage_Forms_Render_Assets::get_core_scripts(),
            Engage_Forms_Render_Assets::get_field_scripts()
        );

        $prepared_css = array();
        $prepared_js = array();
        foreach ( $css as $key => $url ){
            $slug = Engage_Forms_Render_Assets::make_slug( $key );
            if( ! wp_style_is( $slug ) ){
                continue;
            }
        ...
        return new Engage_Forms_API_Response($data, 200, array());
    }
    ...
    public function toggle_active(\WP_REST_Request $request ){
        ...
        try{
			$form = Engage_Forms_Forms::get_form( $request[ 'form_id' ] );
        }catch ( Exception $e ){
            return Engage_Forms_API_Response_Factory::error_form_not_found();
        }
