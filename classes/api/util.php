...
/**
 * Utility functions for Engage Forms REST API
 *
 ...
 */
class Engage_Forms_API_Util {

	/**
	 * The namespace for Engage Forms REST API
	 *
	 * @since 1.4.4
	 *
	...
	public static function api_namespace($version = 'v2'){
		...
		return "ef-api/$version";
	}

	/**
	 * The URL for Engage Forms REST API
	 *
	 * @since 1.4.4
	 *
	...
	public static function check_api_token( WP_REST_Request $request ){
		$allowed = false;
			if( false != ( $token = $request->get_header( 'x_ef_entry_token') ) && is_string( $request[ 'form_id' ] ) ){
				$allowed = Engage_Forms_API_Token::check_token( $token, $request[ 'form_id' ] );
			}
    ...
    {
        $valid_fields = [];
        $form_fields = array_keys(Engage_Forms_Forms::get_fields( $form, false ));
        if (! empty( $field_ids )) {
