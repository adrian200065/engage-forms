 ...
 *
 * Will be placed in .ef-fieldjs-config for each field
 *
 ...
 */
class Engage_Forms_Field_JS implements JsonSerializable {
	...
	/**
	 * Engage_Forms_Render_FieldsJS constructor.
	 *
	 * @since 1.5.0
	 *
		...
		if( ! empty( $this->form[ 'fields' ] ) ){
			foreach( $this->form[ 'fields' ] as  $field ){
				...
				if( ! in_array( $type, $skip ) && method_exists( $this, $type ) ){
					call_user_func( [ $this, $type ], $field[ 'ID' ], $field );
				}
			}

			foreach( $this->fields[ 'defaults' ] as &$default ){
				if( !is_array( $default ) && $default !== null && 0 === strpos( $default, '%' ) ) {
					$default = $this->get_field_default(
						Engage_Forms_Field_Util::get_field_by_slug( str_replace( '%', '', $default ), $this->form )
					);
				}else{
					$default = Engage_Forms::do_magic_tags( $default, null, $this->form );
	...
	protected function button( $field_id ){
		$this->data[ $field_id ] = $this->create_config_array( $field_id, __FUNCTION__, [] );
	}
	...
	protected function field_id( $field_id ) {
		return Engage_Forms_Field_Util::get_base_id( $field_id, $this->form_count, $this->form );
	}
	...
	protected function wysiqyg_options( $field_id ) {
		 ...
		 * @param array $form Form Config
		 */
		$options = apply_filters( 'engage_forms_wysiwyg_options', $options, $field, $this->form );
	...
	protected function phone_better( $field_id, $field ){
		$args =  [
			...
			'messages' => [
				'generic' => __( 'Invalid number', 'engage-forms' ),
				'1' => __( 'Invalid country code', 'engage-forms' ),
				'4' => __( 'Not a number', 'engage-forms' ),
				'3' => __( 'Number is too long', 'engage-forms' ),
				'2' => __( 'Number is too short', 'engage-forms' )
	...
	protected function better_phone_field_js_options( array  $field  ){
		$options = [
			...
			'utilsScript' => EFCORE_URL . 'fields/phone_better/assets/js/utils.js',
		 ...
		 *
		 */
		return apply_filters( 'engage_forms_phone_js_options', $options, $field, $this->form );
	}
	...
	protected function html( $field_id, $field ){
		...
		$this->data[ $field_id ] = $this->create_config_array( $field_id, Engage_Forms_Field_Util::get_type( $field ), [
			'sync' => false
		] );

		/** @var Engage_Forms_Sync_HTML $syncer */
		$syncer = Engage_Forms_Sync_Factory::get_object( $this->form, $field, $id_attr, $this->form_count );

		if ( $syncer->can_sync() ) {
			$this->data[ $field_id ] = array_merge( $this->data[ $field_id ], [
				'sync'       => true,
	...
	protected function star_rating( $field_id, $field ){
		...
		$args = array(
			'starFieldId' => Engage_Forms_Field_Util::star_target( Engage_Forms_Field_Util::get_base_id( $field, $this->form_count, $this->form ) ),
			'options' => array(
				'starOn' => 'raty-'.  $type . '-on',
	...
	protected function calculation( $field_id, $field ){
		...
		/** @var Engage_Forms_Sync_Calc $syncer */
		$syncer = Engage_Forms_Sync_Factory::get_object( $this->form, $field, Engage_Forms_Field_Util::get_base_id( $field, null, $this->form ) );
		...
		$args = array(
			...
			'callback' => Engage_Forms_Field_Calculation::js_function_name( $target_id )
		);
	...
	protected function credit_card_number( $field_id, $field ){
		...
		$this->data[ $field_id ] = $this->create_config_array( $field_id, __FUNCTION__, array(
			'invalid' => __( 'Invalid Card Number', 'engage-forms' ),
			'cvc' => false,
			'exp' => false,
		) );

		if( Engage_Forms_Render_Assets::should_minify( false ) ){
			$this->data[ $field_id ][ 'imgPath' ] = EFCORE_URL . 'assets/build/images/';
		}else{
			$this->data[ $field_id ][ 'imgPath' ] = EFCORE_URL . 'assets/images/';
		}
	...
	protected function credit_card_exp( $field_id, $field ){
		...
		$this->data[ $field_id ] = $this->create_config_array( $field_id, __FUNCTION__, array(
			'invalid' => __( 'Invalid Expiration Date', 'engage-forms' ),
			'ccField' => false
		) );
	...
	protected function credit_card_cvc( $field_id, $field ){
		...
		$this->data[ $field_id ] = $this->create_config_array( $field_id, __FUNCTION__, array(
			'invalid' => __( 'Invalid Security Code', 'engage-forms' ),
			'ccField' => false
		) );
	...
	public function color_picker( $field_id, $field ){
		 ...
		 * @param array $form Form config
		 */
		$settings = apply_filters( 'engage_forms_field_js_color_picker_settings', array(
			'animationSpeed' => 50,
	...
	protected function default_config_args(){
		 ...
		 * @param array $args
		 */
		return apply_filters( 'engage_forms_field_js_config_defaults', array(
			'form_id' => $this->form[ 'ID' ],
			'form_id_attr' => Engage_Forms_Render_Util::form_id_attr( $this->form_count )
	...
	protected function map_field( $type, $field ){
		...
		if( 'checkbox' === $type ){
			$_field[ 'mode' ] = Engage_Forms_Field_Calculation::checkbox_mode( $field, $this->form );
		}
	...
	protected function get_field_default( $field ){
		$default = Engage_Forms_Field_Util::get_default( $field, $this->form, true );
		if( is_bool( $default ) ){
			$default = '';
		}
		...
		switch ( $type ){
			...
			case 'toggle_switch' :
				$default = Engage_Forms_Field_Util::find_select_field_value( $field, $default );
				break;
	...
	protected function map_default( $field, $default ){
		...
		if ( 'calculation' != $this->get_field_type( $field ) ) {
			$calc_default = Engage_Forms_Field_Util::get_default_calc_value( $field, $this->form );
	...
	protected function calc_target_id( array  $field ){
		return Engage_Forms_Field_Util::get_base_id( $field, $this->form_count, $this->form );
	...
	protected function get_field_type( $field ){
		if( ! isset( $this->types[ $field[ 'ID' ] ] ) ){
			$this->types[ $field[ 'ID' ] ] = Engage_Forms_Field_Util::get_type( $field, $this->form );
		}
