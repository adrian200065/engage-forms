 ...
 */
class Engage_Forms_Field_Input extends Engage_Forms_Field_HTML{
	...
	public static function html( array $field, array $field_structure, array $form, $value = null ){
		...
		$field_base_id = Engage_Forms_Field_Util::get_base_id( $field, null, $form );
		if ( null === $value ) {
			if( isset( $field_structure [ 'field_value' ] ) ){
				$value = Engage_Forms::do_magic_tags( $field_structure [ 'field_value' ], null, $form );
			}else{
		...
		if( in_array( $type, self::sync_fields() ) ){
			$syncer = Engage_Forms_Sync_Factory::get_object( $form, $field, $field_base_id );
		...
		if( ! empty( $mask ) ){
			Engage_Forms_Render_Assets::enqueue_script( 'inputmask' );
		}
		...
		if('color_picker' === $type ){
			...
			$attrs[ 'type' ] = 'tel';
			$attrs[ 'class' ][] = 'ef-credit-card ';
			$attr[ 'data-parsley-creditcard' ] = Engage_Forms_Field_Util::credit_card_types( $field, $form );
	...
	protected static function get_mask_string( array  $field ){
		$mask = '';
		if ( 'phone' != Engage_Forms_Field_Util::get_type( $field ) ) {
