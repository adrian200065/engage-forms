 ...
 */
class Engage_Forms_Field_Calculation {
	...
	public static function checkbox_mode( $field, $form ){
		 ...
		 *
		 */
		return (bool) apply_filters( 'engage_forms_checkbox_calculate_sum', true, $field, $form );
	...
	public static function get_value( $field, $form ){
		$entry_value = Engage_Forms::get_field_data( $field[ 'ID' ], $form );
					...
					if( true == self::checkbox_mode( $field, $form ) ){
						if( ! empty(  $entry_value ) ){
							foreach ( $entry_value as $opt => $value ){
								 $entry_value[ $opt ] = Engage_Forms_Field_Util::get_option_calculation_value( $opt, $field, $form );
							}
		 ...
		 */
		$number = apply_filters( 'engage_forms_calculation_field_get_value', $number, $entry_value, $field, $form );
	...
	protected static function find_calc_value( $entry_value, $field, $form ){
		foreach ( $field[ 'config' ][ 'option' ] as $opt_id => $option ){
			if( $entry_value == $option[ 'value' ] ){
				$entry_value = Engage_Forms_Field_Util::get_option_calculation_value( $option, $field, $form );
				break;
			}
