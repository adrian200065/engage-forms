...
/**
 * Class Engage_Forms_Admin_Privacy
 *
...
class Engage_Forms_Admin_Privacy{
	...
	/**
	 * Engage_Forms_Admin_Privacy constructor.
	 *
	 * @since 1.6.1
	 *
	 * @param array $form
	 ...
	 * @since 1.6.1
	 *
	 * @uses "engage_forms_field_wrapper_before_field_setup" action
	 *
	...
	public function add_personally_identifying_question($config, $type, $field_id ){
		if(in_array( $type, $this->fields_to_show_personally_identifying_question() )){
			$id_attr = $field_id . '_' . Engage_Forms_Field_Util::CONFIG_PERSONAL;
			$description_id_attr = $id_attr . '_description';
			$is_personally_identifying = Engage_Forms_Field_Util::is_personally_identifying($field_id, $this->form );
			/* translators: it's abbreviation for Personally Identifying Information */
			$pii_text = esc_html__( 'PII', 'engage-forms' );
			printf( '
				<div class="engage-config-group privacy-field personally-identifying-field">
					<label for="%s">
						%s
					</label>
			...
			',
				...
				sprintf( 'config[fields][%s][config][%s]',
					esc_attr($field_id),
					esc_attr( Engage_Forms_Field_Util::CONFIG_PERSONAL )
				),
				esc_attr($description_id_attr),
				$is_personally_identifying ? 'checked=checked' : '',
				esc_attr($description_id_attr),
				sprintf( '%s <a href="%s" target="_blank">%s</a>',
					esc_html__( 'Does field contain Personally Identifying Information (PII)?', 'engage-forms' ),
					'https://engageforms.com/gdpr#pii?utm_source=wp-admin&utm_medium=form-editor&utm_content=pii-learn-more',
	...
	protected function fields_to_show_personally_identifying_question(){
		if( ! $this->fields_to_show_personally_identifying_question ){
			...
			foreach( Engage_Forms_Fields::get_all() as $field_type => $field_config ){
				if ( Engage_Forms_Fields::not_support( $field_type, 'entry_list' ) ){
					continue;
				}
