...
class Engage_Forms_Admin_Page
{
    ...
    /**
     * Used to form menu slug `engage-forms-$page_postfix`
     *
     * @since 1.7.0
     *
     * @var  string
    ...
    /**
     * Engage_Forms_Admin_Page constructor.
     *
     * @since 1.7.0
     *
     * @param string $page_postfix Used to form menu slug `engage-forms-$page_postfix`
    ...
    {
        $label = sprintf(
            '<span class="engage-forms-menu-dashicon"><span class="dashicons dashicons-%s"></span>%s</span>', esc_attr($this->menu_icon), $this->menu_label);
    ...
    public function render()
    {
        ...
        engage_forms_print_ef_forms_var($handle);
        wp_enqueue_script('wp-api-request');
        Engage_Forms_Admin_Assets::set_ef_admin(Engage_Forms_Render_Assets::make_slug($handle));
         ...
         * @since 1.7.0
         *
         * @param Engage_Forms_Admin_Page $page
         */
        do_action('engage_forms_client_element_rendered', $this);
    ...
    public function enqueue_assets()
    {
        ...
        if (!empty($this->assets['scripts'])) {
            foreach ($this->assets['scripts'] as $handle) {
                Engage_Forms_Render_Assets::enqueue_script($handle);
            }
        }

        if (!empty($this->assets['styles'])) {
            foreach ($this->assets['styles'] as $handle) {
                Engage_Forms_Render_Assets::enqueue_style($handle);
            }
        }

        Engage_Forms_Admin_Assets::enqueue_style('editor-grid');
    ...
    {
        return Engage_Forms::PLUGIN_SLUG . '-';
    }
