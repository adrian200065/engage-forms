function EngageFormsAdminClippys2( elId, config, $ ){
	...
	this.init = function () {
		...
		var docsComponent = {
			template:  '#tmpl--engage-help-clippy',
			props: [ 'important' ],
			methods: {
				link:linkCB,
		...
		var extendComponent = {
			template: '#tmpl--engage-extend-clippy',
			props: [ 'product','title' ],
			methods: {
				link:linkCB,
		...
		$.when(
			get(config.efdotcom.api.important),
			get(config.efdotcom.api.product)
...
function EngageFormsAdminClippys(elId, config, $){
	return new EngageFormsAdminClippys2( elId, config, $ );
}