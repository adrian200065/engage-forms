...
jQuery( function(){
	...
	jQuery('body').on('click', '.modal-side-tab', function(e){
		...
		var clicked = jQuery(this),
			parent = clicked.closest('.engage-modal-body'),
	...
	jQuery('body').on('click', '.engage-group-remover', function(e){

		e.preventDefault();

		var clicked = jQuery(this),
			parent = clicked.closest('.engage-repeater-group');
	...
	jQuery( window ).on('resize', function(){
		...
		var list_toggle = jQuery('#ef_forms_toggle'),
   ...
   */
  //Display controls
  jQuery('#engage-forms-delete-all-form-entries').on('click', function(e) {
    e.preventDefault();
    jQuery('#engage-forms-confirm-delete-all-form-entries').slideToggle("fast");
  });
  //No clicked
  jQuery('#engage-forms-no-confirm-delete-all-form-entries').on('click',function(e) {
    e.preventDefault();
    jQuery('#engage-forms-confirm-delete-all-form-entries').slideToggle("fast");
  });
  //Yes clicked
  jQuery('#engage-forms-yes-confirm-delete-all-form-entries').on('click',function(e) {
    e.preventDefault();

    var url = EF_ADMIN.rest.delete_entries;

    var $spinner = jQuery( '#engage-forms-delete-entries-spinner' );
    $spinner.css({
      visibility: 'visible',
      float:'none'
    });
    ...
    wp.apiRequest({
      ...
      if( r.hasOwnProperty( 'message' ) ) {
        if (r.deleted === true) {
          jQuery('#engage-forms-label-delete-all-entries').append("<div class='engage-forms-deleted'>" + r.message + "</div>");
          setTimeout(function () {
            jQuery('.engage-forms-deleted').remove();
          }, 5000);
          jQuery('#engage-forms-confirm-delete-all-form-entries').slideToggle("fast");
        } else {
          jQuery('#engage-forms-label-delete-all-entries').append("<div class='engage-forms-not-deleted'>" + r.message + "</div>");
          setTimeout(function () {
            jQuery('.engage-forms-not-deleted').remove();
          }, 5000);
          jQuery('#engage-forms-confirm-delete-all-form-entries').slideToggle("fast");
        }
      }
      ...
      if( r.responseJSON.hasOwnProperty( 'message' ) ) {
        jQuery('#engage-forms-label-delete-all-entries').append("<div class='engage-forms-not-deleted'>" + r.responseJSON.message + "</div>");
        setTimeout(function () {
          jQuery('.engage-forms-not-deleted').remove();
        }, 5000);
        jQuery('#engage-forms-confirm-delete-all-form-entries').slideToggle("fast");
 ...
 * @constructor
 */
function EngageFormsButtonPulse( $btn ){
