/*! GENERATED SOURCE FILE engage-forms - v3.0.0 - 2022-03-22 *//**
 ...
 *
 * Can be used constructed with EFAPI() or other data. Designed to provide data to our VueJS entry viewer, but is framework agnostic.
 *
 * @since 1.50
 *
 ...
 * @constructor
 */
function EFFormStoreFactory( formId, allFields, listFields ){
    return {
        state: {
            formId: formId,
            allFields: allFields,
 ...
 * @constructor
 */
function EFEntriesStoreFactory( formId, entries ){
    return {
        state: {
            formId: formId,
            entries: entries,
...
/**
 * A VueJS-powered entry viewer for Engage Forms
 *
 * @since 1.5.0
 *
 * @param formId Form ID
 * @param formStore Form data store, should be created with EFFormStoreFactory()
 * @param entryStore Entry data store, should be created with EFEntriesStoreFactory()
 * @param api API instance for this form. Should be instance of EFAPI
 * @param config Configuration. Probably EF_ENTRY_VIEWER_2_CONFIG, but you can add your own if you like.
 *
 * @returns {*}
 *
 * @constructor
 */
function EFEntryViewer2( formId, formStore, entryStore, api, config ){
    ...
    return new Vue({
        ...
        methods:{
            paginationButtons: function(){
                ...
                var $next = $el.find( '.engage-forms-entry-viewer-next-button' ),
                    $prev = $el.find( '.engage-forms-entry-viewer-prev-button' );
            ...
            notAllowed: function (r) {
                if ( 'object' != typeof  r && 404 != r.status ) {
                    ...
                    jQuery('#engage-forms-entries-nav').remove();
                    jQuery(document.getElementById('engage-forms-entries')).html('<div class="alert alert-warning">' + config.strings.not_allowed + '</div>');
                }
            }
        }
    });
