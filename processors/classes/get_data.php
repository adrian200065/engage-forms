...
class Engage_Forms_Processor_Get_Data implements Engage_Forms_Processor_Data{
	...
	protected function set_fields( $fields, $config, $form ) {
		$message_pattern = __( '%s is required', 'engage-forms' );
		$default_args = array(
			'message' => false,
			'default' => false,
			'sanatize' => 'strip_tags',
		...
		foreach( $fields as $field  => $args ) {
			...
			if ( isset( $config[ $key ] ) ) {
				$_field = Engage_Forms_Field_Util::get_field_by_slug( str_replace( '%', '', $config[ $key ] ), $form );
			} else {
				$_field = null;
			}
	...
	protected function set_value( $config, $form ) {
		foreach ( $this->fields as $field => $args  ) {
			...
			if ( isset( $config[ $field ]) ) {
				if ( $args[ 'magic' ] ) {
					$value = Engage_Forms::do_magic_tags( $config[ $field ], null, $form );
				} else {
					$value = $config[ $field ];
				}
				...
				if ( false !== $field_id_passed ) {
					$value = Engage_Forms::get_field_data( $value, $form );
				}
			 ...
			 * @param array $form Form config.
			 */
			$value = apply_filters( 'engage_forms_processor_value', $value, $field, $args, $config, $form );
