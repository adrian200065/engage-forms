 ...
 */
class Engage_Forms_Processor_Load {
	 ...
	 * @access private
	 *
	 * @var Engage_Forms_Processor_Load
	 */
	private static $init;
	...
	private function load_processors() {
		 ...
		 *
		 * Uses "engage_forms_get_form_processors" filter to add processors
		 *
		 * @since 1.3.5.3
		 */
		do_action( 'engage_forms_pre_load_processors' );
		 ...
		 */
		$processors = apply_filters( 'engage_forms_get_form_processors', array() );
		 ...
		 *
		 * Use: $processors = Engage_Forms_Processor_Load::get_instance()->get_processors(); to get processors
		 *
		 * @since 1.3.5.3
		 */
		do_action( 'engage_forms_post_load_processors' );
	...
	private function validate_processor( $processor ) {
		...
		if ( false != $processor[ 'ef_ver' ] ) {
			if ( ! version_compare( EFCORE_VER, $processor[ 'ef_ver' ], '>=' ) ) {
				$this->add_notice(  __( sprintf( $pattern, $processor[ 'name'], 'Engage Forms', $processor[ 'ef_ver' ], EFCORE_VER  ) ) );
				$fail = true;
			}
		...
		if ( false != $processor[ 'php_ver' ] ) {
			if ( ! version_compare( PHP_VERSION, $processor[ 'php_ver' ], '>=' ) ) {
				$this->add_notice(  __( sprintf( $pattern, $processor[ 'name'], 'PHP', $processor[ 'php_ver' ], EFCORE_VER  ) ) );
				$fail = true;
			}
		...
		if ( false != $processor[ 'wp_ver' ] ) {
			...
			if ( ! version_compare( $wp_version, $processor[ 'wp_ver' ], '>=' ) ) {
				$this->add_notice( __( sprintf( $pattern, $processor[ 'name' ], 'WordPress ', $processor[ 'wp_ver' ], EFCORE_VER  ) ) );
				$fail = true;
	 ...
	 * @since 1.3.0
	 *
	 * @return \Engage_Forms_Processor_Load
	 */
	...
	protected function add_notice( $notice ) {
		if ( class_exists( 'Engage_Forms_Admin' ) ) {
			Engage_Forms_Admin::add_admin_notice( $notice );
		}
