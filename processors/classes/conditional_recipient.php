 ...
 */
class Engage_Forms_Processor_Conditional_Recipient  {
	...
	public static function post_processor( $config  ){
        ...
        foreach( $recipients as $recipient){
            if( is_email( trim( $recipient ) )){
                ...
                $field = trim( Engage_Forms::do_magic_tags($recipient) );
                if(is_email( $field )){
		...
		if( ! empty( $config[ 'remove-default' ]  ) && $config[ 'remove-default' ] ){
			add_filter( 'engage_forms_mailer', array( __CLASS__, 'remove_default' ), 2  );
		}

		add_filter( 'engage_forms_mailer', array( __CLASS__, 'set_mailer_recipients') );
	}
