 ...
 */
class Engage_Forms_Processor_UI {
	...
	public static function config_field( $args ) {
		 ...
		 * @param array $args
		 */
		$args = apply_filters( 'engage_forms_addon_config_field_args', $args );
		 ...
		 * 
		 */
		$classes = implode( ' ', apply_filters( 'engage_forms_ui_field_classes', $classes, $args[ 'id' ], $args ) );
		...
		if ( 'hidden' != $args[ 'type' ] ) {
			$field = sprintf('
		<div class="engage-config-group" id="%s">
			<label for="%s" id="%s">
				%s
			</label>
			<div class="%s">
			...
			%s
				...
				$args[ 'label' ],
				esc_attr( 'engage-config-field ' . $id . '-wrap' ),
		 ...
		 * @param array $args Field args
		 */
		return apply_filters( 'engage_forms_processor_ui_input_group_html', $field, $input_type, $id, $args );
	...
	public static function input( $type, $args, $id, $classes, $required, $has_desc ) {
		 ...
		 *
		 * By returning a string, rest of this method -- Engage_Forms_Processor_UI::input() -- is skipped
		 *
		 * @since 1.4.0
		 *
		 ...
		 * @param bool $has_desc Does this input have a description?
		 */
		$field_html = apply_filters( 'engage_forms_processor_ui_input_pre_html', null, $type, $args, $id, $classes, $required, $has_desc );
		 ...
		 * @param array $args Field args
		 */
		return apply_filters( 'engage_forms_processor_ui_input_html', $field, $type, $id, $args );
	...
	public static function ssl_notice( $name ) {
		...
		return sprintf( '<div class="error"><p>%1s</p></div>', __( sprintf( '%1s requires a valid SSL certificate. It will only function in test mode unless SSL/HTTPS is used. Your site does not appear to be using the secure HTTPS protocol.', $name ), 'engage-forms' ) );
