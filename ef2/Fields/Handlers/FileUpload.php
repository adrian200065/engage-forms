	...
	public function processFiles(array $files, array $hashes)
	{
		...
		foreach ( $files as $file ) {
			if ( !$this->isAllowedType($file) ) {
				throw new Exception(__('A file type is not allowed. Please try another.', 'engage-forms'), 415);
			}

			if( $this->isFileTooLarge($file)){
				throw new Exception(__('A file is too large. Please try another.', 'engage-forms'), 415);
			}
			...
			$isPrivate = \Engage_Forms_Files::is_private($this->field);
			...
			if ( \Engage_Forms_Files::should_add_to_media_library($this->field) ) {
				\Engage_Forms_Files::add_to_media_library($upload, $this->field);
			}
