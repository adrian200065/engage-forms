...
if(!empty($field['config']['before'])){
	$field['config']['before'] = Engage_Forms::do_magic_tags($field['config']['before']);
}
if(!empty($field['config']['after'])){
	$field['config']['after'] = Engage_Forms::do_magic_tags($field['config']['after']);
}
...
$target_id = Engage_Forms_Field_Util::get_base_id( $field, $current_form_count, $form );
...
<?php
Engage_Forms_Render_Util::add_inline_script( $syncer->get_formula( true ), $form );