...
$attrs = array(
	...
	'name' => $field_name,
	'value' => $field_value,
	'id' => Engage_Forms_Field_Util::get_base_id( $field ),
	'data-field' => $field_base_id,
	'class' => $field_class . ' efdatepicker is-efdatepicker',
...
if( !empty( $field['config']['language'] ) ){
	if( file_exists( EFCORE_PATH . 'fields/date_picker/js/locales/bootstrap-datepicker.' . $field['config']['language'] . '.js' ) ){
		$attrs[ 'data-date-language' ] = $field['config']['language'];
		wp_enqueue_script( 'ef-frontend-date-picker-lang', EFCORE_URL . 'fields/date_picker/js/locales/bootstrap-datepicker.' . $field['config']['language'] . '.js', array( Engage_Forms_Render_Assets::field_script_to_localize_slug() ), null, true);
	}
}
...
if( ! empty( $field[ 'hide_label' ] ) ) {
	...
	$attrs[ 'placeholder' ]  = Engage_Forms::do_magic_tags( $place_holder );
