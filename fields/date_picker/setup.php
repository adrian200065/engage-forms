<div class="engage-config-group">
	...
	<div class="engage-config-field">
		<input id="{{_id}}_default" type="text" class="block-input field-config is-not-efdatepicker magic-tag-enabled" data-dontprovide="efdatepicker" id="{{id}}" data-date-format="{{format}}" name="{{_name}}[default]" value="{{default}}" />
	</div>
</div>
<div class="engage-config-group">
	<label for="{{_id}}_format">
        <?php esc_html_e('Format', 'engage-forms'); ?>
    </label>
	<div class="engage-config-field">
		<input id="{{_id}}_format" type="text" class="efdatepicker-set-format block-input field-config" id="{{id}}" name="{{_name}}[format]" value="{{format}}">
	</div>
</div>
<div class="engage-config-group">
    <label>
        <?php esc_html_e('Autoclose', 'engage-forms'); ?>
    </label>
    <div class="engage-config-field">
        <label id="{{_id}}_autoclosed">
            ...
            <?php esc_html_e('Enable autoclose', 'engage-forms'); ?>
        </label>
        <p class="description" id="{{_id}}_autoclosed-description">
            <?php esc_html_e('If enabled, the date picker will automatically close after selecting the final input', 'engage-forms'); ?>
        </p>
    </div>
</div>
<div class="engage-config-group">
    <label for="{{_id}}_startview">
        <?php esc_html_e('Start View', 'engage-forms'); ?>
    </label>
    <div class="engage-config-field">
        <select class="block-input field-config" name="{{_name}}[start_view]" id="{{_id}}_startview" aria-describedby="{{_id}}_startview-description">
            <option value="month" {{#is start_view value="month"}}selected="selected"{{/is}}>
                <?php esc_html_e('Month (Default)', 'engage-forms'); ?>
            </option>
            <option value="year" {{#is start_view value="year"}}selected="selected"{{/is}}>
                <?php esc_html_e('Year', 'engage-forms'); ?>
            </option>
            <option value="decade" {{#is start_view value="decade"}}selected="selected"{{/is}}>
                <?php esc_html_e('Decade', 'engage-forms'); ?>
            </option>
        </select>
        <p class="description" id="{{_id}}_startview-description"><?php _e('The starting view of the date picker (month, year, decade)', 'engage-forms'); ?></p>
    </div>
</div>
<div class="engage-config-group">
    <label for="{{_id}}_startdate">
        <?php esc_html_e('Start Date', 'engage-forms'); ?>
    </label>
    <div class="engage-config-field">
        <input id="{{_id}}_startdate" aria-describedby="{{_id}}_startdate-description" type="text" class="efdatepicker-set-format block-input field-config" name="{{_name}}[start_date]" value="{{start_date}}">
        <p class="description" id="{{_id}}_startdate-description">
            <?php esc_html_e('The starting date of the date picker like +1d, -2y, +4m. 0d for today.', 'engage-forms'); ?>
        </p>
    </div>
</div>
<div class="engage-config-group">
    <label for="{{_id}}_end-date">
        <?php esc_html_e('End Date', 'engage-forms'); ?>
    </label>
    <div class="engage-config-field">
        <input id="{{_id}}_end-date" aria-describedby="{{_id}}_end-date-description" type="text" class="efdatepicker-set-format block-input field-config" name="{{_name}}[end_date]" value="{{end_date}}">
        <p class="description" id="{{_id}}_end-date-description">
                <?php esc_html_e('The ending date of the date picker like +1d, -2y, +4m. 0d for today.', 'engage-forms'); ?>
        </p>
    </div>
</div>
<div class="engage-config-group">
	<label for="{{_id}}_language">
        <?php esc_html_e('language', 'engage-forms'); ?>
    </label>
	<div class="engage-config-field">
		<select id="{{_id}}_language" aria-describedby="{{_id}}_language-description" class="efdatepicker-set-language block-input field-config" id="{{id}}" name="{{_name}}[language]" style="width: 90px;">
		...
		<p class="description" id="{{_id}}_language-description">
            <?php esc_html_e('Language to use. e.g. pt-BR', 'engage-forms'); ?>
        </p>
	</div>
</div>
