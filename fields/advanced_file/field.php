<?php
	...
	$uniqu_code = Engage_Forms_Field_Util::generate_file_field_unique_id($field,$form);
	...
	if( empty( $field['config']['multi_upload_text'] ) ){
		$field['config']['multi_upload_text'] = __( 'Add File', 'engage-forms' );
		if( !empty( $field['config']['multi_upload'] ) ){
			$field['config']['multi_upload_text'] = __( 'Add Files', 'engage-forms' );
		}
	}
	...
	//Fix allowed types
    // @see https://github.com/EngageWP/Engage-Forms/issues/2471
    if ( ! empty( $field['config']['allowed'] )) {
	...
	$field['config']['notices'] = array(
		'file_exceeds_size_limit' => esc_html__( 'File exceeds the maximum upload size for this site.', 'engage-forms' ),
		'zero_byte_file' => esc_html__( 'This file is empty. Please try another.', 'engage-forms' ),
		'invalid_filetype' => esc_html__( 'This file type is not allowed. Please try another.', 'engage-forms' ),
	);
	...
	<?php echo $field_label; ?>
		<?php echo $field_before; ?>
			<div
					...
					class="ef-multi-uploader-list"
			></div>

			<button
					id="<?php echo esc_attr($field_id); ?>_trigger"
					type="button"
					class="btn btn-block ef-uploader-trigger"
