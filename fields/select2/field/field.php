...
if( !empty( $field['config']['advanced_populate']['filter'] ) ){
	...
	if(!empty($hastags[1])){
		...
		foreach($hastags[1] as $tag_key=>$tag){
			foreach($form['fields'] as $key_id=>$fefg){
				if($fefg['slug'] === $tag){
...
if( empty( $field['config']['color'] ) ){
	...
	if( !empty( $field['config']['placeholder'] ) ){
		$placeholder = 'data-placeholder="' . esc_attr( Engage_Forms::do_magic_tags( $field['config']['placeholder'] ) ). '"';
	}
	...
	<?php echo $field_before; ?>
		...
		if(!empty($field['config']['option'])){
			...
			foreach($field['config']['option'] as $option_key=>$option){
				...
				?>
				<option
					data-calc-value="<?php echo esc_attr( Engage_Forms_Field_Util::get_option_calculation_value( $option, $field, $form ) ); ?>"
...
window.addEventListener('DOMContentLoaded', function(){
	jQuery( function($){
		...
		$(document).on('ef.bind', '#<?php echo esc_html( $field_id ); ?>', function() {
			$(this).select2( opts );
		});
	});
});
