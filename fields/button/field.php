...
if($field['config']['type'] == 'next' || $field['config']['type'] == 'prev'){
	...
	$field[ 'config' ][ 'class' ] = $field[ 'config' ][ 'class' ] . ' ef-page-btn ef-page-btn-' . $field[ 'config' ][ 'type' ];
}elseif( 'button' == $field['config']['type' ] ){
	$btnType = 'button';
	if( !empty( $field['config']['target'] ) ){
		$field['config']['class'] .= ' ef-form-trigger';
...
$attr_string_button =  engage_forms_field_attributes( $attrs, $field, $form );

$attrs = array(
	'class' => 'button_trigger_' . Engage_Forms_Render_Util::get_current_form_count(),
	'type' => 'hidden',
	'name' => $field_name,
	'id' => $field_id . '_btn',
	'value' => $field_value,
...
$attr_string_hidden_field =  engage_forms_implode_field_attributes( engage_forms_escape_field_attributes_array( $attrs ) );
