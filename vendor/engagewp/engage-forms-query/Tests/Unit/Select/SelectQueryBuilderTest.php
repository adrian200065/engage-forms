...
class SelectQueryBuilderTest extends TestCase
{
	...
	{
		$entry = $this->entryGeneratorFactory();
		$expectedSql = "SELECT `{$this->entryTableName()}`.* FROM `{$this->entryTableName()}` WHERE (`{$this->entryTableName()}`.`form_id` = 'ef12345') ORDER BY `{$this->entryTableName()}`.`form_id` DESC";
		$entry->queryByFormsId('ef12345');
	...
	{
		$entry = $this->entryGeneratorFactory();
		$expectedSql = "SELECT `{$this->entryTableName()}`.* FROM `{$this->entryTableName()}` WHERE (`{$this->entryTableName()}`.`form_id` = 'ef12345') ORDER BY `{$this->entryTableName()}`.`form_id` ASC";
		$entry->queryByFormsId('ef12345');
